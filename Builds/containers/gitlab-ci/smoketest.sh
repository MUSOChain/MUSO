...
if [ "$use_private" -gt 0 ] ; then
    REPO_ROOT="https://MUSO:${ARTIFACTORY_DEPLOY_KEY_MUSO}@${ARTIFACTORY_HOST}/artifactory"
else
...
if [ "${pkgtype}" = "dpkg" ] ; then
    ...
    if [ "${install_from}" = "repo" ] ; then
        ...
        # apt-cache policy MUSO
        apt-get -y install MUSO=${dpkg_full_version}
    elif [ "${install_from}" = "local" ] ; then
        # cached pkg install
        updateWithRetry
        apt-get -y install libprotobuf-dev libssl-dev
        rm -f build/dpkg/packages/MUSO-dbgsym*.*
...
INSTALLED=$(/opt/MUSO/bin/MUSO --version | awk '{print $NF}')
if [ "${MUSO_version}" != "${INSTALLED}" ] ; then
    echo "INSTALLED version ${INSTALLED} does not match ${MUSO_version}"
    exit 1
fi
# run unit tests
/opt/MUSO/bin/MUSO --unittest --unittest-jobs $(nproc)
/opt/MUSO/bin/validator-keys --unittest
