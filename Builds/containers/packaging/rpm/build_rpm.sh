...
cd /opt/MUSO_bld/pkg
cp -fpu MUSO/Builds/containers/packaging/rpm/MUSO.spec .
cp -fpu MUSO/Builds/containers/shared/update_sources.sh .
source update_sources.sh

# Build the rpm

IFS='-' read -r MUSO_RPM_VERSION RELEASE <<< "$MUSO_VERSION"
export MUSO_RPM_VERSION
...
git archive --format tar.gz --prefix MUSO/ -o ../rpmbuild/SOURCES/MUSO.tar.gz HEAD
# TODO include validator-keys sources
cd ..

source /opt/rh/devtoolset-8/enable

rpmbuild --define "_topdir ${PWD}/rpmbuild" -ba MUSO.spec
rc=$?; if [[ $rc != 0 ]]; then
    error "error building rpm"
fi

# Make a tar of the rpm and source rpm
RPM_VERSION_RELEASE=$(rpm -qp --qf='%{NAME}-%{VERSION}-%{RELEASE}' ./rpmbuild/RPMS/x86_64/MUSO-[0-9]*.rpm)
tar_file=$RPM_VERSION_RELEASE.tar.gz

cp ./rpmbuild/RPMS/x86_64/* ${PKG_OUTDIR}
cp ./rpmbuild/SRPMS/* ${PKG_OUTDIR}

RPM_MD5SUM=$(rpm -q --queryformat '%{SIGMD5}\n' -p ./rpmbuild/RPMS/x86_64/MUSO-[0-9]*.rpm 2>/dev/null)
DBG_MD5SUM=$(rpm -q --queryformat '%{SIGMD5}\n' -p ./rpmbuild/RPMS/x86_64/MUSO-debuginfo*.rpm 2>/dev/null)
DEV_MD5SUM=$(rpm -q --queryformat '%{SIGMD5}\n' -p ./rpmbuild/RPMS/x86_64/MUSO-devel*.rpm 2>/dev/null)
SRC_MD5SUM=$(rpm -q --queryformat '%{SIGMD5}\n' -p ./rpmbuild/SRPMS/*.rpm 2>/dev/null)

RPM_SHA256="$(sha256sum ./rpmbuild/RPMS/x86_64/MUSO-[0-9]*.rpm | awk '{ print $1}')"
DBG_SHA256="$(sha256sum ./rpmbuild/RPMS/x86_64/MUSO-debuginfo*.rpm | awk '{ print $1}')"
DEV_SHA256="$(sha256sum ./rpmbuild/RPMS/x86_64/MUSO-devel*.rpm | awk '{ print $1}')"
...
echo "rpm_version=$MUSO_RPM_VERSION" >> ${PKG_OUTDIR}/build_vars
