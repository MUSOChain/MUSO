...
We don't recommend macos for MUSO production use at this time. Currently, the
...
modification, build MUSO using clang, and run the system tests you will need
...
should satisfy the the minimum version requirements for MUSO.
...
listed on [GitHub](https://github.com/MUSO/MUSO/releases GitHub).
...
For simple command line building we recommend using the _Unix Makefile_ or
_Ninja_ generator with cmake. All builds should be done in a separate directory
from the source tree root (a subdirectory is fine). For example, from the root
of the MUSO source tree:
