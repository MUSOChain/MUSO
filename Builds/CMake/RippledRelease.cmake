  ...
  if (DOCKER)
    ...
    add_custom_target (rpm_container
      docker build
        ...
        -t MUSO-rpm-builder:${container_label}
      ...
      SOURCES
        ...
        Builds/containers/shared/MUSO.service
        Builds/containers/shared/update_sources.sh
        Builds/containers/shared/update-MUSO.sh
        Builds/containers/packaging/rpm/MUSO.spec
    ...
    add_custom_target (rpm
      docker run
        -e NIH_CACHE_ROOT=/opt/MUSO_bld/pkg/.nih_c
        -v ${NIH_CACHE_ROOT}/pkgbuild:/opt/MUSO_bld/pkg/.nih_c
        -v ${CMAKE_CURRENT_SOURCE_DIR}:/opt/MUSO_bld/pkg/MUSO
        -v ${CMAKE_CURRENT_BINARY_DIR}/packages:/opt/MUSO_bld/pkg/out
        "$<$<BOOL:${map_user}>:--volume=/etc/passwd:/etc/passwd;--volume=/etc/group:/etc/group;--user=${DOCKER_USER_ID}:${DOCKER_GROUP_ID}>"
        -t MUSO-rpm-builder:${container_label}
        /bin/bash -c "cp -fpu MUSO/Builds/containers/packaging/rpm/build_rpm.sh . && ./build_rpm.sh"
      VERBATIM
      USES_TERMINAL
      COMMAND_EXPAND_LISTS
      SOURCES
    ...
    add_custom_target (dpkg_container
      docker build
        ...
        --build-arg GIT_COMMIT=${commit_hash}
        -t MUSO-dpkg-builder:${container_label}
      ...
      SOURCES
        ...
        Builds/containers/packaging/dpkg/debian/MUSO-dev.install
        Builds/containers/packaging/dpkg/debian/MUSO.install
        Builds/containers/packaging/dpkg/debian/MUSO.links
        Builds/containers/packaging/dpkg/debian/MUSO.postinst
        Builds/containers/packaging/dpkg/debian/MUSO.postrm
        Builds/containers/packaging/dpkg/debian/MUSO.preinst
        Builds/containers/packaging/dpkg/debian/MUSO.prerm
    ...
    add_custom_target (dpkg
      docker run
        ...
        "$<$<BOOL:${map_user}>:--volume=/etc/passwd:/etc/passwd;--volume=/etc/group:/etc/group;--user=${DOCKER_USER_ID}:${DOCKER_GROUP_ID}>"
        -t MUSO-dpkg-builder:${container_label}
        /bin/bash -c "cp -fpu MUSO/Builds/containers/packaging/dpkg/build_dpkg.sh . && ./build_dpkg.sh"
      VERBATIM
      USES_TERMINAL
      COMMAND_EXPAND_LISTS
      SOURCES
    ...
    #     - DOCKER_IMAGE="<HUB REPO PATH>/MUSO-ci-builder:YYYY-MM-DD"
    add_custom_target (ci_container
      docker build
        ...
        -t MUSO-ci-builder:${container_label}
