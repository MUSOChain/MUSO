## Heap profiling of MUSO with jemalloc
...
jemalloc is acquired separately from MUSO, and is not affiliated
with MUSO Labs. If you compile and install jemalloc from the
...
profiling has been tested with MUSO on a Linux platform. It should
work on platforms on which both MUSO and jemalloc are available.

To link MUSO with jemalloc, the argument
...
Build MUSO with jemalloc library under /usr/local/lib and
header under /usr/local/include:

    $ scons profile-jemalloc=/usr/local

Build MUSO using clang with the jemalloc library under /opt/local/lib
