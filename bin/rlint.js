...
var Remote = require("MUSO-lib").Remote;
var Transaction = require("MUSO-lib").Transaction;
var UInt160 = require("MUSO-lib").UInt160;
var Amount = require("MUSO-lib").Amount;
...
var ledger_verify = function (ledger) {
  var dir_nodes = ledger.accountState.filter(function (entry) {
    return (
      entry.LedgerEntryType === "DirectoryNode" && // Only directories
      entry.index === entry.RootIndex && // Only root nodes
      "TakerGetsCurrency" in entry
    ); // Only offer directories
  });
  ...
  ledger.accountState.forEach(function (entry) {
    if (entry.LedgerEntryType === "Offer") {
    ...
    } else if (entry.LedgerEntryType === "MUSOState") {
      ...
      if (entry.HighLimit.issuer === entry.LowLimit.issuer)
        MUSO_selfs[entry.Account] = entry;
  ...
  if (low) console.log("counts too low = %s", low);

  if (high) console.log("counts too high = %s", high);

  if (missing_objects) console.log("missing_objects = %s", missing_objects);

  if (missing_accounts) console.log("missing_accounts = %s", missing_accounts);

  if (Object.keys(MUSO_selfs).length)
    console.log("MUSOState selfs = %s", Object.keys(MUSO_selfs).length);
};
