...
var httpd_response = function (res, opts) {
  ...
  res.end(
    "<HTML>" +
      ...
      '<LI><A HREF="/">home</A>' +
      "<LI>" +
      html_link("r4EM4gBQfr1QgQLXSPF4r7h84qE9mb6iCC") +
...
var html_link = function (generic) {
  return (
    '<A HREF="' +
    build_uri({ type: "account", account: generic }) +
    '">' +
    generic +
    "</A>"
  );
      ...
      function (err) {
        if (err) {
          ...
          async.forEach(
            ...
            function (o, callback) {
              opts.remote
                .request_account_info(obj.node.Account)
...
if (process.argv.length < 4 || process.argv.length > 7) {
  ...
  server.listen(port, ip, undefined, function () {
