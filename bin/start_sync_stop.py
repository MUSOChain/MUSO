#!/usr/bin/env python
"""A script to test MUSO in an infinite loop of start-sync-stop.
...
DEFAULT_EXE = 'MUSO'
DEFAULT_CONFIGURATION_FILE = 'MUSO.cfg'
...
async def MUSO(exe=DEFAULT_EXE, config_file=DEFAULT_CONFIGURATION_FILE):
    """A context manager for a MUSO process."""
    # Start the server.
    ...
    logging.info(f'MUSO started with pid {process.pid}')
    try:
        yield process
    finally:
        # Ask it to stop.
        logging.info(f'asking MUSO (pid: {process.pid}) to stop')
        start = time.time()
        process.terminate()
        ...
        try:
            ...
            logging.warning(f'killing MUSO ({process.pid})')
            try:
                process.kill()
        ...
        logging.info(
            f'MUSO stopped after {end - start:.1f} seconds with code {code}'
        )
...
async def sync(
    ...
    """Poll MUSO on an interval until it has been synced for a duration."""
...
async def loop(test,
    ...
    """
    Start-test-stop MUSO in an infinite loop.
    ...
    while True:
        ...
        async with MUSO(exe, config_file) as process:
            start = time.perf_counter()
...
try:
    asyncio.run(loop(test, exe=args.MUSO, config_file=args.conf))
