...
if [[ "${TARGET}" == "validator-keys" ]] ; then
    ...
    export APP_PATH="$PWD/build/${BUILD_DIR}/MUSO"
fi
    ...
    for l in $(diff -w --suppress-common-lines <(echo "$before") <(echo "$after")) ; do
        if [[ "$l" =~ ^[[:space:]]*\>[[:space:]]*(.+)$ ]] ; then
            ...
            gdb \
                ...
                -ex "set logging on" \
                -ex "print 'MUSO::BuildInfo::versionString'" \
