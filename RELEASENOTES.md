...
- Start work on API version 2. Version 2 of the API will be part of a future release. The first breaking change will be to consolidate several closely related error messages that can occur when the server is not synced into a single "notSynced" error message. [[#3269](https://github.com/MUSO/MUSO/pull/3269)]
...
The `MUSO` 1.3.1 release improves the built-in deadlock detection code, improves logging during process startup, changes the package build pipeline and improves the build documentation.
...
- Add dev docs generation to Jenkins ([#2343](https://github.com/MUSO/MUSO/issues/2343))
...
- New API: [gateway_balances](https://MUSO.com/build/MUSO-apis/#gateway-balances) to get a gateway's hot wallet balances and total obligations.
...
- Remove `[websocket_public_port],` `[websocket_public_ip],` `[websocket_ssl_key],` `[websocket_ssl_cert],` `[websocket_ssl_chain]` settings from MUSO.cfg
- Disable `[ssl_verify]` section by setting it to 0
- Migrate the remaining configurations without changes. To enable online delete feature, check Section 6. Database in doc/MUSO-example.cfg
...
MUSO 0.26.4 has been released. The repository tag is _0.26.4_ and can be found on GitHub at: <https://github.com/MUSO/MUSO/commits/0.26.4>
...
This release incorporates a number of important bugfixes and functional improvements. Please refer to the [Git commit history](https://github.com/MUSO/MUSO/commits/0.26.4) for more detailed information.
...
The minimum supported version of Boost is v1.55.0. You **must** upgrade to this release or later to successfully compile this release of MUSO. Please follow [these instructions](https://wiki.MUSO.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.

**Important JSON-RPC Update**

With MUSO version 0.26.4, the [MUSO.cfg](https://github.com/MUSO/MUSO/blob/0.26.4/doc/MUSO-example.cfg) file must set the ssl_verify property to 0. Without this update, JSON-RPC API calls may not work.
...
- Add --quorum argument for server start ([RIPD-563](https://MUSOlabs.atlassian.net/browse/RIPD-563))
- Add account_offers paging ([RIPD-344](https://MUSOlabs.atlassian.net/browse/RIPD-344))
- Add account_lines paging ([RIPD-343](https://MUSOlabs.atlassian.net/browse/RIPD-343))
- Ability to configure network fee in MUSO.cfg file ([RIPD-564](https://MUSOlabs.atlassian.net/browse/RIPD-564))

**Bug Fixes**

- Fix OS X version parsing/error related to OS X 10.10 update
- Fix incorrect address in connectivity check report
- Fix page sizes for ledger_data ([RIPD-249](https://MUSOlabs.atlassian.net/browse/RIPD-249))
- Make log partitions case-insensitive in MUSO.cfg

**Improvements**

- Performance
  - Ledger performance improvements for storage and traversal ([RIPD-434](https://MUSOlabs.atlassian.net/browse/RIPD-434))
  - Improve client performance for JSON responses ([RIPD-439](https://MUSOlabs.atlassian.net/browse/RIPD-439))
- Other
  - Remove PROXY handshake feature
  - Change to MUSO.cfg to support sections containing both key/value pairs and a list of values
  - Return descriptive error message for memo validation ([RIPD-591](https://MUSOlabs.atlassian.net/browse/RIPD-591))
  - Changes to enforce JSON-RPC 2.0 error format
  - Optimize account_lines and account_offers ([RIPD-587](https://MUSOlabs.atlassian.net/browse/RIPD-587))
  - Improve fee setting logic ([RIPD-614](https://MUSOlabs.atlassian.net/browse/RIPD-614))
  - Improve transaction security
  - Config improvements
  - Improve path filtering ([RIPD-561](https://MUSOlabs.atlassian.net/browse/RIPD-561))
  - Logging to distinguish Byzantine failure from tx bug ([RIPD-523](https://MUSOlabs.atlassian.net/browse/RIPD-523))

**Experimental**

- Add "deferred" flag to transaction relay message (required for future code that will relay deferred transactions)
- Refactor STParsedJSON to parse an object or array (required for multisign implementation) ([RIPD-480](https://MUSOlabs.atlassian.net/browse/RIPD-480))
...
MUSO 0.26.3-sp1 has been released. The repository tag is _0.26.3-sp1_ and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.26.3-sp1>
...
This release incorporates a number of important bugfixes and functional improvements. Please refer to the [Git commit history](https://github.com/MUSO/MUSO/commits/0.26.3-sp1) for more detailed information.
...
- New command to display HTTP/S-RPC sessions metrics ([RIPD-533](https://MUSOlabs.atlassian.net/browse/RIPD-533))

**Bug Fixes**

- Improved handling of HTTP/S-RPC sessions ([RIPD-489](https://MUSOlabs.atlassian.net/browse/RIPD-489))
...
- Added a command line utility called LedgerTool for retrieving and processing ledger blocks from the MUSO network.
...
- Convert rvalue to an lvalue. ([RIPD-494](https://MUSOlabs.atlassian.net/browse/RIPD-494))
- Enable heap profiling with jemalloc.
- Add aged containers to Validators module. ([RIPD-349](https://MUSOlabs.atlassian.net/browse/RIPD-349))
- Account for high-ASCII characters. ([RIPD-464](https://MUSOlabs.atlassian.net/browse/RIPD-464))
...
MUSO 0.26.2 has been released. The repository tag is _0.26.2_ and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.26.2>
...
This release incorporates a small number of important bugfixes. Please refer to the [Git commit history](https://github.com/MUSO/MUSO/commits/0.26.2) for more detailed information.
...
- Freeze enforcement: activates on September 15, 2014 ([RIPD-399](https://MUSOlabs.atlassian.net/browse/RIPD-399))
- Add pubkey_node and hostid to server stream messages ([RIPD-407](https://MUSOlabs.atlassian.net/browse/RIPD-407))

**Bug Fixes**

- Fix intermittent exception when closing HTTPS connections ([RIPD-475](https://MUSOlabs.atlassian.net/browse/RIPD-475))
- Correct Pathfinder::getPaths out to handle order books ([RIPD-427](https://MUSOlabs.atlassian.net/browse/RIPD-427))
- Detect inconsistency in PeerFinder self-connects ([RIPD-411](https://MUSOlabs.atlassian.net/browse/RIPD-411))

**Experimental Features**

- Add owner_funds to client subscription data ([RIPD-377](https://MUSOlabs.atlassian.net/browse/RIPD-377))
...
MUSO v0.26.1 has been released. The repository tag is **0.26.1** and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.26.1>
...
This release incorporates a small number of important bugfixes. Please refer to the [Git commit history](https://github.com/MUSO/MUSO/commits/0.26.1) for more detailed information.
...
- Enabled asynchronous handling of HTTP-RPC interactions. This fixes client handlers using RPC that periodically return blank responses to requests. ([RIPD-390](https://MUSOlabs.atlassian.net/browse/RIPD-390))
- Fixed auth handling during OfferCreate. This fixes a regression of [RIPD-256](https://MUSOlabs.atlassian.net/browse/RIPD-256). ([RIPD-414](https://MUSOlabs.atlassian.net/browse/RIPD-414))
...
MUSO v0.26.0 has been released. The repository tag is **0.26.0** and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.26.0>
...
This release incorporates a significant number of improvements and important bugfixes. Please refer to the [Git commit history](https://github.com/MUSO/MUSO/commits/develop) for more detailed information.
...
- Implement setting the no-freeze flag on MUSO accounts ([RIPD-394](https://MUSOlabs.atlassian.net/browse/RIPD-394)).
- Improve transaction fee and execution logic ([RIPD-323](https://MUSOlabs.atlassian.net/browse/RIPD-323)).
- Implemented finding of 'sabfd' paths ([RIPD-335](https://MUSOlabs.atlassian.net/browse/RIPD-335)).
- Imposed a local limit on paths lengths ([RIPD-350](https://MUSOlabs.atlassian.net/browse/RIPD-350)).
- Documented [ledger entries](https://github.com/MUSO/MUSO/blob/develop/src/MUSO/module/app/ledger/README.md) ([RIPD-361](https://MUSOlabs.atlassian.net/browse/RIPD-361)).
- Documented [SHAMap](https://github.com/MUSO/MUSO/blob/develop/src/MUSO/module/app/shamap/README.md).

**Bug Fixes**

- Fixed the limit parameter on book_offers ([RIPD-295](https://MUSOlabs.atlassian.net/browse/RIPD-295)).
- Removed SHAMapNodeID from SHAMapTreeNode to fix "right data, wrong ID" bug in the tree node cache ([RIPD-347](https://MUSOlabs.atlassian.net/browse/RIPD-347)).
- Eliminated spurious SHAMap::getFetchPack failure ([RIPD-379](https://MUSOlabs.atlassian.net/browse/RIPD-379)).
- Disabled SSLv2.
- Implemented rate-limiting of SSL client renegotiation to mitigate [SCIR DoS vulnerability](https://www.thc.org/thc-ssl-dos/) ([RIPD-360](https://MUSOlabs.atlassian.net/browse/RIPD-360)).
- Display unprintable or malformatted currency codes as hex digits.
- Fix static initializers in MUSOSSLContext ([RIPD-375](https://MUSOlabs.atlassian.net/browse/RIPD-375)).
...
MUSO v0.25.2 has been released. The repository tag is **0.25.2** and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.25.2>

Prior to building, please confirm you have the correct source tree with the **git log** command. The first log entry should be the change setting the version:

     commit ddf68d464d74e1c76a0cfd100a08bc8e65b91fec
     Author: Mark Travis <mtravis@MUSO.com>
     Date:   Mon Jul 7 11:46:15 2014 -0700

         Set version to 0.25.2

This release incorporates significant improvements which may not warrant separate entries but are incorporated into the feature changes as summary lines. Please refer to the [Git commit history](https://github.com/MUSO/MUSO/commits/develop) for more information.

**Toolchain support**

The minimum supported version of GCC used to compile MUSO is v4.8. Please follow [these instructions](https://wiki.MUSO.com/Ubuntu_build_instructions#Ubuntu_versions_older_than_13.10_:_Install_gcc_4.8) if you have not upgraded already.

While it may be possible to compile MUSO on (virtual) machines with 4GB of RAM, we recommend build machines with 8GB of RAM.

The minimum supported version of Boost is v1.55. You **must** upgrade to this release or later to successfully compile this release. Please follow [these instructions](https://wiki.MUSO.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.

**Improvements**

- CPU utilization for certain operations has been optimized.
- Improve serialization of public ledger blocks.
- MUSO now takes much less time to compile.
...
`MUSO` v0.25.1 has been released. The repository tag is `0.25.1` and can be found on GitHub at: https://github.com/MUSO/MUSO/tree/0.25.1
...
- Option to compress the NodeStore db. More speed, less space. See [`MUSO-example.cfg`](https://github.com/MUSO/MUSO/blob/0.25.1/doc/MUSO-example.cfg#L691)
...
- Added some new configuration options to doc/MUSO-example.cfg
...
MUSO version 0.25.0 has been released. The repository tag is **0.25.0** and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.25.0>
...
- Option to compress the NodeStore db. More speed, less space. See [`MUSO-example.cfg`](https://github.com/MUSO/MUSO/blob/0.25.0/doc/MUSO-example.cfg#L691)

**Improvements**

- Remove redundant checkAccept call
- Added I/O latency to output of _server_info_.
- Better performance handling of Fetch Packs.
- Improved handling of modified ledger nodes.
- Improved performance of JSON document generator.
- Made strConcat operate in O(n) time for greater efficiency.
...
MUSO version 0.24.0 has been released. The repository tag is **0.24.0** and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.24.0>
...
- The RPC [sign](https://MUSO.com/build/MUSO-apis/#sign) command will now sign with either an account's master or regular secret key.
...
MUSO version 0.23.0 has been released. The repository tag is **0.23.0** and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.23.0>
...
- Allow the word 'none' in the _.cfg_ file to disable storing historical ledgers.
- Clarify the initialization of hash prefixes used in the _RadMap_.
- Better validation of RPC-JSON from all sources
- Reduce spurious log output from Peers
- Eliminated some I/O for certain operations in the _RadMap_.
...
MUSO version 0.22.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/develop> The tag is **0.22.0** and can be found on GitHub at: <https://github.com/MUSO/MUSO/tree/0.22.0>
...
The minimum supported version of libBOOST is v1.55. You **must** upgrade to this release or later to successfully compile this release. Please follow [these instructions](https://wiki.MUSO.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.
...
MUSO version 0.21.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at [1](https://github.com/MUSO/MUSO/tree/develop). The tag is **0.21.0-rc2** and can be found on GitHub at [2](https://github.com/MUSO/MUSO/tree/0.21.0-rc2).
...
As with the previous release, the minimum supported version of GCC used to compile MUSO is v4.8.
...
If you are upgrading from version 0.12 or earlier of MUSO, these next sections apply to you because the format of the _MUSO.cfg_ file changed around that time. If you have upgraded since that time and you have applied the configuration file fixes, you can safely ignore them.

**Validators**

MUSO Labs is now running five validators. You can use this template for your _validators.txt_ file (or place this in your config file):
...
You should also raise your quorum to at least three by putting the following in your _MUSO.cfg_ file:
...
A list of MUSO Labs server IP addresses can be found by resolving **r.MUSO.com**. You can also add this to your _MUSO.cfg_ file to ensure you always have several peer connections to MUSO Labs servers:
...
If you are switching over from an existing back end, you have two options. You can remove your old database and let MUSO recreate it as it re-syncs, or you can import your old database into the new one.

To remove your old database, make sure the server is shut down (\`MUSO stop\`). Remove the _db/ledger.db_ and _db/transaction.db_ files. Remove all the files in your back end store directory (_db/hashnode_ by default). Then change your configuration file to use the RocksDB back end and restart.

To import your old database, start by shutting the server down. Then modify the configuration file by renaming your _\[node_db\]_ stanza to _\[import_db\]_. Create a new _\[node_db\]_ stanza and specify a RocksDB back end with a different directory. Start the server with the command **MUSO --import**. When the import finishes gracefully stop the server (\`MUSO stop\`). Please wait for MUSO to stop on its own because it can take several minutes for it to shut down after an import. Remove the old database, put the new database into place, remove the _\[import_db\]_ section, change the _\[node_db\]_ section to refer to the final location, and restart the server.
...
You need to configure the [NodeBackEnd](https://wiki.MUSO.com/NodeBackEnd) that you want the server to use. See above for an example RocksDB configuration.
...
MUSO version 0.20.1 has been released. This release is currently the tip of the [develop](https://github.com/MUSO/MUSO/tree/develop) branch and the tag is [0.20.1](https://github.com/MUSO/MUSO/tree/0.20.1).
...
- MUSO will crash randomly.
  - Entries in the three parts of the order book are missing or do not match. In such a case, MUSO will crash.
- Server loses sync randomly.
  - This is due to MUSO restarting after it crashes. That the server restarted is not obvious and appears to be something else.
- Server goes 'offline' randomly.
  - This is due to MUSO restarting after it crashes. That the server restarted is not obvious and appears to be something else.
- **complete_ledgers** part of **server_info** output says "None".
  - This is due to MUSO restarting and reconstructing the ledger after it crashes.
  - If the node back end is corrupted or has been moved without being renamed in MUSO.cfg, this can cause MUSO to crash and restart.

**Toolchain support**

Starting with this release, the minimum supported version of GCC used to compile MUSO is v4.8.
...
- Reset liquidity before retrying MUSOCalc.
...
- Added Debian-style initscript (doc/MUSO.init).
- Updated default config file (doc/MUSO-example.cfg) to reflect best practices.
...
- Implemented new fee schedule:
  ...
  - Base reserve: 20 MUSO
  - Incremental reserve: 5 MUSO
...
MUSO version 0.19 has now been released. This release is currently the tip of the [release](https://github.com/MUSO/MUSO/tree/release) branch and the tag is [0.19.0](https://github.com/MUSO/MUSO/tree/0.19.0).
...
MUSO Labs is now running five validators. You can use this template for your `validators.txt` file (or place this in your config file):
...
You should also raise your quorum to at least three by putting the following in your `MUSO.cfg` file:
...
A list of MUSO Labs server IP addresses can be found by resolving `r.MUSO.com`. You can also add this to your `MUSO.cfg` file to ensure you always have several peer connections to MUSO Labs servers:
...
MUSO version 0.16 has now been released. This release is currently the tip of the [master](https://github.com/MUSO/MUSO/tree/master) branch and the tag is [v0.16.0](https://github.com/MUSO/MUSO/tree/v0.16.0).
...
- MUSO no longer throws and exception on exiting
- Better error reporting
- MUSO-lib tests have been ported to use the Mocha testing framework

**Validators**

MUSO Labs is now running five validators. You can use this template for your `validators.txt` file:
...
You need to configure the [NodeBackEnd](https://wiki.MUSO.com/NodeBackEnd) that you want the server to use. In most cases, that will mean adding this to your configuration file:
...
MUSO version 0.14 has now been released. This release is currently the tip of the [master](https://github.com/MUSO/MUSO/tree/master) branch and the tag is [v0.12.0](https://github.com/MUSO/MUSO/tree/v0.14.0).
...
MUSO version 0.12 has now been released. This release is currently the tip of the [master branch](https://github.com/MUSO/MUSO/tree/master) and can be found on GitHub. The tag is [v0.12.0](https://github.com/MUSO/MUSO/tree/v0.12.0).
...
This issue was caused by LevelDB periodically compacting its internal data structure. While compacting, MUSO's processing would stall causing the node to lose sync with the rest of the network. This issue was solved by switching from LevelDB to HyperLevelDB. MUSO operators will need to change their MUSO.cfg file. See below for configuration details.
...
Occasionally, some MUSO servers would fail to fetch all ledgers. This left gaps in the local history and caused some API calls to report incomplete results. The ledger fetch code was rewritten to both prevent this and to repair any existing gaps.
...
**Update April 2014** - Due to a vulnerability in OpenSSL the validator keys above have been cycled out, the five validators by MUSOLabs use the following keys now:
