...
#ifndef MUSO_CORE_CONFIG_H_INCLUDED
#define MUSO_CORE_CONFIG_H_INCLUDED
...
#include <MUSO/protocol/SystemParameters.h>  // VFALCO Breaks levelization
...
public:
    ...
    std::vector<std::string> IPS;           // Peer IPs from MUSO.cfg.
    std::vector<std::string> IPS_FIXED;     // Fixed Peer IPs from MUSO.cfg.
    std::vector<std::string> SNTP_SERVERS;  // SNTP servers from MUSO.cfg.
    ...
    MUSOAmount FEE_DEFAULT{10};
    MUSOAmount FEE_ACCOUNT_RESERVE{200 * DROPS_PER_MUSO};
    MUSOAmount FEE_OWNER_RESERVE{50 * DROPS_PER_MUSO};
