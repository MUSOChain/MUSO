 ...
 *   differently by MUSO. In this case, it is likely that the functions
...
-- account_tx() RPC helper. From the MUSO reporting process, only the
-- parameters without defaults are required. For the parameters with
-- defaults, validation should be done by MUSO, such as:
-- _in_account_id should be a valid MUSO base58 address.
...
--
-- There is no need for MUSO to do any type of lookup on max/min
...
BEGIN
    ...
    IF _in_marker_seq IS NOT NULL OR _in_marker_index IS NOT NULL THEN
        ...
        IF _in_marker_seq IS NULL OR _in_marker_index IS NULL THEN
            -- The MUSO implementation returns no transaction results
...
-- in MUSO's online_delete routine.
...
-- Note that ledgers.closing_time is number of seconds since the MUSO
