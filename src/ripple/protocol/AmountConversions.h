...
#ifndef MUSO_PROTOCOL_AMOUNTCONVERSION_H_INCLUDED
#define MUSO_PROTOCOL_AMOUNTCONVERSION_H_INCLUDED
...
inline STAmount
toSTAmount(MUSOAmount const& MUSO)
{
    bool const isNeg = MUSO.signum() < 0;
    std::uint64_t const umant = isNeg ? -MUSO.drops() : MUSO.drops();
    return STAmount(umant, isNeg);
}

inline STAmount
toSTAmount(MUSOAmount const& MUSO, Issue const& iss)
{
    assert(isMUSO(iss.account) && isMUSO(iss.currency));
    return toSTAmount(MUSO);
}
...
template <>
inline MUSOAmount
toAmount<MUSOAmount>(STAmount const& amt)
{
    assert(amt.mantissa() < std::numeric_limits<std::int64_t>::max());
    bool const isNeg = amt.negative();
    std::int64_t const sMant =
