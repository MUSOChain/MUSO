...
STAmount::STAmount(SerialIter& sit, SField const& name) : STBase(name)
{
    ...
    if (isMUSO(issue.currency))
        Throw<std::runtime_error>("invalid native currency");

    issue.account = sit.get160();

    if (isMUSO(issue.account))
...
STAmount::STAmount(MUSOAmount const& amount)
...
STAmount::MUSO() const
{
    ...
    auto drops = static_cast<MUSOAmount::value_type>(mValue);

    if (mIsNegative)
        drops = -drops;

    return MUSOAmount{drops};
}
...
//
// On the wire:
// - high bit is 0 for MUSO, 1 for issued currency
...
//   (but MUSO only needs 57 bits for the max value of 10^17 drops)
//
...
amountFromString(Issue const& issue, std::string const& amount)
{
    ...
    // Can't specify MUSO using fractional representation
    if (isMUSO(issue) && match[3].matched)
        Throw<std::runtime_error>("MUSO must be specified in integral drops.");
...
amountFromJson(SField const& name, Json::Value const& v)
{
    ...
    {
        Throw<std::runtime_error>(
            "MUSO may not be specified with a null Json value");
    }
    else if (v.isObject())
    {
        value = v[jss::value];
    ...
    {
        if (v.isObjectOrNull())
            Throw<std::runtime_error>("MUSO may not be specified as an object");
        issue = MUSOIssue();
    }
    else
    {
...
multiply(STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isMUSO(issue))
    {
    ...
    bool roundUp)
{
    ...
    bool const MUSO = isMUSO(issue);

    if (v1.native() && v2.native() && MUSO)
    {
        std::uint64_t minV =
            (getSNValue(v1) < getSNValue(v2)) ? getSNValue(v1) : getSNValue(v2);
        std::uint64_t maxV =
    ...
    if (resultNegative != roundUp)
        canonicalizeRound(MUSO, amount, offset);
    ...
    bool roundUp)
{
    ...
    if (resultNegative != roundUp)
        canonicalizeRound(isMUSO(issue), amount, offset);
