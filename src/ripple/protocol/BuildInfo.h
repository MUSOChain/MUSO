...
#ifndef MUSO_PROTOCOL_BUILDINFO_H_INCLUDED
#define MUSO_PROTOCOL_BUILDINFO_H_INCLUDED
...
/** Encode an arbitrary server software version in a 64-bit integer.
    ...
    The MUSO-specific format (implementation ID is: 0x18 0x3B) is:
...
/** Check if the encoded software version is a MUSO software version.

    @param version another node's encoded software version
    @return true if the version is a MUSO software version, false otherwise
*/
bool
isMUSOVersion(std::uint64_t version);

/** Check if the version is newer than the local node's MUSO software
    version.

    @param version another node's encoded software version
    @return true if the version is newer than the local node's MUSO software
        version, false otherwise.

    @note This function only understands version numbers that are generated by
        MUSO. Please see the encodeSoftwareVersion() function for detail.
*/
bool
