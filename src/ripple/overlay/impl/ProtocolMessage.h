...
#ifndef MUSO_OVERLAY_PROTOCOLMESSAGE_H_INCLUDED
#define MUSO_OVERLAY_PROTOCOLMESSAGE_H_INCLUDED
...
parseMessageHeader(
    ...
    std::size_t size)
{
    using namespace MUSO::compression;
...
parseMessageContent(MessageHeader const& header, Buffers const& buffers)
{
    ...
    if (header.algorithm != compression::Algorithm::None)
    {
        ...
        auto const payloadSize = MUSO::compression::decompress(
            stream,
