...
#ifndef MUSO_LEDGER_CASHDIFF_H_INCLUDED
#define MUSO_LEDGER_CASHDIFF_H_INCLUDED
...
public:
    ...
    // Checks for the MUSO round-to-zero case.  Returns zero if not detected.
    // Otherwise returns -1 if seen on lhs, +1 if seen on rhs.
    //
    // For tiny offers of TakerPays IOU and TakerGets MUSO, cases have been
    // observed where MUSO rounding allows a tiny amount of IOU to be
    // removed from an Offer while returning no MUSO to the offer owner.
    // That's because the MUSO amount was rounded down to zero drops.
    //
...
// If both v1 and v2 are MUSO, consider any diff of 2 drops or less to be dust.
bool
