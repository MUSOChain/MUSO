    ...
    beast::Journal j)
{
    ...
    if (isMUSO(currency))
    {
        return {MUSOLiquid(view, account, 0, j)};
    }
    ...
    beast::Journal j)
{
    ...
    auto const balance = view.balanceHook(id, MUSOAccount(), fullBalance);
...
trustCreate(
    ...
    uint256 const& uIndex,      // --> MUSO state entry
    SLE::ref sleAccount,        // --> the account being set.
    const bool bAuth,           // --> authorize account.
    const bool bNoMUSO,       // --> others cannot MUSO through
    ...
    beast::Journal j)
{
    ...
    auto const sleMUSOState = std::make_shared<SLE>(ltMUSO_STATE, uIndex);
    view.insert(sleMUSOState);
    ...
    sleMUSOState->setFieldU64(sfLowNode, *lowNode);
    sleMUSOState->setFieldU64(sfHighNode, *highNode);
    ...
    if (bNoMUSO)
    {
        uFlags |= (bSetHigh ? lsfHighNoMUSO : lsfLowNoMUSO);
    }
    if (bFreeze)
    {
    ...
    if ((slePeer->getFlags() & lsfDefaultMUSO) == 0)
    {
        // The other side's default is no rippling
        uFlags |= (bSetHigh ? lsfLowNoMUSO : lsfHighNoMUSO);
    }

    sleMUSOState->setFieldU32(sfFlags, uFlags);
    adjustOwnerCount(view, sleAccount, 1, j);

    // ONLY: Create MUSO balance.
    sleMUSOState->setFieldAmount(
...
trustDelete(
    ApplyView& view,
    std::shared_ptr<SLE> const& sleMUSOState,
    AccountID const& uLowAccountID,
    AccountID const& uHighAccountID,
    beast::Journal j)
{
    // Detect legacy dirs.
    std::uint64_t uLowNode = sleMUSOState->getFieldU64(sfLowNode);
    std::uint64_t uHighNode = sleMUSOState->getFieldU64(sfHighNode);

    JLOG(j.trace()) << "trustDelete: Deleting MUSO line: low";
    ...
    JLOG(j.trace()) << "trustDelete: Deleting MUSO line: high";
    ...
    JLOG(j.trace()) << "trustDelete: Deleting MUSO line: state";
    view.erase(sleMUSOState);
    ...
    beast::Journal j)
{
    ...
    assert(!isMUSO(uSenderID) && uSenderID != noAccount());
    assert(!isMUSO(uReceiverID) && uReceiverID != noAccount());

    // If the line exists, modify it accordingly.
    if (auto const sleMUSOState = view.peek(index))
    {
        STAmount saBalance = sleMUSOState->getFieldAmount(sfBalance);
        ...
        JLOG(j.trace()) << "MUSOCredit: " << to_string(uSenderID) << " -> "
                        << to_string(uReceiverID)
                        << " : before=" << saBefore.getFullText()
                        << " amount=" << saAmount.getFullText()
                        << " after=" << saBalance.getFullText();

        std::uint32_t const uFlags(sleMUSOState->getFieldU32(sfFlags));
        bool bDelete = false;
        ...
        if (saBefore > beast::zero
            ...
            &&
            static_cast<bool>(
                uFlags & (!bSenderHigh ? lsfLowNoMUSO : lsfHighNoMUSO)) !=
                static_cast<bool>(
                    view.read(keylet::account(uSenderID))->getFlags() &
                    lsfDefaultMUSO) &&
            !(uFlags & (!bSenderHigh ? lsfLowFreeze : lsfHighFreeze)) &&
            !sleMUSOState->getFieldAmount(
                !bSenderHigh ? sfLowLimit : sfHighLimit)
            // Sender trust limit is 0.
            && !sleMUSOState->getFieldU32(
        ...
        sleMUSOState->setFieldAmount(sfBalance, saBalance);
        // ONLY: Adjust MUSO balance.
    ...
    bool const noMUSO = (sleAccount->getFlags() & lsfDefaultMUSO) == 0;
    ...
    beast::Journal j)
{
    ...
    assert(!isMUSO(uSenderID) && !isMUSO(uReceiverID));
    assert(uSenderID != uReceiverID);
    ...
    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        ...
        auto const ter =
            MUSOCredit(view, uSenderID, uReceiverID, saAmount, false, j);
    ...
    JLOG(j.debug()) << "MUSOSend> " << to_string(uSenderID) << " - > "
                    << to_string(uReceiverID)
                    << " : deliver=" << saAmount.getFullText()
                    << " cost=" << saActual.getFullText();

    TER terResult = MUSOCredit(view, issuer, uReceiverID, saAmount, true, j);

    if (tesSUCCESS == terResult)
        terResult = MUSOCredit(view, uSenderID, issuer, saActual, true, j);
    ...
    beast::Journal j)
{
    ...
    if (!saAmount.native())
    {
        ...
        return MUSOSend(view, uSenderID, uReceiverID, saAmount, saActual, j);
    }

    /* MUSO send which does not check reserve and can do pure adjustment.
    ...
    if (sender)
    {
        ...
        else
        {
            ...
            view.creditHook(uSenderID, MUSOAccount(), saAmount, sndBal);

            // Decrement MUSO balance.
    ...
    {
        // Increment MUSO balance.
        auto const rcvBal = receiver->getFieldAmount(sfBalance);
        receiver->setFieldAmount(sfBalance, rcvBal + saAmount);
        view.creditHook(MUSOAccount(), uReceiverID, saAmount, -rcvBal);
    ...
    beast::Journal j)
{
    ...
    if (before > beast::zero
        ...
        // Sender reserve is set.
        && static_cast<bool>(
               flags & (!bSenderHigh ? lsfLowNoMUSO : lsfHighNoMUSO)) !=
            static_cast<bool>(sle->getFlags() & lsfDefaultMUSO) &&
...
issueIOU(
    ...
    beast::Journal j)
{
    assert(!isMUSO(account) && !isMUSO(issue.account));
    ...
    bool noMUSO = (receiverAccount->getFlags() & lsfDefaultMUSO) == 0;
    ...
    beast::Journal j)
{
    ...
    JLOG(j.trace()) << "transferMUSO: " << to_string(from) << " -> "
