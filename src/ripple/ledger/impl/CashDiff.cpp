...
#include <MUSO/ledger/detail/ApplyStateTable.h>
...
struct CashSummary
{
    ...
    std::vector<std::pair<AccountID, MUSOAmount>> MUSOChanges;
    ...
    hasDiff() const
    {
        return !MUSOChanges.empty() || !trustChanges.empty() ||
    ...
    sort()
    {
        std::sort(MUSOChanges.begin(), MUSOChanges.end());
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (isDelete)
    {
        ...
        {
            case ltACCOUNT_ROOT:
                result.MUSOChanges.push_back(
                    std::make_pair(prev[sfAccount], MUSOAmount{0}));
                return true;
    ...
    else
    {
        ...
        {
            case ltACCOUNT_ROOT: {
                auto const curMUSO = cur[sfBalance].MUSO();
                if (!before || (*before)[sfBalance].MUSO() != curMUSO)
                    result.MUSOChanges.push_back(
                        std::make_pair(cur[sfAccount], curMUSO));
                return true;
            }
            case ltMUSO_STATE: {
...
{
private:
    // Note differences in destroyed MUSO between two ApplyStateTables.
    struct DropsGone
    {
        MUSOAmount lhs;
        MUSOAmount rhs;
...
countKeys(detail::CashSummary const& lhs, detail::CashSummary const& rhs)
{
    ...
    addIn(countKeys(lhs.MUSOChanges, rhs.MUSOChanges));
...
int
CashDiff::Impl::MUSORoundToZero() const
{
    // The case has one OfferChange that is present on both lhs_ and rhs_.
    // That OfferChange should have MUSO for TakerGets.  There should be a 1
    ...
    // The side with the smaller MUSO balance in the OfferChange should have
    // two MUSO differences.  The other side should have no MUSO differences.
    if (smaller.MUSOChanges.size() != 2)
        return 0;
    if (!bigger.MUSOChanges.empty())
        return 0;
...
CashDiff::Impl::rmDust()
{
    ...
    // amount differences: MUSOChanges, trustChanges, offerChanges, and
    // offerDeletions.  Rifle through those containers and remove any
    // entries that are _almost_ the same between lhs and rhs.

    // MUSOChanges.  We call a difference of 2 drops or less dust.
    removedDust |= rmVecDust(
        lhsDiffs_.MUSOChanges,
        rhsDiffs_.MUSOChanges,
        [](MUSOAmount const& lhs, MUSOAmount const& rhs) {
    ...
    detail::ApplyStateTable const& rhs)
{
    ...
    // MUSOChanges:
    setDiff(lhsDiffs.MUSOChanges, rhsDiffs.MUSOChanges, lhsDiffs_.MUSOChanges);
    setDiff(rhsDiffs.MUSOChanges, lhsDiffs.MUSOChanges, rhsDiffs_.MUSOChanges);
...
int
CashDiff::MUSORoundToZero() const
{
    return impl_->MUSORoundToZero();
}
...
diffIsDust(STAmount const& v1, STAmount const& v2, std::uint8_t e10)
{
    ...
    // Handling MUSO is different from IOU.
    if (v1.native())
    {
