    ...
    STAmount const& amount) const
{
    ...
    */
    ...
    if (isMUSO(issuer) && adjustedAmt < beast::zero)
        // A calculated negative MUSO balance is not an error case. Consider a
        // payment snippet that credits a large MUSO amount and then debits the
...
PaymentSandbox::balanceChanges(ReadView const& view) const
{
    ...
    // get the change in MUSO balance, Account == root, issuer == root, currency
    // == MUSO
...
MUSOAmount
PaymentSandbox::MUSODestroyed() const
{
