...
#include <MUSO/nodestore/impl/ManagerImp.h>
...
ManagerImp::missing_backend()
{
    Throw<std::runtime_error>(
        "Your MUSO.cfg is missing a [node_db] entry, "
        "please see the MUSO-example.cfg file!");
}
    ...
    beast::Journal journal)
{
    ...
    if (!factory)
    {
#ifndef MUSO_REPORTING
        if (boost::iequals(type, "cassandra"))
            Throw<std::runtime_error>(
                "To use Cassandra as a nodestore, build MUSO with "
                "-Dreporting=ON");
#endif
        missing_backend();
    }
