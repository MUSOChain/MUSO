...
private:
    ...
    jvParseCurrencyIssuer(std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match(strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length())
            {
                // Could confirm issuer is a valid MUSO address.
...
private:
    ...
    parseJson2(Json::Value const& jvParams)
    {
        ...
        if (valid_parse && isValidJson2(jv))
        {
            if (jv.isObject())
            {
                ...
                jv1[jss::MUSOrpc] = jv[jss::MUSOrpc];
            ...
            for (Json::UInt j = 0; j < jv.size(); ++j)
            {
                ...
                jv1[j][jss::MUSOrpc] = jv[j][jss::MUSOrpc];
        ...
        if (jv.isMember(jss::MUSOrpc))
            jv_error[jss::MUSOrpc] = jv[jss::MUSOrpc];
    ...
    Json::Value
    parseMUSOPathFind(Json::Value const& jvParams)
    {
        Json::Reader reader;
        Json::Value jvRequest{Json::objectValue};
...
public:
        ...
        bool allowAnyCommand)
    {
        ...
        static Command const commands[] = {
            ...
            {"MUSO_path_find", &RPCParser::parseMUSOPathFind, 1, 2},
...
cmdLineToJSONRPC(std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::MUSOrpc))
        jv[jss::MUSOrpc] = paramsObj[jss::MUSOrpc];
