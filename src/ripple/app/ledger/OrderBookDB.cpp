...
{
    bool toMUSO = isMUSO(book.out);
    std::lock_guard sl(mLock);

    if (toMUSO)
    {
        // We don't want to search through all the to-MUSO or from-MUSO order
        // books!
        for (auto ob : mSourceMap[book.in])
        {
            if (isMUSO(ob->getCurrencyOut()))  // also to MUSO
                return;
        }
    }
    else
...
bool
OrderBookDB::isBookToMUSO(Issue const& issue)
{
    std::lock_guard sl(mLock);
    return mMUSOBooks.count(issue) > 0;
}
