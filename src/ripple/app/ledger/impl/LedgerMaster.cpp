...
#include <MUSO/basics/MathUtilities.h>
#include <MUSO/basics/TaggedCache.h>
...
LedgerMaster::checkAccept(std::shared_ptr<Ledger const> const& ledger)
{
    ...
    if (ledger->seq() % 256 == 0)
    {
        // Check if the majority of validators run a higher version MUSO
        // software. If so print a warning.
        //
        // Once the HardenedValidations amendment is enabled, validators include
        // their MUSO software version in the validation messages of every
            ...
            if (higherVersionCount > 0 && MUSOCount > 0)
            {
                ...
                needPrint = unlSize > 0 &&
                    calculatePercent(vals.size(), unlSize) >=
                        reportingPercent &&
                    calculatePercent(higherVersionCount, MUSOCount) >=
                        cutoffPercent;
            }
        }
