...
#include <MUSO/beast/clock/basic_seconds_clock.h>
...
run(int argc, char** argv)
{
    ...
    beast::setCurrentThreadName(
        "MUSO: main " + BuildInfo::getVersionString());
    ...
    {
        std::cerr << "MUSO: " << ex.what() << std::endl;
        std::cerr << "Try 'MUSO --help' for a list of options." << std::endl;
        return 1;
    }
    ...
    if (vm.count("version"))
    {
        std::cout << "MUSO version " << BuildInfo::getVersionString()
                  << std::endl;
        return 0;
    }
    ...
    else
    {
        if (vm.count("unittest-jobs"))
        {
            ...
            std::cerr << "MUSO: '--unittest-jobs' specified without "
                         "'--unittest'.\n";
    ...
    beast::setCurrentThreadName("MUSO: rpc");
...
main(int argc, char** argv)
{
    ...
    auto const result(MUSO::run(argc, argv));
