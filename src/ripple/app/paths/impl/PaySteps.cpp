...
bool
checkNear(MUSOAmount const& expected, MUSOAmount const& actual)
{
    return expected == actual;
};

static bool
isMUSOAccount(STPathElement const& pe)
{
    if (pe.getNodeType() != STPathElement::typeAccount)
        return false;
    return isMUSO(pe.getAccountID());
};
    ...
    Issue const& curIssue)
{
    ...
    if (ctx.isFirst && e1->isAccount() &&
        ...
        isMUSO(e1->getCurrency()))
    {
        return make_MUSOEndpointStep(ctx, e1->getAccountID());
    }

    if (ctx.isLast && isMUSOAccount(*e1) && e2->isAccount())
        return make_MUSOEndpointStep(ctx, e2->getAccountID());
    ...
    if (isMUSO(curIssue.currency) && isMUSO(outCurrency))
    {
        JLOG(j.info()) << "Found MUSO/MUSO offer payment step";
        return {temBAD_PATH, std::unique_ptr<Step>{}};
    }

    assert(e2->isOffer());

    if (isMUSO(outCurrency))
        return make_BookStepIX(ctx, curIssue);

    if (isMUSO(curIssue.currency))
...
toStrand(
    ...
    beast::Journal j)
{
    if (isMUSO(src) || isMUSO(dst) || !isConsistent(deliver) ||
    ...
    for (auto const& pe : path)
    {
        ...
        if (hasIssuer && isMUSO(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};

        if (hasAccount && isMUSO(pe.getAccountID()))
            return {temBAD_PATH, Strand{}};

        if (hasCurrency && hasIssuer &&
            isMUSO(pe.getCurrency()) != isMUSO(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};
    ...
    */
        ...
        else if (cur->isOffer() && next->isAccount())
        {
                ...
                !isMUSO(next->getAccountID()))
            {
                if (isMUSO(curIssue))
                {
                    ...
                    {
                        // Last step. insert MUSO endpoint step
                        auto msr =
                            make_MUSOEndpointStep(ctx(), next->getAccountID());
                        if (msr.first != tesSUCCESS)
                            return {msr.first, Strand{}};
                        result.push_back(std::move(msr.second));
                    }
    ...
    auto checkStrand = [&]() -> bool {
        auto stepAccts = [](Step const& s) -> std::pair<AccountID, AccountID> {
            ...
            return std::make_pair(MUSOAccount(), MUSOAccount());
        };
    ...
    beast::Journal j)
{
    ...
    else if (paths.empty())
    {
        JLOG(j.debug()) << "Flow: Invalid transaction: No paths and direct "
                           "MUSO not allowed.";
        return {temMUSO_EMPTY, std::vector<Strand>{}};
    }
...
bool
isDirectMUSOToMUSO(Strand const& strand)
{
    return false;
}
...
template <>
bool
isDirectMUSOToMUSO<MUSOAmount, MUSOAmount>(Strand const& strand)
{
    return (strand.size() == 2);
}

template bool
isDirectMUSOToMUSO<MUSOAmount, IOUAmount>(Strand const& strand);
template bool
isDirectMUSOToMUSO<IOUAmount, MUSOAmount>(Strand const& strand);
template bool
isDirectMUSOToMUSO<IOUAmount, IOUAmount>(Strand const& strand);
