...
#include <MUSO/app/paths/NodeDirectory.h>
#include <MUSO/app/paths/impl/FlatSets.h>
#include <MUSO/app/paths/impl/Steps.h>
#include <MUSO/app/tx/impl/OfferStream.h>
...
public:
        ...
        DebtDirection prevStepDir) const
    {
        ...
        auto rate = [&](AccountID const& id) {
            if (isMUSO(id) || id == this->strandDst_)
                return parityRate;
    ...
    Callback& callback) const
{
    ...
    while (offers.step())
    {
        ...
        // An account can always own MUSO or their own IOUs.
        if (flowCross && (!isMUSO(offer.issueIn().currency)) &&
...
BookStep<TIn, TOut, TDerived>::check(StrandContext const& ctx) const
{
    ...
    auto issuerExists = [](ReadView const& view, Issue const& iss) -> bool {
        return isMUSO(iss.account) || view.read(keylet::account(iss.account));
    };
    ...
    if (ctx.prevStep)
    {
        if (auto const prev = ctx.prevStep->directStepSrcAcct())
        {
            ...
            if (!sle)
                return terNO_LINE;
            if ((*sle)[sfFlags] &
                ((cur > *prev) ? lsfHighNoMUSO : lsfLowNoMUSO))
                return terNO_MUSO;
        }
    }
...
template <class TIn, class TOut, class TDerived>
static bool
equalHelper(Step const& step, MUSO::Book const& book)
{
...
bool
bookStepEqual(Step const& step, MUSO::Book const& book)
{
    bool const inMUSO = isMUSO(book.in.currency);
    bool const outMUSO = isMUSO(book.out.currency);
    if (inMUSO && outMUSO)
        return equalHelper<
            MUSOAmount,
            MUSOAmount,
            BookPaymentStep<MUSOAmount, MUSOAmount>>(step, book);
    if (inMUSO && !outMUSO)
        return equalHelper<
            MUSOAmount,
            IOUAmount,
            BookPaymentStep<MUSOAmount, IOUAmount>>(step, book);
    if (!inMUSO && outMUSO)
        return equalHelper<
            IOUAmount,
            MUSOAmount,
            BookPaymentStep<IOUAmount, MUSOAmount>>(step, book);
    if (!inMUSO && !outMUSO)
        return equalHelper<
            IOUAmount,
            IOUAmount,
...
{
    return make_BookStepHelper<IOUAmount, MUSOAmount>(ctx, in, MUSOIssue());
}

std::pair<TER, std::unique_ptr<Step>>
make_BookStepXI(StrandContext const& ctx, Issue const& out)
{
    return make_BookStepHelper<MUSOAmount, IOUAmount>(ctx, MUSOIssue(), out);
}
