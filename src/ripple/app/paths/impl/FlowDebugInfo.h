...
#ifndef MUSO_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
#define MUSO_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
...
struct FlowDebugInfo
{
    ...
    to_string(bool writePassInfo) const
    {
        ...
        if (writePassInfo)
        {
            ...
            auto writeNestedMUSOAmtList =
                [&ostr, &writeMUSOAmtList](
                    std::vector<std::vector<EitherAmount>> const& amts) {
                    ostr << '[';
                    if (!amts.empty())
                    {
                        writeMUSOAmtList(amts[0], '|');
                        for (size_t i = 1, e = amts.size(); i < e; ++i)
                        {
                            ostr << ';';
                            writeMUSOAmtList(amts[i], '|');
                        }
                    }
                    ostr << ']';
                };
            ...
            if (passInfo.nativeIn)
                writeMUSOAmtList(passInfo.in);
            else
                writeIouAmtList(passInfo.in);
            ostr << ", out_pass: ";
            if (passInfo.nativeOut)
                writeMUSOAmtList(passInfo.out);
            else
                ...
                !passInfo.liquiditySrcIn.back().empty())
            {
                ostr << ", l_src_in: ";
                if (passInfo.nativeIn)
                    writeNestedMUSOAmtList(passInfo.liquiditySrcIn);
                else
                    writeNestedIouAmtList(passInfo.liquiditySrcIn);
                ostr << ", l_src_out: ";
                if (passInfo.nativeOut)
                    writeNestedMUSOAmtList(passInfo.liquiditySrcOut);
                else
...
{
    return {sb.balanceChanges(rv), sb.MUSODestroyed()};
}
...
balanceDiffsToString(boost::optional<BalanceDiffs> const& bd)
{
    ...
    auto const& MUSODestroyed = bd->second;
    std::ostringstream ostr;
    ostr << ", MUSODestroyed: " << to_string(MUSODestroyed);
    ostr << ", balanceDiffs: ";
