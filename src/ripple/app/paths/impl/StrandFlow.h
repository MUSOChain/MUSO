...
#ifndef MUSO_APP_PATHS_IMPL_STRANDFLOW_H_INCLUDED
#define MUSO_APP_PATHS_IMPL_STRANDFLOW_H_INCLUDED
...
/**
   ...
   @param strand Steps of Accounts to MUSO through and offer books to use
    ...
    beast::Journal j)
{
    ...
    if (isDirectMUSOToMUSO<TInAmt, TOutAmt>(strand))
    {
            ...
            for (auto i = s; i--;)
            {
                ...
                else if (!strand[i]->equalOut(r.second, stepOut))
                {
                    ...
                    if (strand[i]->isZero(r.second))
                    {
                        ...
                        // zero. I.e. 10^-80 IOU into an IOU -> MUSO offer.
...

        {
            ...
            for (auto i = limitingStep + 1; i < s; ++i)
            {
                ...
                if (strand[i]->isZero(r.second))
                {
                    ...
                    // I.e. 10^-80 IOU into an IOU -> MUSO offer.
...
/**
   ...
   @param baseView Trust lines and balances
   @param strands Each strand contains the steps of accounts to MUSO through
