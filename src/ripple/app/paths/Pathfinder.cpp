...
The pathfinding request is identified by category, MUSO to MUSO, MUSO to
non-MUSO, non-MUSO to MUSO, same currency non-MUSO to non-MUSO, cross-currency
non-MUSO to non-MUSO.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder(
    std::shared_ptr<MUSOLineCache> const& cache,
    ...
    , mEffectiveDst(
          isMUSO(saDstAmount.getIssuer()) ? uDstAccount
    ...
    , mSrcAmount(srcAmount.value_or(STAmount(
          {uSrcCurrency,
           uSrcIssuer.value_or(
               isMUSO(uSrcCurrency) ? MUSOAccount() : uSrcAccount)},
          1u,
          0,
          true)))
...
{
    assert(!uSrcIssuer || isMUSO(uSrcCurrency) == isMUSO(uSrcIssuer.get()));
}
...
Pathfinder::findPaths(int searchLevel)
{
    ...
    auto currencyIsMUSO = isMUSO(mSrcCurrency);

    bool useIssuerAccount = mSrcIssuer && !currencyIsMUSO && !isMUSO(*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsMUSO ? AccountID() : account;
    ...
    bool bSrcMUSO = isMUSO(mSrcCurrency);
    bool bDstMUSO = isMUSO(mDstAmount.getCurrency());
    ...
    if (!mLedger->exists(keylet::account(mDstAccount)))
    {
        ...
        if (!bDstMUSO)
        {
            JLOG(j_.debug()) << "New account not being funded in MUSO ";
            return false;
        }
    ...
    if (bSrcMUSO && bDstMUSO)
    {
        // MUSO -> MUSO
        JLOG(j_.debug()) << "MUSO to MUSO payment";
        paymentType = pt_MUSO_to_MUSO;
    }
    else if (bSrcMUSO)
    {
        // MUSO -> non-MUSO
        JLOG(j_.debug()) << "MUSO to non-MUSO payment";
        paymentType = pt_MUSO_to_nonMUSO;
    }
    else if (bDstMUSO)
    {
        // non-MUSO -> MUSO
        JLOG(j_.debug()) << "non-MUSO to MUSO payment";
        paymentType = pt_nonMUSO_to_MUSO;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency())
    {
        // non-MUSO -> non-MUSO - Same currency
        JLOG(j_.debug()) << "non-MUSO to non-MUSO - same currency";
        paymentType = pt_nonMUSO_to_same;
    }
    else
    {
        // non-MUSO to non-MUSO - Different currency
        JLOG(j_.debug()) << "non-MUSO to non-MUSO - cross currency";
        paymentType = pt_nonMUSO_to_nonMUSO;
    }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    try
    {
        ...
        auto rc = path::MUSOCalc::MUSOCalculate(
            sandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
        ...
        if (!convert_all_)
        {
            ...
            rcInput.partialPaymentAllowed = true;
            rc = path::MUSOCalc::MUSOCalculate(
                sandbox,
                mSrcAmount,
...
isDefaultPath(STPath const& path)
{
    ...
    // https://forum.MUSO.com/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender =
        isMUSO(mSrcCurrency) || (srcIssuer == mSrcAccount);
...
Pathfinder::issueMatchesOrigin(Issue const& issue)
{
    ...
    bool matchingAccount = isMUSO(issue.currency) ||
    ...
    AccountID const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getMUSOLines(account))
        {
            MUSOState* rspEntry = (MUSOState*)item.get();
            ...
            else if (rspEntry->getNoMUSOPeer())
            {
...
Pathfinder::addPathsForType(PathType const& pathType)
{
    ...
    switch (nodeType)
    {
        ...
        case nt_MUSO_BOOK:
            addLinks(parentPaths, pathsOut, afADD_BOOKS | afOB_MUSO);
            break;
    ...
    Currency const& currency)
{
    ...
    auto const flag(
        (toAccount > fromAccount) ? lsfHighNoMUSO : lsfLowNoMUSO);

    return sleMUSO && (sleMUSO->getFieldU32(sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no MUSO" on the link?
bool
Pathfinder::isNoMUSOOut(STPath const& currentPath)
{
    ...
    // no MUSO on the output. A path with no MUSO on its output can't be
    // followed by a link with no MUSO on its input.
    auto const& fromAccount = (currentPath.size() == 1)
        ? mSrcAccount
        : (currentPath.end() - 2)->getAccountID();
    auto const& toAccount = endElement.getAccountID();
    return isNoMUSO(fromAccount, toAccount, endElement.getCurrency());
}
    ...
    int addFlags)
{
    ...
    bool const bOnMUSO = uEndCurrency.isZero();
    ...
    JLOG(j_.trace()) << "addLink< flags=" << addFlags << " onMUSO=" << bOnMUSO;
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnMUSO)
        {
            ...
            {  // non-default path to MUSO destination
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoMUSOOut(isNoMUSOOut(currentPath));
                bool const bDestOnly(addFlags & afAC_LAST);

                auto& MUSOLines(mRLCache->getMUSOLines(uEndAccount));

                AccountCandidates candidates;
                candidates.reserve(MUSOLines.size());

                for (auto const& item : MUSOLines)
                {
                    auto* rs = dynamic_cast<MUSOState const*>(item.get());
                    if (!rs)
                    {
                        JLOG(j_.error()) << "Couldn't decipher MUSOState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen(acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoMUSOOut && rs->getNoMUSO())
                        {
    ...
    {
        // add order books
        if (addFlags & afOB_MUSO)
        {
            // to MUSO only
            if (!bOnMUSO &&
                app_.getOrderBookDB().isBookToMUSO({uEndCurrency, uEndIssuer}))
            {
                STPathElement pathElement(
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut() == mDstAmount.getCurrency())))
                {
                    ...
                    {  // to MUSO
                        ...
                        {
                            // destination is MUSO, add account and path is
                            // complete
                            JLOG(j_.trace())
...
makePath(char const* string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x':  // MUSO book
                ret.push_back(Pathfinder::nt_MUSO_BOOK);
                break;
...
Pathfinder::initPathTable()
{
    ...
    fillPaths(
        pt_nonMUSO_to_MUSO,
        {{1, "sxd"},   // gateway buys MUSO
         {2, "saxd"},  // source -> gateway -> book(MUSO) -> dest
         {6, "saaxd"},
         {7, "sbxd"},
         {8, "sabxd"},
         {9, "sabaxd"}});

    // non-MUSO to non-MUSO (same currency)
    fillPaths(
        pt_nonMUSO_to_same,
        {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},  // source -> gateway -> book to MUSO -> book ->
                           // destination
            {6, "saxfad"},
