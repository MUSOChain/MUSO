...
/** Get the current MUSOLineCache, updating it if necessary.
    Get the correct ledger to use.
*/
std::shared_ptr<MUSOLineCache>
    ...
    bool authoritative)
{
    ...
    {
        mLineCache = std::make_shared<MUSOLineCache>(ledger);
    }
    return mLineCache;
}
    ...
    Job::CancelCallback shouldCancel)
{
    ...
    std::shared_ptr<MUSOLineCache> cache;
...
// Make an old-style MUSO_path_find request
Json::Value
...
PathRequests::doLegacyPathRequest(
    ...
    Json::Value const& request)
{
    auto cache = std::make_shared<MUSOLineCache>(inLedger);
