...
#ifndef MUSO_APP_PATHS_PATHFINDER_H_INCLUDED
#define MUSO_APP_PATHS_PATHFINDER_H_INCLUDED

#include <MUSO/app/ledger/Ledger.h>
#include <MUSO/app/paths/MUSOLineCache.h>
#include <MUSO/core/LoadEvent.h>
#include <MUSO/protocol/STAmount.h>
#include <MUSO/protocol/STPathSet.h>

namespace MUSO {

/** Calculates payment paths.

    The @ref MUSOCalc determines the quality of the found paths.
...
public:
    ...
    */
    ...
    enum NodeType {
        ...
        nt_MUSO_BOOK,   // The order book from this currency to MUSO.
    ...
    enum PaymentType {
        ...
        pt_MUSO_to_nonMUSO,
        pt_nonMUSO_to_MUSO,
        pt_nonMUSO_to_same,   // Destination currency is the same as source.
        pt_nonMUSO_to_nonMUSO  // Destination currency is NOT the same as source.
    };
...
private:
    ...
    // set the "no MUSO" flag on the link?
    bool
    isNoMUSOOut(STPath const& currentPath);

    // Is the "no MUSO" flag set from one account to another?
    bool
    isNoMUSO(
    ...
    std::shared_ptr<MUSOLineCache> mRLCache;
    ...
    // Add order book to MUSO only
    static std::uint32_t const afOB_MUSO = 0x010;
