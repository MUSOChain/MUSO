...
#include <MUSO/app/paths/PathRequest.h>
...
bool
PathRequest::isValid(std::shared_ptr<MUSOLineCache> const& crCache)
{
    ...
    if (!sleDest)
    {
        ...
        {
            // Only MUSO can be send to a non-existent account.
    ...
    {
        bool const disallowMUSO(sleDest->getFlags() & lsfDisallowMUSO);

        auto usDestCurrID =
            accountDestCurrencies(*raDstAccount, crCache, !disallowMUSO);
    ...
    Json::Value& jvArray)
{
        ...
        for (auto const& c : currencies)
        {
            if (!sameAccount || c != saDstAmount.getCurrency())
            {
                ...
                sourceCurrencies.insert(
                    {c, c.isZero() ? MUSOAccount() : *raSrcAccount});
            }
        }
    }
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        auto& sourceAccount = !isMUSO(issue.account)
            ? issue.account
            : isMUSO(issue.currency) ? MUSOAccount() : *raSrcAccount;
        STAmount saMaxAmount = saSendMax.value_or(
            STAmount({issue.currency, sourceAccount}, 1u, 0, true));

        JLOG(m_journal.debug())
            << iIdentifier << " Paths found, calling MUSOCalc";
        ...
        if (rc.result() == tesSUCCESS)
        {
            ...
            if (hasCompletion())
            {
                // Old MUSO_path_find API requires this
        ...
        {
            JLOG(m_journal.debug()) << iIdentifier << " MUSOCalc returns "
                                    << transHuman(rc.result());
        }
    }
...
Json::Value
PathRequest::doUpdate(std::shared_ptr<MUSOLineCache> const& cache, bool fast)
{
    using namespace std::chrono;
    JLOG(m_journal.debug())
    ...
    if (hasCompletion())
    {
        // Old MUSO_path_find API gives destination_currencies
        auto& destCurrencies =
