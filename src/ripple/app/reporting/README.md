Reporting mode is a special operating mode of MUSO, designed to handle RPCs
...
to the p2p network. To run MUSO in reporting mode, you must also run a
separate MUSO node in p2p mode, to use as an ETL source. Multiple reporting
...
```
$ cqlsh [host] [port]
> CREATE KEYSPACE MUSO WITH REPLICATION =
...
the normal MUSO behavior, where the open ledger is used by default.
