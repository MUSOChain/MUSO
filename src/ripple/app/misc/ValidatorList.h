...
#ifndef MUSO_APP_MISC_VALIDATORLIST_H_INCLUDED
#define MUSO_APP_MISC_VALIDATORLIST_H_INCLUDED
...
/**
    ...
    MUSO accepts ledger proposals and validations from trusted validator
    ...
    This class manages the set of validation public keys the local MUSO node
    ...
    @li @c "blob": Base64-encoded JSON string containing a @c "sequence", @c
        ...
        contains the MUSO timestamp (seconds since January 1st, 2000 (00:00
        UTC)) for when the list becomes valid. @c "validUntil" contains the
        MUSO timestamp for when the list expires. @c "validators" contains
...
hash_append(Hasher& h, TMValidatorListCollection const& msg)
{
    ...
    hash_append(
        h,
        msg.manifest(),
        MUSO::ValidatorList::parseBlobs(msg),
        msg.version());
}
