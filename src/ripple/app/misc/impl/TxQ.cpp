...
/*
    ...
    Copyright (c) 2012, 2013, 2019 MUSO Labs Inc.
...
getFeeLevelPaid(ReadView const& view, STTx const& tx)
{
    auto const [baseFee, effectiveFeePaid] = [&view, &tx]() {
        MUSOAmount baseFee = view.fees().toDrops(calculateBaseFee(view, tx));
        MUSOAmount feePaid = tx[sfFee].MUSO();

        // If baseFee is 0 then the cost of a basic transaction is free.
        MUSOAmount const ref = baseFee.signum() > 0
            ? MUSOAmount{0}
    ...
    beast::Journal j)
{
    ...
    else
    {
        ...
        if (requiresMultiTxn)
        {
            ...
            MUSOAmount potentialSpend = beast::zero;
            MUSOAmount totalFee = beast::zero;
            for (auto iter = txIter->first; iter != txIter->end; ++iter)
            {
                // If we're replacing this transaction don't include
                // the replaced transaction's MUSO spend.  Otherwise add
            ...
            /* Check if the total fees in flight are greater
                ...
                Assume: Minimum account reserve is 20 MUSO.
                Example 1: If I have 1,000,000 MUSO, I can queue
                    a transaction with a 1,000,000 MUSO fee. In
                    the meantime, some other transaction may
                    lower my balance (eg. taking an offer). When
                    the transaction executes, I will either
                    spend the 1,000,000 MUSO, or the transaction
                    will get stuck in the queue with a
                    `terINSUF_FEE_B`.
                Example 2: If I have 1,000,000 MUSO, and I queue
                    10 transactions with 0.1 MUSO fee, I have 1 MUSO
                    in flight. I can now queue another tx with a
                    999,999 MUSO fee. When the first 10 execute,
                    ...
                    999,999 MUSO, or get stuck in the queue.
                Example 3: If I have 1,000,000 MUSO, and I queue
                    7 transactions with 3 MUSO fee, I have 21 MUSO
            ...
            */
            auto const balance = (*sleAccount)[sfBalance].MUSO();
            ...
            */
            ...
            // Subtract the fees and MUSO spend from all of the other
            ...
            assert(potentialTotalSpend > MUSOAmount{0});
    ...
    beast::Journal j)
{
    ...
    if (feeLevelPaid >= requiredFeeLevel)
    {
        ...
        auto const [txnResult, didApply] =
            MUSO::apply(app, view, *tx, flags, j);
