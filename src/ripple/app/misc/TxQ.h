...
/*
    ...
    Copyright (c) 2012-19 MUSO Labs Inc.
...
#ifndef MUSO_TXQ_H_INCLUDED
#define MUSO_TXQ_H_INCLUDED
...
#include <MUSO/protocol/SeqProxy.h>
    ...
    */
    ...
    public:
        ...
        /**
            ...
            @param app MUSO Application object.
            @param view View of the LCL that was just closed or received.
            @param timeLeap Indicates that MUSO is under load so fees
            should grow faster.
...
template <class T>
MUSOAmount
toDrops(FeeLevel<T> const& level, MUSOAmount const& baseFee)
{
    if (auto const drops = mulDiv(level, baseFee, TxQ::baseLevel); drops.first)
        return drops.second;

    return MUSOAmount(STAmount::cMaxNativeN);
}

inline FeeLevel64
toFeeLevel(MUSOAmount const& drops, MUSOAmount const& baseFee)
{
