    ...
    std::shared_ptr<SHAMap> const& initialPosition)
{
    ...
    for (auto const& val : set)
    {
        if (val->isTrusted())
        {
            ...
            {
                using MUSOtype = MUSOAmount::value_type;
                auto const vote = val->getFieldU64(sfBaseFee);
                if (vote <= std::numeric_limits<MUSOtype>::max() &&
                    isLegalAmount(MUSOAmount{unsafe_cast<MUSOtype>(vote)}))
                    baseFeeVote.addVote(
                        MUSOAmount{unsafe_cast<MUSOAmount::value_type>(vote)});
                else
            ...
            if (val->isFieldPresent(sfReserveBase))
            {
                baseReserveVote.addVote(
                    MUSOAmount{val->getFieldU32(sfReserveBase)});
            }
            else
            {
            ...
            if (val->isFieldPresent(sfReserveIncrement))
            {
                incReserveVote.addVote(
                    MUSOAmount{val->getFieldU32(sfReserveIncrement)});
            }
            else
            {
...
setup_FeeVote(Section const& section)
{
    FeeVote::Setup setup;
    {
        ...
        if (set(temp, "reference_fee", section) &&
            temp <= std::numeric_limits<MUSOAmount::value_type>::max())
