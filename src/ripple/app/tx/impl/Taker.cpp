...
#include <MUSO/app/tx/impl/Taker.h>
    ...
    , journal_(journal)
{
    ...
    // flavor we expect:
    assert(
        cross_type != CrossType::MUSOToIou ||
        (isMUSO(issue_in()) && !isMUSO(issue_out())));

    assert(
        cross_type != CrossType::IouToMUSO ||
        (!isMUSO(issue_in()) && isMUSO(issue_out())));

    // And make sure we're not crossing MUSO for MUSO
    assert(!isMUSO(issue_in()) || !isMUSO(issue_out()));
    ...
    Rate const& rate_out)
{
    ...
    // of an autobridge.
    if (cross_type_ == CrossType::MUSOToIou && (remaining_.in < f.order.in))
    {
    ...
    Rate const& rate_in)
{
    ...
    if (!sell_ && cross_type_ == CrossType::IouToMUSO)
    {
...
BasicTaker::do_cross(Amounts offer, Quality quality, AccountID const& owner)
{
    ...
    if (cross_type_ == CrossType::MUSOToIou)
    {
        result = flow_MUSO_to_iou(
            offer,
            quality,
            owner_funds,
            taker_funds,
    ...
    else if (cross_type_ == CrossType::IouToMUSO)
    {
        result = flow_iou_to_MUSO(
            offer,
            quality,
            owner_funds,
            taker_funds,
    ...
    AccountID const& owner2)
{
    ...
    // The amount available to flow via MUSO is the amount that the owner of the
    ...
    // of MUSO that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out MUSO of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // MUSO to the largest of the two offers.
    auto MUSO_funds = get_funds(owner1, offer1.out);

    if (owner1 == owner2)
    {
        JLOG(journal_.trace())
            << "The bridge endpoints are owned by the same account.";
        MUSO_funds = std::max(offer1.out, offer2.in);
    }
    ...
    if (auto stream = journal_.debug())
    {
        ...
        stream << "      MUSO: " << format_amount(MUSO_funds);
    }
    ...
    auto flow1 =
        flow_iou_to_MUSO(offer1, quality1, MUSO_funds, leg1_in_funds, leg1_rate);

    if (!flow1.sanity_check())
        Throw<std::logic_error>("Computed flow1 fails sanity check.");

    auto flow2 =
        flow_MUSO_to_iou(offer2, quality2, leg2_out_funds, MUSO_funds, leg2_rate);
    ...
    // equalize them, so that the amount of MUSO that flows out of the first leg
    // is the same as the amount of MUSO that flows into the second leg. We take
...
{
    if (!isMUSO(amount))
        Throw<std::logic_error>("Using transferMUSO with IOU");
    ...
    return MUSO::transferMUSO(view_, from, to, amount, journal_);
}
...
{
    if (isMUSO(amount))
        Throw<std::logic_error>("Using redeemIOU with MUSO");
    ...
    auto ret = MUSO::redeemIOU(view_, account, amount, issue, journal_);
...
{
    if (isMUSO(amount))
        Throw<std::logic_error>("Using issueIOU with MUSO");
    ...
    return MUSO::issueIOU(view_, account, amount, issue, journal_);
}
...
Taker::fill(BasicTaker::Flow const& flow, Offer& offer)
{
    ...
    if (cross_type() != CrossType::MUSOToIou)
    {
        assert(!isMUSO(flow.order.in));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferMUSO(account(), offer.owner(), flow.order.in);
    }

    // Now send funds from the account whose offer we're taking
    if (cross_type() != CrossType::IouToMUSO)
    {
        assert(!isMUSO(flow.order.out));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferMUSO(offer.owner(), account(), flow.order.out);
    }
    ...
    Offer& leg2)
{
    ...
    // leg1 to leg2: bridging over MUSO
    if (result == tesSUCCESS)
        result = transferMUSO(leg1.owner(), leg2.owner(), flow1.order.out);
...
{
    // In direct crossings, at least one leg must not be MUSO.
    if (isMUSO(offer.amount().in) && isMUSO(offer.amount().out))
        return tefINTERNAL;
...
{
    // In bridged crossings, MUSO must can't be the input to the first leg
    // or the output of the second leg.
    if (isMUSO(leg1.amount().in) || isMUSO(leg2.amount().out))
        return tefINTERNAL;
...
Taker::calculateRate(
    ...
    AccountID const& account)
{
    return isMUSO(issuer) || (account == issuer) ? parityRate
