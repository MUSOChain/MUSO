...
/*
    PaymentChannel
        ...
        Payment channels permit off-ledger checkpoints of MUSO payments flowing
        in a single direction. A channel sequesters the owner's MUSO in its own
    ...
    PaymentChannelCreate
        ...
        Amount
            The amount of MUSO to deposit in the channel immediately.
        SettleDelay
        ...
        DestinationTag (optional)
            ...
            Wallet to be mapped back onto the MUSO ledger. The destination tag
        ...
        SourceTag (optional)
            ...
            to be mapped back onto the MUSO ledger. Source tags are similar to
    ...
    PaymentChannelClaim
        ...
        Channel
            The 256-bit ID of the channel.
        Balance (optional)
            The total amount of MUSO delivered after this claim is processed
   (optional, not needed if just closing). Amount (optional) The amount of MUSO
...
{
    return TxConsequences{ctx.tx, ctx.tx[sfAmount].MUSO()};
}
...
PayChanCreate::preflight(PreflightContext const& ctx)
{
    ...
    if (!isMUSO(ctx.tx[sfAmount]) || (ctx.tx[sfAmount] <= beast::zero))
...
PayChanCreate::preclaim(PreclaimContext const& ctx)
{
...

    {
        ...
        // Obeying the lsfDisallowMUSO flag was a bug.  Piggyback on
        // featureDepositAuth to remove the bug.
        if (!ctx.view.rules().enabled(featureDepositAuth) &&
            ((*sled)[sfFlags] & lsfDisallowMUSO))
            return tecNO_TARGET;
    }
...
PayChanClaim::preflight(PreflightContext const& ctx)
{
    ...
    if (bal && (!isMUSO(*bal) || *bal <= beast::zero))
        return temBAD_AMOUNT;

    auto const amt = ctx.tx[~sfAmount];
    if (amt && (!isMUSO(*amt) || *amt <= beast::zero))
        return temBAD_AMOUNT;
    ...
    if (auto const sig = ctx.tx[~sfSignature])
    {
        ...
        auto const reqBalance = bal->MUSO();
        auto const authAmt = amt ? amt->MUSO() : reqBalance;
...
PayChanClaim::doApply()
{
    ...
    {
        auto const chanBalance = slep->getFieldAmount(sfBalance).MUSO();
        auto const chanFunds = slep->getFieldAmount(sfAmount).MUSO();
        auto const reqBalance = ctx_.tx[sfBalance].MUSO();
        ...
        if (!depositAuth &&
            (txAccount == src && (sled->getFlags() & lsfDisallowMUSO)))
            return tecNO_TARGET;
        ...
        MUSOAmount const reqDelta = reqBalance - chanBalance;
