...
#include <MUSO/protocol/STAccount.h>
...
CashCheck::preclaim(PreclaimContext const& ctx)
{
    ...
    }
    {
        ...
        // less than value the check cannot be cashed.
        {
            ...
            // Note that src will have one reserve's worth of additional MUSO
            // once the check is cashed, since the check's reserve will no
            // longer be required.  So, if we're dealing in MUSO, we add one
            // reserve's worth to the available funds.
            if (value.native())
                availableFunds += MUSOAmount{ctx.view.fees().increment};
...
CashCheck::doApply()
{
    ...
    if (srcId != account_)
    {
        ...
        // Flow() doesn't do MUSO to MUSO transfers.
        if (sendMax.native())
        {
            // Here we need to calculate the amount of MUSO sleSrc can send.
            ...
            // argument.
            STAmount const srcLiquid{MUSOLiquid(psb, srcId, -1, viewJ)};

            // Now, how much do they need in order to be successful?
            STAmount const MUSODeliver{
                optDeliverMin
                    ? std::max(*optDeliverMin, std::min(sendMax, srcLiquid))
                    : ctx_.tx.getFieldAmount(sfAmount)};

            if (srcLiquid < MUSODeliver)
            {
                // Vote no. However the transaction might succeed if applied
                // in a different order.
                JLOG(j_.trace()) << "Cash Check: Insufficient MUSO: "
                                 << srcLiquid.getFullText() << " < "
                                 << MUSODeliver.getFullText();
                return tecUNFUNDED_PAYMENT;
            }

            if (optDeliverMin && doFix1623)
                // Set the DeliveredAmount metadata.
                ctx_.deliver(MUSODeliver);

            // The source account has enough MUSO so make the ledger change.
            if (TER const ter{
                    transferMUSO(psb, srcId, account_, MUSODeliver, viewJ)};
                ter != tesSUCCESS)
            {
