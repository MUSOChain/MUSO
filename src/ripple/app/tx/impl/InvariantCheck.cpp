...
#include <MUSO/app/tx/impl/InvariantCheck.h>
    ...
    beast::Journal const& j)
{
    ...
    if (fee > tx.getFieldAmount(sfFee).MUSO())
    {
    ...
    std::shared_ptr<SLE const> const& after)
{
     ...
     * previous MUSO values and add to the total any new MUSO values. The net
    ...
    if (before)
    {
        ...
        {
            case ltACCOUNT_ROOT:
                drops_ -= (*before)[sfBalance].MUSO().drops();
                break;
            case ltPAYCHAN:
                drops_ -=
                    ((*before)[sfAmount] - (*before)[sfBalance]).MUSO().drops();
                break;
            case ltESCROW:
                drops_ -= (*before)[sfAmount].MUSO().drops();
                break;
            default:
                break;
        }
    ...
    if (after)
    {
        ...
        {
            case ltACCOUNT_ROOT:
                drops_ += (*after)[sfBalance].MUSO().drops();
                break;
            case ltPAYCHAN:
                if (!isDelete)
            ...
            case ltESCROW:
                if (!isDelete)
                    drops_ += (*after)[sfAmount].MUSO().drops();
                break;
            default:
                break;
        }
    ...
    beast::Journal const& j)
{
    ...
    // transaction created MUSO out of thin air. That's not possible.
    if (drops_ > 0)
    {
        JLOG(j.fatal()) << "Invariant failed: MUSO net change was positive: "
                        << drops_;
        return false;
    }
    ...
    if (-drops_ != fee.drops())
    {
        JLOG(j.fatal()) << "Invariant failed: MUSO net change of " << drops_
    ...
    beast::Journal const& j)
{
    if (bad_)
    {
        JLOG(j.fatal()) << "Invariant failed: incorrect account MUSO balance";
        return false;
    }
    ...
    std::shared_ptr<SLE const> const& after)
{
    auto isBad = [](STAmount const& pays, STAmount const& gets) {
        ...
        // Can't have an MUSO to MUSO offer:
    ...
    std::shared_ptr<SLE const> const& after)
{
    auto isBad = [](STAmount const& amount) {
        ...
        if (amount.MUSO() <= MUSOAmount{0})
            return true;

        if (amount.MUSO() >= INITIAL_MUSO)
            return true;
...
void
NoMUSOTrustLines::visitEntry(
    bool,
    std::shared_ptr<SLE const> const&,
    std::shared_ptr<SLE const> const& after)
{
    if (after && after->getType() == ltMUSO_STATE)
    {
        // checking the issue directly here instead of
        // relying on .native() just in case native somehow
        // were systematically incorrect
        MUSOTrustLine_ =
            after->getFieldAmount(sfLowLimit).issue() == MUSOIssue() ||
            after->getFieldAmount(sfHighLimit).issue() == MUSOIssue();
    }
}

bool
NoMUSOTrustLines::finalize(
    STTx const&,
    TER const,
    MUSOAmount const,
    ReadView const&,
    beast::Journal const& j)
{
    ...
    JLOG(j.fatal()) << "Invariant failed: an MUSO trust line was created";
    return false;
}
