...
preflight1(PreflightContext const& ctx)
{
    ...
    if (!fee.native() || fee.negative() || !isLegalAmount(fee.MUSO()))
    {
...
Transactor::checkFee(PreclaimContext const& ctx, FeeUnit64 baseFee)
{
    ...
    auto const feePaid = ctx.tx[sfFee].MUSO();
    ...
    auto const balance = (*sle)[sfBalance].MUSO();
...
Transactor::payFee()
{
    auto const feePaid = ctx_.tx[sfFee].MUSO();
    ...
    // VFALCO Should we call view().rawDestroyMUSO() here as well?
...
Transactor::apply()
{
    ...
    if (sle)
    {
        mPriorBalance = STAmount{(*sle)[sfBalance]}.MUSO();
...
std::pair<TER, MUSOAmount>
Transactor::reset(MUSOAmount fee)
{
    ctx_.discard();
    ...
    auto const balance = txnAcct->getFieldAmount(sfBalance).MUSO();
...
Transactor::operator()()
{
    ...
    auto fee = ctx_.tx.getFieldAmount(sfFee).MUSO();
