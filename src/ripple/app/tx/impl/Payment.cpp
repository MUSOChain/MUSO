...
#include <MUSO/app/tx/impl/Payment.h>
...
// See https://MUSO.com/wiki/Transaction_Format#Payment_.280.29

TxConsequences
Payment::makeTxConsequences(PreflightContext const& ctx)
{
    auto calculateMaxMUSOSpend = [](STTx const& tx) -> MUSOAmount {
        STAmount const maxAmount =
            tx.isFieldPresent(sfSendMax) ? tx[sfSendMax] : tx[sfAmount];

        // If there's no sfSendMax in MUSO, and the sfAmount isn't
        // in MUSO, then the transaction does not spend MUSO.
        return maxAmount.native() ? maxAmount.MUSO() : beast::zero;
    };

    return TxConsequences{ctx.tx, calculateMaxMUSOSpend(ctx.tx)};
}
...
Payment::preflight(PreflightContext const& ctx)
{
    ...
    bool const defaultPathsAllowed = !(uTxFlags & tfNoMUSOirect);
    ...
    // isZero() is MUSO.  FIX!
    bool const bMUSODirect = uSrcCurrency.isZero() && uDstCurrency.isZero();
    ...
    if (bMUSODirect && bMax)
    {
        ...
        JLOG(j.trace()) << "Malformed transaction: "
                        << "SendMax specified for MUSO to MUSO.";
        return temBAD_SEND_MUSO_MAX;
    }
    if (bMUSODirect && bPaths)
    {
        // MUSO is sent without paths.
        JLOG(j.trace()) << "Malformed transaction: "
                        << "Paths specified for MUSO to MUSO.";
        return temBAD_SEND_MUSO_PATHS;
    }
    if (bMUSODirect && partialPaymentAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: "
                        << "Partial payment specified for MUSO to MUSO.";
        return temBAD_SEND_MUSO_PARTIAL;
    }
    if (bMUSODirect && limitQuality)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: "
                        << "Limit quality specified for MUSO to MUSO.";
        return temBAD_SEND_MUSO_LIMIT;
    }
    if (bMUSODirect && !defaultPathsAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: "
                        << "No MUSO direct specified for MUSO to MUSO.";
        return temBAD_SEND_MUSO_NO_DIRECT;
    }
...
{
    // MUSO if source or destination is non-native or if there are paths.
    ...
    {
        // MUSO payment with at least one intermediate step and uses
        // transitive balances.
...
Payment::doApply()
{
    ...
    bool const bMUSO = paths || sendMax || !saDstAmount.native();

    // If the destination has lsfDepositAuth set, then only direct MUSO
    // payments (no intermediate steps) are allowed to the destination.
    if (!depositPreauth && bMUSO && reqDepositAuth)
    ...
    if (bMUSO)
    {
        ...
        path::MUSOCalc::Output rc;
        {
            PaymentSandbox pv(&view());
            JLOG(j_.debug()) << "Entering MUSOCalc in payment: "
        ...
        // Because of its overhead, if MUSOCalc
    ...
    auto const mmm = std::max(reserve, ctx_.tx.getFieldAmount(sfFee).MUSO());

    if (mPriorBalance < saDstAmount.MUSO() + mmm)
    {
        // Vote no. However the transaction might succeed, if applied in
        // a different order.
        JLOG(j_.trace()) << "Delay transaction: Insufficient funds: "
                         << " " << to_string(mPriorBalance) << " / "
                         << to_string(saDstAmount.MUSO() + mmm) << " ("
                         << to_string(reserve) << ")";
    ...
    {
        // If depositPreauth is enabled, then an account that requires
        // authorization has three ways to get an MUSO Payment in:
        //  1. If Account == Destination, or
        //  2. If Account is deposit preauthorized by destination, or
        //  3. If the destination's MUSO balance is
        ...
        // then consumes all of its MUSO.  Without the rule if an
        // account with lsfDepositAuth set spent all of its MUSO, it
        // would be unable to acquire more MUSO required to pay fees.
        //
        ...
        if (uDstAccountID != account_)
        {
            if (!view().exists(keylet::depositPreauth(uDstAccountID, account_)))
            {
                ...
                MUSOAmount const dstReserve{view().fees().accountReserve(0)};
