...
SetAccount::preflight(PreflightContext const& ctx)
{
    ...
    // DisallowMUSO
    //
    bool bSetDisallowMUSO =
        (uTxFlags & tfDisallowMUSO) || (uSetFlag == asfDisallowMUSO);
    bool bClearDisallowMUSO =
        (uTxFlags & tfAllowMUSO) || (uClearFlag == asfDisallowMUSO);

    if (bSetDisallowMUSO && bClearDisallowMUSO)
    {
        JLOG(j.trace()) << "Malformed transaction: Contradictory flags set.";
        return temINVALID_FLAG;
    }
...
SetAccount::doApply()
{
    ...
    bool const bClearRequireAuth{
        (uTxFlags & tfOptionalAuth) || (uClearFlag == asfRequireAuth)};
    bool const bSetDisallowMUSO{
        (uTxFlags & tfDisallowMUSO) || (uSetFlag == asfDisallowMUSO)};
    bool const bClearDisallowMUSO{
        (uTxFlags & tfAllowMUSO) || (uClearFlag == asfDisallowMUSO)};
    ...
    // DisallowMUSO
    //
    if (bSetDisallowMUSO && !(uFlagsIn & lsfDisallowMUSO))
    {
        JLOG(j_.trace()) << "Set lsfDisallowMUSO.";
        uFlagsOut |= lsfDisallowMUSO;
    }

    if (bClearDisallowMUSO && (uFlagsIn & lsfDisallowMUSO))
    {
        JLOG(j_.trace()) << "Clear lsfDisallowMUSO.";
        uFlagsOut &= ~lsfDisallowMUSO;
    }
    ...
    // DefaultMUSO
    //
    if (uSetFlag == asfDefaultMUSO)
    {
        JLOG(j_.trace()) << "Set lsfDefaultMUSO.";
        uFlagsOut |= lsfDefaultMUSO;
    }
    else if (uClearFlag == asfDefaultMUSO)
    {
        JLOG(j_.trace()) << "Clear lsfDefaultMUSO.";
        uFlagsOut &= ~lsfDefaultMUSO;
    }
