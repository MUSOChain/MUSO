...
#include <MUSO/app/tx/impl/CreateOffer.h>
#include <MUSO/beast/utility/WrappedSink.h>
#include <MUSO/ledger/CashDiff.h>
...
CreateOffer::makeTxConsequences(PreflightContext const& ctx)
{
    auto calculateMaxMUSOSpend = [](STTx const& tx) -> MUSOAmount {
        ...
        return amount.native() ? amount.MUSO() : beast::zero;
    };
...
CreateOffer::preflight(PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.debug()) << "Malformed offer: redundant (MUSO for MUSO)";
        return temBAD_OFFER;
    }
    ...
    // We don't allow a non-native currency to use the currency code MUSO.
...
CreateOffer::reachedOfferCrossingLimit(Taker const& taker) const
{
    ...
    // The crossing limit is part of the MUSO protocol and
    ...
    NetClock::time_point const when)
{
    ...
    assert(!isMUSO(takerAmount.in) && !isMUSO(takerAmount.out));

    if (isMUSO(takerAmount.in) || isMUSO(takerAmount.out))
        Throw<std::logic_error>("Bridging with MUSO and an endpoint.");
    ...
    OfferStream offers_leg2(
        view,
        view_cancel,
        Book(MUSOIssue(), taker.issue_out()),
        when,
        stepCounter_,
        j_);
    ...
    Amounts const& takerAmount)
{
    ...
    //
    // We check this in preclaim, but when selling MUSO
    ...
    Amounts const& takerAmount)
{
    try
    {
        ...
        //
        // We check this in preclaim, but when selling MUSO charged fees can
        ...
        // of the takerAmount currencies are MUSO then we cross through an
        // additional path with MUSO as the intermediate between two books.
...
enum class SBoxCmp { same, dustDiff, offerDelDiff, MUSORound, diff };
...
to_string(SBoxCmp c)
{
    switch (c)
    {
        ...
        case SBoxCmp::MUSORound:
            return "MUSO round to zero";
        case SBoxCmp::diff:
            return "different";
    }
    return {};
    ...
    beast::Journal j)
{
    ...
    if (diff.hasDiff())
    {
        ...
        // There is a special case of an offer with MUSO on one side where
        // the MUSO gets rounded to zero.  It mostly looks like dust-level
        // differences.  It is easier to detect if we look for it before
        // removing the dust differences.
        if (int const side = diff.MUSORoundToZero())
        {
            char const* const whichSide = side > 0 ? "; Flow" : "; Taker";
            j.stream(kWarning)
                << "FlowCross: " << name << " different" << whichSide
                << " MUSO rounded to zero.  tx: " << ctx.tx.getTransactionID();
            return SBoxCmp::MUSORound;
        }
...
CreateOffer::preCompute()
{
    ...
    bool const pays_MUSO = ctx_.tx.getFieldAmount(sfTakerPays).native();
    bool const gets_MUSO = ctx_.tx.getFieldAmount(sfTakerGets).native();
    if (pays_MUSO && !gets_MUSO)
        cross_type_ = CrossType::IouToMUSO;
    else if (gets_MUSO && !pays_MUSO)
        cross_type_ = CrossType::MUSOToIou;
...
CreateOffer::applyGuts(Sandbox& sb, Sandbox& sbCancel)
{
    ...
    {
        MUSOAmount reserve = ctx_.view().fees().accountReserve(
