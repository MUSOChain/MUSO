...
#ifndef MUSO_APP_TX_INVARIANTCHECK_H_INCLUDED
#define MUSO_APP_TX_INVARIANTCHECK_H_INCLUDED
...
/**
 * @brief Invariant: A transaction must not create MUSO and should only destroy
 * the MUSO fee.
 *
 * We iterate through all account roots, payment channels and escrow entries
 * that were modified and calculate the net change in MUSO caused by the
 * transactions.
 */
class MUSONotCreated
{
...
/**
 * @brief Invariant: An account MUSO balance must be in MUSO and take a value
 *                   between 0 and INITIAL_MUSO drops, inclusive.
 *
 * We iterate all account roots modified by the transaction and ensure that
 * their MUSO balances are reasonable.
 */
class MUSOBalanceChecks
{
    bool bad_ = false;
 ...
 * no offers which contain negative amounts or which exchange MUSO for MUSO.
 */
class NoBadOffers
{
    bool bad_ = false;
 ...
 * methods
 *
 * @see MUSO::InvariantChecker_PROTOTYPE
 */
