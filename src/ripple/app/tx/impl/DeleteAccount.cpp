...
DeleteAccount::preflight(PreflightContext const& ctx)
{
    ...
    if (ctx.tx[sfAccount] == ctx.tx[sfDestination])
        // An account cannot be deleted and give itself the resulting MUSO.
...
DeleteAccount::doApply()
{
    ...
    if (mSourceBalance > MUSOAmount(0) && dst->isFlag(lsfPasswordSpent))
