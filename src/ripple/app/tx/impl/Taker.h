...
#ifndef MUSO_APP_BOOK_TAKER_H_INCLUDED
#define MUSO_APP_BOOK_TAKER_H_INCLUDED

#include <MUSO/app/tx/impl/Offer.h>
...
enum class CrossType { MUSOToIou, IouToMUSO, IouToIou };
...
protected:
    ...
    struct Flow
    {
        ...
        sanity_check() const
        {
            ...
            if (isMUSO(order.in) && isMUSO(order.out))
                return false;
...
private:
    ...
    // The amount of MUSO that flowed if we were autobridging
    STAmount MUSO_flow_;
