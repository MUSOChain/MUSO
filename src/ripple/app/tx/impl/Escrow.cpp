...
#include <MUSO/app/tx/impl/Escrow.h>
...
#include <MUSO/conditions/Condition.h>
#include <MUSO/conditions/Fulfillment.h>
...
/*
    ...
    Escrow is a feature of the MUSO that allows you to send conditional
    MUSO payments. These conditional payments, called escrows, set aside MUSO and
    ...
    The MUSO set aside in an escrow is locked up. No one can use or destroy the
    MUSO until the escrow has been successfully finished or canceled. Before the
    expiration time, only the intended receiver can get the MUSO. After the
    expiration time, the MUSO can only be returned to the sender.

    For more details on escrow, including examples, diagrams and more please
    visit https://MUSO.com/build/escrow/#escrow
    ...
    `EscrowCreate`
    --------------
        See: https://MUSO.com/build/transactions/#escrowcreate

    `EscrowFinish`
    --------------
        See: https://MUSO.com/build/transactions/#escrowfinish

    `EscrowCancel`
    --------------
        See: https://MUSO.com/build/transactions/#escrowcancel
*/
...
EscrowCreate::preflight(PreflightContext const& ctx)
{
    ...
    {
        using namespace MUSO::cryptoconditions;
...
EscrowCreate::doApply()
{
    ...
    {
        auto const balance = STAmount((*sle)[sfBalance]).MUSO();
        auto const reserve =
        ...
        if (balance < reserve + STAmount(ctx_.tx[sfAmount]).MUSO())
            return tecUNFUNDED;
    }
    ...
    // Check destination account
    {
        ...
        // Obeying the lsfDissalowMUSO flag was a bug.  Piggyback on
