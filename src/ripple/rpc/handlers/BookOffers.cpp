...
doBookOffers(RPC::JsonContext& context)
{
    ...
    {
        pay_issuer = MUSOAccount();
    }

    if (isMUSO(pay_currency) && !isMUSO(pay_issuer))
        return RPC::make_error(
            rpcSRC_ISR_MALFORMED,
            "Unneeded field 'taker_pays.issuer' for "
            "MUSO currency specification.");
    ...
    {
        get_issuer = MUSOAccount();
    }

    if (isMUSO(get_currency) && !isMUSO(get_issuer))
        return RPC::make_error(
            rpcDST_ISR_MALFORMED,
    ...
    if (!isMUSO(get_currency) && isMUSO(get_issuer))
        return RPC::make_error(
            rpcDST_ISR_MALFORMED,
            "Invalid field 'taker_gets.issuer', expected non-MUSO issuer.");
