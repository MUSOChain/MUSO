...
doLedgerEntry(RPC::JsonContext& context)
{
    ...
    else if (context.params.isMember(jss::MUSO_state))
    {
        expectedType = ltMUSO_STATE;
        Currency uCurrency;
        Json::Value jvMUSOState = context.params[jss::MUSO_state];

        if (!jvMUSOState.isObject() ||
            !jvMUSOState.isMember(jss::currency) ||
            !jvMUSOState.isMember(jss::accounts) ||
            !jvMUSOState[jss::accounts].isArray() ||
            2 != jvMUSOState[jss::accounts].size() ||
            !jvMUSOState[jss::accounts][0u].isString() ||
            !jvMUSOState[jss::accounts][1u].isString() ||
            (jvMUSOState[jss::accounts][0u].asString() ==
             jvMUSOState[jss::accounts][1u].asString()))
        {
            jvResult[jss::error] = "malformedRequest";
        }
        else
        {
            ...
            if (!id1 || !id2)
            {
                jvResult[jss::error] = "malformedAddress";
                         ...
                         uCurrency, jvMUSOState[jss::currency].asString()))
            {
...
{
    org::MUSOl::rpc::v1::GetLedgerEntryRequest& request = context.params;
    org::MUSOl::rpc::v1::GetLedgerEntryResponse response;
