...
walletPropose(Json::Value const& params)
{
    ...
    // to detect such keys to avoid user confusion.
    {
        if (params.isMember(jss::passphrase))
            seed = RPC::parseMUSOLibSeed(params[jss::passphrase]);
        else if (params.isMember(jss::seed))
            seed = RPC::parseMUSOLibSeed(params[jss::seed]);
    ...
    if (!MUSOLibSeed && params.isMember(jss::passphrase))
    {
