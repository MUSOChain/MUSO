...
void
addLine(Json::Value& jsonLines, MUSOState const& line)
{
    ...
    if (line.getNoMUSO() || !line.getDefaultMUSO())
        jPeer[jss::no_MUSO] = line.getNoMUSO();
    if (line.getNoMUSOPeer() || !line.getDefaultMUSO())
        jPeer[jss::no_MUSO_peer] = line.getNoMUSOPeer();
    if (line.getFreeze())
...
doAccountLines(RPC::JsonContext& context)
{
    ...
    if (params.isMember(jss::marker))
    {
        ...
        auto const sleLine = ledger->read({ltMUSO_STATE, startAfter});
        ...
        auto const line = MUSOState::makeItem(accountID, sleLine);
        if (line == nullptr)
            return rpcError(rpcINVALID_PARAMS);
        ...
        if (!forEachItemAfter(
                *ledger,
                ...
                [&visitData](std::shared_ptr<SLE const> const& sleCur) {
                    auto const line =
                        MUSOState::makeItem(visitData.accountID, sleCur);
                    if (line != nullptr &&
                        (!visitData.hasPeer ||
    ...
    if (visitData.items.size() == reserve)
    {
        ...
        MUSOState::pointer line(visitData.items.back());
