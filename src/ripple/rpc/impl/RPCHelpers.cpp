...
ledgerFromRequest<>(
    ...
    GRPCContext<org::MUSOl::rpc::v1::GetAccountInfoRequest>&);

// explicit instantiation of above function
template Status
ledgerFromRequest<>(
    std::shared_ptr<ReadView const>&,
    GRPCContext<org::MUSOl::rpc::v1::GetLedgerEntryRequest>&);

// explicit instantiation of above function
template Status
ledgerFromRequest<>(
    std::shared_ptr<ReadView const>&,
    GRPCContext<org::MUSOl::rpc::v1::GetLedgerDataRequest>&);

// explicit instantiation of above function
template Status
ledgerFromRequest<>(
    std::shared_ptr<ReadView const>&,
    GRPCContext<org::MUSOl::rpc::v1::GetLedgerRequest>&);

template <class T>
Status
ledgerFromSpecifier(
    T& ledger,
    org::MUSOl::rpc::v1::LedgerSpecifier const& specifier,
    Context& context)
{
    ledger.reset();

    using LedgerCase = org::MUSOl::rpc::v1::LedgerSpecifier::LedgerCase;
...
boost::optional<Seed>
parseMUSOLibSeed(Json::Value const& value)
{
    // MUSO-lib encodes seed used to generate an Ed25519 wallet in a
    // non-standard way. While MUSO never encode seeds that way, we
...
keypairForSignature(Json::Value const& params, Json::Value& error)
{
    ...
    {
        seed = RPC::parseMUSOLibSeed(params[secretType]);
