...
#include <MUSO/app/tx/apply.h>  // Validity::Valid
...
#include <MUSO/rpc/impl/LegacyPathFind.h>
    ...
    bool doPath)
{
    ...
    if (!tx_json.isMember(jss::Paths) && params.isMember(jss::build_path))
    {
        ...
        if (sendMax.native() && amount.native())
            return RPC::make_error(
                rpcINVALID_PARAMS, "Cannot build MUSO to MUSO paths.");

        {
            ...
            {
                Pathfinder pf(
                    std::make_shared<MUSOLineCache>(ledger),
                    srcAddressID,
                    *dstAccountID,
    ...
    Application& app)
{
    ...
    if (signingArgs.isMultiSigning())
    {
        ...
        auto multisig = MUSO::sign(pk, sk, s.slice());
    ...
    ProcessTransactionFn const& processTransaction)
{
    ...
    // Validate the fields in the serialized transaction.
    {
        ...
        // The Fee field must be in MUSO and greater than zero.
        ...
        if (!isLegalNet(fee))
        {
            ...
            err << "Invalid " << sfFee.fieldName
                << " field.  Fees must be specified in MUSO.";
            return RPC::make_error(rpcINVALID_PARAMS, err.str());
        }
        if (fee <= STAmount{0})
        {
