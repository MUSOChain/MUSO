...
#include <MUSO/basics/make_SSLContext.h>
#include <MUSO/beast/net/IPAddressConversion.h>
...
#include <MUSO/rpc/json_body.h>
#include <MUSO/server/Server.h>
#include <MUSO/server/SimpleWriter.h>
#include <MUSO/server/impl/JSONRPCUtil.h>
    ...
    Json::Value const& jv)
{
    ...
    try
    {
             ...
             jv[jss::command].asString() != jv[jss::method].asString()))
        {
            ...
            if (jv.isMember(jss::MUSOrpc))
                jr[jss::MUSOrpc] = jv[jss::MUSOrpc];
    ...
    boost::string_view user)
{
    ...
    for (unsigned i = 0; i < size; ++i)
    {
        ...
        std::string MUSOrpc = "1.0";
        if (params.isMember(jss::MUSOrpc))
        {
            if (!params[jss::MUSOrpc].isString())
            {
                usage.charge(Resource::feeInvalidRPC);
                if (!batch)
                {
                    HTTPReply(400, "MUSOrpc is not a string", output, rpcJ);
                    return;
                }

                Json::Value r = jsonRPC;
                r[jss::error] = make_json_error(
                    method_not_found, "MUSOrpc is not a string");
                reply.append(r);
                continue;
            }
            MUSOrpc = params[jss::MUSOrpc].asString();
        }
        ...
        if (params.isMember(jss::MUSOrpc))
            r[jss::MUSOrpc] = params[jss::MUSOrpc];
...
ServerHandlerImp::statusResponse(http_request_type const& request) const
{
    ...
    if (app_.serverOkay(reason))
    {
        ...
        msg.body() = "<!DOCTYPE html><html><head><title>" + systemName() +
            " Test page for MUSO</title></head><body><h1>" + systemName() +
            " Test</h1><p>This page shows MUSO http(s) "
