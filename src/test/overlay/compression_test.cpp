...
#include <MUSO/core/TimeKeeper.h>
#include <MUSO/overlay/Compression.h>
#include <MUSO/overlay/Message.h>
#include <MUSO/overlay/impl/Handshake.h>
#include <MUSO/overlay/impl/ProtocolMessage.h>
#include <MUSO/overlay/impl/ZeroCopyStream.h>
...
using namespace MUSO::test;
using namespace MUSO::test::jtx;

static uint256
ledgerHash(LedgerInfo const& info)
{
    return MUSO::sha512Half(
...
public:
        ...
        std::string msg)
    {
        ...
        auto header = MUSO::detail::parseMessageHeader(
        ...
        auto decompressedSize = MUSO::compression::decompress(
            stream,
        ...
        BEAST_EXPECT(std::equal(
            uncompressed.begin() + MUSO::compression::headerBytes,
            uncompressed.end(),
            decompressed.begin()));
    }
    ...
    buildLedgerData(uint32_t n, Logs& logs)
    {
        ...
        uint256 const hash(MUSO::sha512Half(12356789));
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            info.hash = MUSO::sha512Half(i);
            info.txHash = MUSO::sha512Half(i + 1);
            info.accountHash = MUSO::sha512Half(i + 2);
            info.parentHash = parentHash;
            info.drops = MUSOAmount(10);
            info.closeTimeResolution = tk->now().time_since_epoch();
            info.closeTime = tk->now();
            parentHash = ledgerHash(info);
            Serializer nData;
            MUSO::addRaw(info, nData);
    ...
    buildGetObjectByHash()
    {
        ...
        uint256 hash(MUSO::sha512Half(123456789));
        getObject->set_ledgerhash(hash.data(), hash.size());
        getObject->set_fat(true);
        for (int i = 0; i < 100; i++)
        {
            uint256 hash(MUSO::sha512Half(i));
    ...
    testHandshake()
    {
        ...
        auto handshake = [&](int outboundEnable, int inboundEnable) {
            ...
            auto http_resp = MUSO::makeResponse(
                true,
                http_request,
                addr,
                addr,
...
BEAST_DEFINE_TESTSUITE_MANUAL(compression, MUSO_data, MUSO);
