...
public:
    ...
    test_noReserve(FeatureBitset features)
    {
        ...
        auto const smallSignersReserve = reserve1 ? MUSO(250) : MUSO(350);
        ...
        }
        {
            ...
            // possible list.
            auto const addReserveBigSigners = reserve1 ? MUSO(0) : MUSO(350);
    ...
    test_masterSigners(FeatureBitset features)
    {
        ...
        env.fund(MUSO(1000), alice, becky, cheri);
        env.close();
    ...
    test_heterogeneousSigners(FeatureBitset features)
    {
        ...
        env.fund(MUSO(1000), alice, becky, cheri, daria);
        env.close();
    ...
    test_txTypes(FeatureBitset features)
    {
        ...
        env.fund(MUSO(1000), alice, becky, zelda, gw);
        env.close();
        ...
        auto const baseFee = env.current()->fees().base;
        std::uint32_t aliceSeq = env.seq(alice);
        env(pay(alice, env.master, MUSO(1)),
            msig(becky, bogie),
            fee(3 * baseFee));
        env.close();
        BEAST_EXPECT(env.seq(alice) == aliceSeq + 1);
    ...
    test_noMultiSigners(FeatureBitset features)
    {
        ...
        Account const alice{"alice", KeyType::ed25519};
        Account const becky{"becky", KeyType::secp256k1};
        env.fund(MUSO(1000), alice, becky);
        env.close();
    ...
    test_signForHash(FeatureBitset features)
    {
        ...
        // Use sign_for to sign a transaction where alice pays 10 MUSO to
        // masterpassphrase.
        auto const baseFee = env.current()->fees().base;
        Json::Value jvSig1;
...
BEAST_DEFINE_TESTSUITE(MultiSign, app, MUSO);
