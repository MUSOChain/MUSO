...
public:
    ...
    test(std::size_t n)
    {
        ...
        env.fund(MUSO(billion), gw, "alice");
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(PlumpBook, tx, MUSO, 5);
...
BEAST_DEFINE_TESTSUITE(ThinBook, tx, MUSO);
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(OversizeMeta, tx, MUSO, 3);
...
public:
    ...
    oversize(std::size_t n)
    {
        ...
        createOffers(env, USD, n);
        env(pay("alice", gw, USD(billion)));
        env(offer("alice", USD(1), MUSO(1)), ter(std::ignore));
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(FindOversizeCross, tx, MUSO, 50);
