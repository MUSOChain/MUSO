...
#include <MUSO/protocol/SField.h>
...
class AccountTxPaging_test : public beast::unit_test::suite
{
    ...
    testAccountTxPaging()
    {
        ...
        for (auto i = 0; i < 5; ++i)
        {
            ...
            env(offer(A1, MUSO(11), A1["USD"](1)));
            env(offer(A2, MUSO(10), A2["USD"](1)));
            env(offer(A3, MUSO(9), A3["USD"](1)));
            env.close();
        }
    ...
    {
    public:
        org::MUSOl::rpc::v1::GetAccountTransactionHistoryRequest request;
        org::MUSOl::rpc::v1::GetAccountTransactionHistoryResponse reply;
    ...
    bool
    checkTransaction(
        org::MUSOl::rpc::v1::GetTransactionResponse const& tx,
        int sequence,
        int ledger)
    {
        return (
    ...
    nextBinary(
        ...
        int limit = -1,
        bool forward = false,
        org::MUSOl::rpc::v1::Marker* marker = nullptr)
    {
    ...
    struct TxCheck
    {
        ...
        std::string hash;
        std::function<bool(org::MUSOl::rpc::v1::Transaction const& res)>
            checkTxn;
    };
    ...
    testAccountTxContentsGrpc()
    {
        ...
        env.fund(MUSO(1000000), alice, gw);
        env.close();
        ...
        // Payment
        env(pay(alice, gw, MUSO(100)), stag(42), dtag(24), last_ledger_seq(20));
        ...
        env(offer(alice, USD(50), MUSO(150)), sig(alie));
        ...
        // Escrow
        {
            ...
            Json::Value escrowWithFinish{escrow(alice, alice, MUSO(500))};
            ...
            txns.emplace_back(env.tx());
            Json::Value escrowWithCancel{escrow(alice, alice, MUSO(500))};
        ...
        // Check
        {
            ...
            env(check::create(alice, gw, MUSO(300)), sig(alie));

            auto txn = env.tx();
            auto const gwCheckId = keylet::check(gw, env.seq(gw)).key;
            env(check::create(gw, alice, MUSO(200)));
            env.close();
            ...
            env(check::cash(alice, gwCheckId, MUSO(200)), sig(alie));
            ...
            {5,
             ...
             [&txns, this](auto res) {
                 ...
                 return BEAST_EXPECT(res.has_payment()) &&
                     BEAST_EXPECT(
                            res.payment().amount().value().has_MUSO_amount()) &&
                     BEAST_EXPECT(
                            res.payment()
                                .amount()
                                .value()
        ...
        using MetaCheck =
            std::function<bool(org::MUSOl::rpc::v1::Meta const& res)>;
                ...
                return BEAST_EXPECT(meta.transaction_index() == 0) &&
                    ...
                    BEAST_EXPECT(
                           std::count_if(
                               ...
                               [](org::MUSOl::rpc::v1::AffectedNode const&
                                      entry) {
                                   return entry.ledger_entry_type() ==
                                       org::MUSOl::rpc::v1::LedgerEntryType::
        ...
        auto doMetaCheck = [this](auto txn, auto txMetaCheck) {
            return BEAST_EXPECT(txn.has_meta()) &&
                ...
                BEAST_EXPECT(
                       ...
                       org::MUSOl::rpc::v1::TransactionResult::
                           RESULT_TYPE_TES) &&
                BEAST_EXPECT(
...
BEAST_DEFINE_TESTSUITE(AccountTxPaging, app, MUSO);
