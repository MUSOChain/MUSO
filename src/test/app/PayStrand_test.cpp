...
struct MUSOEndpointStepInfo
{
    AccountID acc;
};

enum class TrustFlag { freeze, auth, noMUSO };
    ...
    switch (f)
    {
        ...
        case TrustFlag::auth:
            if (useHigh)
                return lsfHighAuth;
            return lsfLowAuth;
        case TrustFlag::noMUSO:
            if (useHigh)
                return lsfHighNoMUSO;
            return lsfLowNoMUSO;
    }
...
bool
equal(std::unique_ptr<Step> const& s1, MUSOEndpointStepInfo const& MUSOsi)
{
    if (!s1)
        return false;
    return test::MUSOEndpointStepEqual(*s1, MUSOsi.acc);
}

bool
equal(std::unique_ptr<Step> const& s1, MUSO::Book const& bsi)
{
    if (!s1)
        return false;
...
ape(AccountID const& a)
{
    return STPathElement(
        STPathElement::typeAccount, a, MUSOCurrency(), MUSOAccount());
};
...
{
    return STPathElement(
        STPathElement::typeIssuer, MUSOAccount(), MUSOCurrency(), account);
};
...
{
    return STPathElement(
        STPathElement::typeCurrency, MUSOAccount(), c, MUSOAccount());
};
    ...
    valid() const
    {
        return (allowCompound_ ||
            ...
            (!hasAny(
             ...
             has(SB::iss)) &&
            (!hasAny({SB::MUSO, SB::existingCur, SB::prevCur}) ||
             has(SB::cur)) &&
            // These will be duplicates
            (count({SB::MUSO, SB::existingCur, SB::prevCur}) <= 1) &&
...
struct ExistingElementPool
{
    ...
    std::vector<MUSO::Currency> currencies;
        ...
        boost::optional<size_t> const& offererIndex)
    {
        ...
        // create offers to/from MUSO to every other ious
        for (auto const& iou : ious)
        {
            auto const owner =
                offererIndex ? accounts[*offererIndex] : iou.account;
            env(offer(owner, iou(1000), MUSO(1000)), txflags(tfPassive));
            env(offer(owner, MUSO(1000), iou(1000)), txflags(tfPassive));
            env.close();
        }
    }

    std::int64_t
    totalMUSO(ReadView const& v, bool incRoot)
    {
        std::uint64_t totalMUSO = 0;
    ...
    // Check that the balances for all accounts for all currencies & MUSO are the
    // same
    bool
    checkBalances(ReadView const& v1, ReadView const& v2)
    {
        ...
        auto MUSOBalance = [](ReadView const& v, MUSO::Keylet const& k) {
            auto const sle = v.read(k);
            if (!sle)
                return STAmount{};
            return (*sle)[sfBalance];
        };
        auto lineBalance = [](ReadView const& v, MUSO::Keylet const& k) {
            auto const sle = v.read(k);
            if (!sle)
                return STAmount{};
            return (*sle)[sfBalance];
        };
        std::uint64_t totalMUSO[2]{};
        for (auto ai1 = accounts.begin(), aie = accounts.end(); ai1 != aie;
             ++ai1)
        {
            {
                ...
                auto const b1 = MUSOBalance(v1, ak);
                auto const b2 = MUSOBalance(v2, ak);
                totalMUSO[0] += b1.mantissa();
                totalMUSO[1] += b2.mantissa();
                if (b1 != b2)
                    diffs.emplace_back(b1, b2, MUSOAccount(), *ai1);
            }
            for (auto ai2 = accounts.begin(); ai2 != aie; ++ai2)
            {
                if (ai1 >= ai2)
...
struct PayStrand_test : public beast::unit_test::suite
{
    ...
    testToStrand(FeatureBitset features)
    {
        ...
        using B = MUSO::Book;
        using MUSOS = MUSOEndpointStepInfo;
...

        {
            ...
            {
                STPath const path = STPath({ipe(USD), cpe(MUSOCurrency())});
                auto [ter, _] = toStrand(
                    *env.current(),
                    alice,
                    alice,
...

        {
            ...
            // Path with MUSO cross currency bridged payment
            test(
                env,
                EUR,
                USD.issue(),
                STPath({cpe(MUSOCurrency())}),
                tesSUCCESS,
                D{alice, gw, usdC},
                B{USD, MUSO},
                B{MUSO, EUR},
            ...
            // MUSO -> MUSO transaction can't include a path
            test(env, MUSO, boost::none, STPath({ape(carol)}), temBAD_PATH);
            ...
            test(
                ...
                STPath({STPathElement(
                    0, MUSOAccount(), MUSOCurrency(), MUSOAccount())}),
                temBAD_PATH);
        ...
        {
            Env env(*this, features);
            env.fund(MUSO(10000), alice, bob, noMUSO(gw));
            env.trust(USD(1000), alice, bob);
            env(pay(gw, alice, USD(100)));
            test(env, USD, boost::none, STPath(), terNO_MUSO);
        }
...

        {
            ...
            path.emplace_back(boost::none, MUSOCurrency(), boost::none);
    ...
    testRIPD1373(FeatureBitset features)
    {
...

        {
            ...
            env(offer(bob, MUSO(100), bob["USD"](100)), txflags(tfPassive));
            env(offer(gw, MUSO(100), USD(100)), txflags(tfPassive));
...

        {
            ...
            env(offer(bob, MUSO(100), USD(100)), txflags(tfPassive));
            env(offer(bob, USD(100), MUSO(100)), txflags(tfPassive));

            // payment path: MUSO -> MUSO/USD -> USD/MUSO
            env(pay(alice, carol, MUSO(100)),
                path(~USD, ~MUSO),
                txflags(tfNoMUSOirect),
                ter(temBAD_SEND_MUSO_PATHS));
        }
    ...
    testLoop(FeatureBitset features)
    {
...

        {
            ...
            // payment path: USD -> USD/MUSO -> MUSO/USD
            env(pay(alice, carol, USD(100)),
                sendmax(USD(100)),
                path(~MUSO, ~USD),
                txflags(tfNoMUSOirect),
        ...
        }
        {
            ...
            // payment path: MUSO->MUSO/USD->USD/EUR->USD/CNY
    ...
    testNoAccount(FeatureBitset features)
    {
        ...
        STPathSet pathSet;
        ::MUSO::path::MUSOCalc::Input inputs;
        inputs.defaultPathsAllowed = true;
        try
        {
            PaymentSandbox sb{env.current().get(), tapNONE};
            {
                auto const r = ::MUSO::path::MUSOCalc::MUSOCalculate(
                    sb,
                    sendMax,
                    deliver,
                    dstAcc,
...
BEAST_DEFINE_TESTSUITE(PayStrand, app, MUSO);
