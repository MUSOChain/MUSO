...
class Freeze_test : public beast::unit_test::suite
{
    ...
    void
    testMUSOState(FeatureBitset features)
    {
        testcase("MUSOState Freeze");
        ...
        env.fund(MUSO(1000), G1, alice, bob);
        env.close();
        ...
        env(offer(alice, MUSO(500), G1["USD"](100)));
        env.close();
...

        {
            ...
            env(offer(bob, G1["USD"](5), MUSO(25)));
            auto affected = env.meta()->getJson(
                JsonOptions::none)[sfAffectedNodes.fieldName];
...

        {
            ...
            env(offer(bob, MUSO(1), G1["USD"](5)), ter(tecUNFUNDED_OFFER));
    ...
    testGlobalFreeze(FeatureBitset features)
    {
        ...
        env.fund(MUSO(12000), G1);
        env.fund(MUSO(1000), A1);
        env.fund(MUSO(20000), A2, A3, A4);
        env.close();
        ...
        env(offer(G1, MUSO(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(G1, G1["USD"](100), MUSO(10000)), txflags(tfPassive));
        env(offer(A1, MUSO(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(A2, G1["USD"](100), MUSO(10000)), txflags(tfPassive));
        env.close();
...

        {
            ...
            env(offer(A3, G1["BTC"](1), MUSO(1)));

            //    test: assets can be sold on the market
            env(offer(A4, MUSO(1), G1["BTC"](1)));
...

        {
            ...
            env(offer(A3, G1["BTC"](1), MUSO(1)), ter(tecFROZEN));

            //    test: assets can't be sold on the market
            env(offer(A4, MUSO(1), G1["BTC"](1)), ter(tecFROZEN));
        }
    ...
    testOffersWhenFrozen(FeatureBitset features)
    {
        ...
        env.fund(MUSO(1000), G1, A3, A4);
        env.fund(MUSO(2000), A2);
        env.close();
        ...
        env(offer(A3, MUSO(1000), G1["USD"](1000)), txflags(tfPassive));
        env.close();

        // removal after successful payment
        //    test: make a payment with partially consuming offer
        env(pay(A2, G1, G1["USD"](1)), paths(G1["USD"]), sendmax(MUSO(1)));
        env.close();
        ...
        env(offer(A4, MUSO(999), G1["USD"](999)));
        env.close();
        ...
        env(offer(A2, G1["USD"](999), MUSO(999)));
        affected =
            env.meta()->getJson(JsonOptions::none)[sfAffectedNodes.fieldName];
...
BEAST_DEFINE_TESTSUITE(Freeze, app, MUSO);
}  // namespace MUSO
