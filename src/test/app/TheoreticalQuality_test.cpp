...
struct MUSOCalcTestParams
{
    ...
    explicit MUSOCalcTestParams(Json::Value const& jv)
                ...
                for (auto const& pe : path)
                {
                        ...
                        pe.isMember(jss::currency) && pe.isMember(jss::issuer))
                    {
                        ...
                        if (!isMUSO(currency))
                            ...
                            assert(isMUSO(*parseBase58<AccountID>(
                                pe[jss::issuer].asString())));
...
public:
    ...
    testDirectStep(boost::optional<int> const& reqNumIterations)
    {
        ...
        for (int i = 0; i < numTestIterations; ++i)
        {
            ...
            MUSOCalcTestParams rcp{env.json(
    ...
    testBookStep(boost::optional<int> const& reqNumIterations)
    {
        ...
        for (int i = 0; i < numTestIterations; ++i)
        {
            ...
            env.fund(MUSO(10000), alice, bob, carol, dan, oscar);
            env.close();
...
BEAST_DEFINE_TESTSUITE(TheoreticalQuality, app, MUSO);
