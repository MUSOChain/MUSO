...
#include <MUSO/app/tx/apply.h>
#include <MUSO/basics/Log.h>
#include <MUSO/basics/mulDiv.h>
...
class TxQ1_test : public beast::unit_test::suite
{
        ...
        std::uint32_t increment)
    {
        ...
        auto const flagPerLedger =
            1 + MUSO::detail::supportedAmendments().size();
        ...
        BEAST_EXPECT(fees.base == MUSOAmount{base});
        BEAST_EXPECT(fees.units == FeeUnit64{units});
        BEAST_EXPECT(fees.reserve == MUSOAmount{reserve});
        BEAST_EXPECT(fees.increment == MUSOAmount{increment});
...
public:
    ...
    testQueueSeq()
    {
        ...
        env.fund(MUSO(50000), noMUSO(alice, bob, charlie, daria));
        ...
        env.fund(MUSO(50000), noMUSO(elmo, fred, gwen, hank));
        ...
        env(noop(charlie));
        env(noop(daria));
        env(pay(alice, iris, MUSO(1000)), queued);
        ...
        env.require(balance(iris, MUSO(1000)));
    ...
    testQueueTicket()
    {
        ...
        env.fund(MUSO(50000), noMUSO(alice));
        env(noop(alice));
        env(noop(alice));
        env(noop(alice));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
    ...
    testTecResult()
    {
        ...
        // Create accounts
        env.fund(MUSO(50000), noMUSO(alice, gw));
        ...
        env(offer(alice, MUSO(1000), USD(1000)), ter(tecUNFUNDED_OFFER));
        ...
        env(offer(alice, MUSO(1000), USD(1000)), ter(terQUEUED));
    ...
    testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(MUSO(50000), noMUSO(alice, bob, charlie));
    ...
    testLastLedgerSeq()
    {
        ...
        env.fund(MUSO(1000), noMUSO(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(MUSO(1000), noMUSO(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(MUSO(1000), noMUSO(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    testZeroFeeTxn()
    {
        ...
        env.fund(MUSO(1000), noMUSO(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(MUSO(1000), noMUSO(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    testFailInPreclaim()
    {
        ...
        env.fund(MUSO(1000), noMUSO(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, MUSO(-1000)), ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(MUSO(100000)), ter(terINSUF_FEE_B));
    }
        ...
        // Now cheat, and bypass the queue.
        {
            ...
            env.app().openLedger().modify(
                [&](OpenView& view, beast::Journal j) {
                    std::tie(ter, didApply) = MUSO::apply(
    ...
    testMultiTxnPerAccount()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(drops(2000), noMUSO(alice));
        env.fund(MUSO(500000), noMUSO(bob, charlie, daria));
        ...
        aliceFee =
            env.le(alice)->getFieldAmount(sfBalance).MUSO().drops() - (59);
        env(noop(alice),
            seq(aliceSeq),
            fee(aliceFee),
        ...
        // Alice is broke
        env.require(balance(alice, MUSO(0)));
        ...
        std::int64_t bobFee =
            env.le(bob)->getFieldAmount(sfBalance).MUSO().drops() - (9 * 10 - 1);
        env(noop(bob),
            seq(bobSeq + 5),
            fee(bobFee),
            ter(telCAN_NOT_QUEUE_BALANCE));
        ...
        bobFee =
            env.le(bob)->getFieldAmount(sfBalance).MUSO().drops() - (9 * 10);
        env(noop(bob),
            seq(bobSeq + 5),
            fee(bobFee),
            ter(telCAN_NOT_QUEUE_BALANCE));
    ...
    testUnexpectedBalanceChange()
    {
        ...
        env.fund(drops(5000), noMUSO(alice));
        env.fund(MUSO(50000), noMUSO(bob));
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more MUSO, and it'll process.
        */
    ...
    testBlockersSeq()
    {
        ...
        env.fund(MUSO(50000), noMUSO(alice, bob));
        env.memoize(charlie);
    ...
    testInFlightBalance()
    {
        ...
        env.fund(MUSO(50000), noMUSO(alice, charlie), gw);
        ...
        //////////////////////////////////////////
        // Offer with high MUSO out and low fee doesn't block
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, MUSO(50000)), owners(alice, 0));

        // If this offer crosses, all of alice's
        // MUSO will be taken (except the reserve).
        env(offer(alice, BUX(5000), MUSO(50000)), queued);
        ...
        // has plenty of MUSO, because the offer didn't
        // cross (of course).
        ...
        //////////////////////////////////////////
        // Offer with high MUSO out and high total fee blocks later txs
        fillQueue(env, alice);
        ...
        env(offer(alice, BUX(5000), MUSO(50000)), fee(drops(100)), queued);
        ...
        //////////////////////////////////////////
        // Offer with high MUSO out and super high fee blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        aliceSeq = env.seq(alice);
        aliceBal = env.balance(alice);
        ...
        env(offer(alice, BUX(5000), MUSO(50000)), fee(drops(300)), queued);
        checkMetrics(env, 1, limit * 2, limit + 1, limit, 256);
        ...
        //////////////////////////////////////////
        // Offer with low MUSO out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        aliceSeq = env.seq(alice);
        aliceBal = env.balance(alice);
        ...
        // of alice's MUSO will be taken.
        env(offer(alice, BUX(50), MUSO(500)), queued);
        ...
        //////////////////////////////////////////
        // Large MUSO payment doesn't block later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        ...
        // (minus the reserve).
        env(pay(alice, charlie, MUSO(50000)), queued);
        ...
        //////////////////////////////////////////
        // Small MUSO payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        ...
        env(pay(alice, charlie, MUSO(500)), queued);
        ...
        env.require(
            balance(alice, aliceBal - MUSO(500) - drops(20)), owners(alice, 0));
        ...
        // alice's MUSO balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        checkMetrics(env, 2, limit * 2, limit + 1, limit, 256);
        ...
        // MUSO balance, but her USD balance went to charlie.
        env.require(
        ...
        //////////////////////////////////////////
        // Large MUSO to IOU payment doesn't block later txs.

        env(offer(gw, MUSO(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire MUSO balance to charlie in the
        // form of USD.
        BEAST_EXPECT(MUSO(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)), sendmax(MUSO(60000)), queued);
        ...
        // to charlie using only a portion of her MUSO balance
        env.require(
            balance(alice, aliceBal - MUSO(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small MUSO to IOU payment allows later txs.
        ...
        // a portion of her MUSO balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > MUSO(6001));
        env(pay(alice, charlie, USD(500)), sendmax(MUSO(6000)), queued);
        ...
        // to charlie using only a portion of her MUSO balance
        env.require(
            balance(alice, aliceBal - MUSO(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    testConsequences()
    {
        ...
        env.memoize("carol");
        {
            ...
            BEAST_EXPECT(pf.consequences.potentialSpend() == MUSO(0));
        }
    ...
    testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(MUSO(500000), noMUSO(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    testSignAndSubmitSequence()
    {
        ...
        auto const alice = Account("alice");
        auto const bob = Account("bob");
        env.fund(MUSO(100000), alice, bob);
    ...
    testServerSubscribe()
    {
        ...
        env.fund(MUSO(50000), noMUSO(a, b, c, d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(MUSO(50000), noMUSO(e, f, g, h, i));
    ...
    testClearQueuedAccountTxs()
    {
        ...
        checkMetrics(env, 0, boost::none, 0, 3, 256);
        env.fund(MUSO(50000000), alice, bob);
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(MUSO(1)), seq(aliceSeq++), ter(terQUEUED));
    ...
    testInLedgerSeq()
    {
        ...
        env.app().openLedger().modify([&](OpenView& view, beast::Journal j) {
            ...
            auto const result =
                MUSO::apply(env.app(), view, *tx.stx, tapUNLIMITED, j);
    ...
    testInLedgerTicket()
    {
        ...
        env(noop(alice), ticket::use(tktSeq0 + 3), fee(MUSO(1)));
    ...
    testReexecutePreflight()
    {
        ...
        env.close();
        env.fund(MUSO(10000), bob);
        env.close();
        env.fund(MUSO(10000), carol);
        env.close();
        env.fund(MUSO(10000), daria);
        env.close();
        env.fund(MUSO(10000), ellie);
        env.close();
        env.fund(MUSO(10000), fiona);
        env.close();
        ...
        auto expectedPerLedger =
            MUSO::detail::supportedAmendments().size() + 1;
    ...
    testQueueFullDropPenalty()
    {
        ...
        // The noMUSO is to reduce the number of transactions required to
        // fund the accounts.  There is no rippling in this test.
        env.fund(MUSO(10000), noMUSO(alice, bob, carol, daria, ellie, fiona));
        env.close();
    ...
    testCancelQueuedOffers()
    {
        ...
        // The noMUSO is to reduce the number of transactions required to
        // fund the accounts.  There is no rippling in this test.
        env.fund(MUSO(100000), noMUSO(alice));
        env.close();

        {
            ...
            auto const aliceSeq = env.seq(alice);
            env(offer(alice, USD(1000), MUSO(1000)), ter(terQUEUED));

            env(offer(alice, USD(1000), MUSO(1001)),
                seq(aliceSeq + 1),
                ter(terQUEUED));

            // Alice creates transactions that cancel the first set of
            // offers, one through another offer, and one cancel
            env(offer(alice, USD(1000), MUSO(1002)),
                seq(aliceSeq + 2),
...
BEAST_DEFINE_TESTSUITE_PRIO(TxQ1, app, MUSO, 1);
BEAST_DEFINE_TESTSUITE_PRIO(TxQ2, app, MUSO, 1);
