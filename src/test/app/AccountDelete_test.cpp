...
public:
    ...
    testBasics()
    {
        ...
        env.fund(MUSO(10000), alice, becky, carol, gw);
        env.close();

        // Alice can't delete her account and then give herself the MUSO.
        ...
        env(offer(carol, gw["USD"](51), MUSO(51)));
...

        {
            ...
            // Verify that alice's MUSO, minus the fee, was transferred to becky.
            BEAST_EXPECT(
                env.balance(becky) ==
...

        {
            ...
            // Verify that Carol's MUSO, minus the fee, was transferred to becky.
            BEAST_EXPECT(
                env.balance(becky) ==
    ...
    testDirectories()
    {
        ...
        for (int i{0}; i < 45; ++i)
        {
            env(offer(alice, gw["USD"](1), MUSO(1)));
            env.close();
        }
    ...
    testOwnedTypes()
    {
        ...
        env.fund(MUSO(100000), alice, becky, gw);
        env.close();
        ...
        {
            env(offer(alice, gw["USD"](1), MUSO(1)));
            env(offer(becky, gw["USD"](1), MUSO(1)));
            env.close();
        }
        ...
        env(check::create(alice, becky, MUSO(1)));
        env.close();
        ...
        env(escrowCreate(alice, becky, MUSO(333), env.now() + 2s));
        env.close();
        ...
        env(payChanCreate(
            alice, becky, MUSO(57), 4s, env.now() + 2s, alice.pk()));
        env.close();
        ...
        env(payChanCreate(gw, alice, MUSO(68), 4s, env.now() + 2s, alice.pk()));
        env.close();
    ...
    testResurrection()
    {
        ...
        auto const payChanMUSO = MUSO(37);

        env(payChanCreate(
            alice, becky, payChanMUSO, 4s, env.now() + 1h, alice.pk()));
        env.close();
        ...
        // All it takes is a large enough MUSO payment to resurrect
        // becky's account.  Try too small a payment.
        env(pay(alice, becky, MUSO(19)), ter(tecNO_DST_INSUF_MUSO));
        env.close();

        // Actually resurrect becky's account.
        env(pay(alice, becky, MUSO(20)));
        env.close();

        // becky's account root should be back.
        BEAST_EXPECT(env.closed()->exists(beckyAcctKey));
        BEAST_EXPECT(env.balance(becky) == MUSO(20));
        ...
        auto payChanClaim = [&]() {
            ...
            jv[sfBalance.jsonName] =
                payChanMUSO.value().getJson(JsonOptions::none);
            return jv;
        };
        env(payChanClaim());
        env.close();

        BEAST_EXPECT(env.balance(becky) == MUSO(20) + payChanMUSO);
    }
    ...
    testTooManyOffers()
    {
        ...
        env.fund(MUSO(10000000), alice, gw);
        env.close();
        ...
        {
            env(offer(alice, gw[currency](1), MUSO(1)));
            env.close();
    ...
    testImplicitlyCreatedTrustline()
    {
        ...
        // a trust line.
        env(offer(alice, BUX(30), MUSO(30)));
        env.close();

        // gw crosses alice's offer.  alice should end up with BUX(30).
        env(offer(gw, MUSO(30), BUX(30)));
        env.close();
    ...
    testBalanceTooSmallForFee()
    {
        ...
        // reserve of 200 MUSO.
        env.fund(env.current()->fees().accountReserve(0), noMUSO(alice));
        env.close();

        // Burn a chunk of alice's funds so she only has 1 MUSO remaining in
        // her account.
        env(noop(alice), fee(env.balance(alice) - MUSO(1)));
        env.close();
        ...
        // her current balance in MUSO.  Again the transaction fails.
        BEAST_EXPECT(env.balance(alice) == MUSO(1));
        env(acctdelete(alice, env.master), fee(MUSO(1)), ter(telINSUF_FEE_P));
        env.close();
        {
            std::shared_ptr<ReadView const> closed{env.closed()};
            BEAST_EXPECT(closed->exists(keylet::account(alice.id())));
...
BEAST_DEFINE_TESTSUITE(AccountDelete, app, MUSO);
