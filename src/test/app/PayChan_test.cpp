...
#include <MUSO/ledger/Directory.h>
#include <MUSO/protocol/Feature.h>
#include <MUSO/protocol/Indexes.h>
#include <MUSO/protocol/PayChan.h>
...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization(msg, channel, authAmt.MUSO());
    ...
    testSimple()
    {
        ...
        env(create(alice, bob, MUSO(1000), settleDelay, pk));
        BEAST_EXPECT(channelBalance(*env.current(), chan) == MUSO(0));
        BEAST_EXPECT(channelAmount(*env.current(), chan) == MUSO(1000));

        {
            auto const preAlice = env.balance(alice);
            env(fund(alice, chan, MUSO(1000)));
            auto const feeDrops = env.current()->fees().base;
            BEAST_EXPECT(env.balance(alice) == preAlice - MUSO(1000) - feeDrops);
        }

        auto chanBal = channelBalance(*env.current(), chan);
        auto chanAmt = channelAmount(*env.current(), chan);
        BEAST_EXPECT(chanBal == MUSO(0));
        BEAST_EXPECT(chanAmt == MUSO(2000));

        {
            // bad amounts (non-MUSO, negative amounts)
            env(create(alice, bob, USDA(1000), settleDelay, pk),
                ter(temBAD_AMOUNT));
            env(fund(alice, chan, USDA(1000)), ter(temBAD_AMOUNT));
            env(create(alice, bob, MUSO(-1000), settleDelay, pk),
                ter(temBAD_AMOUNT));
            env(fund(alice, chan, MUSO(-1000)), ter(temBAD_AMOUNT));
        }

        // invalid account
        env(create(alice, "noAccount", MUSO(1000), settleDelay, pk),
            ter(tecNO_DST));
        // can't create channel to the same account
        env(create(alice, alice, MUSO(1000), settleDelay, pk),
            ter(temDST_IS_SRC));
        // invalid channel
        ...
        env(fund(
            ...
            ter(tecNO_ENTRY));
        // not enough funds
        env(create(alice, bob, MUSO(10000), settleDelay, pk), ter(tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-MUSO)
            auto const iou = USDA(100).value();
            auto const negMUSO = MUSO(-100).value();
            auto const posMUSO = MUSO(100).value();
            env(claim(alice, chan, iou, iou), ter(temBAD_AMOUNT));
            env(claim(alice, chan, posMUSO, iou), ter(temBAD_AMOUNT));
            env(claim(alice, chan, iou, posMUSO), ter(temBAD_AMOUNT));
            env(claim(alice, chan, negMUSO, negMUSO), ter(temBAD_AMOUNT));
            env(claim(alice, chan, posMUSO, negMUSO), ter(temBAD_AMOUNT));
            env(claim(alice, chan, negMUSO, posMUSO), ter(temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = MUSO(500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + MUSO(-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance(bob);
            STAmount const authAmt = chanBal + MUSO(500);
        ...
        env(fund(bob, chan, MUSO(1000)), ter(tecNO_PERMISSION));
        BEAST_EXPECT(channelBalance(*env.current(), chan) == chanBal);
        BEAST_EXPECT(channelAmount(*env.current(), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig = signClaimAuth(bob.pk(), bob.sk(), chan, MUSO(1500));
            env(claim(
                    bob,
                    chan,
                    MUSO(1500).value(),
    ...
    testCancelAfter()
    {
        ...
        auto const carol = Account("carol");
        {
            ...
            auto const channelFunds = MUSO(1000);
            auto const chan = channel(alice, bob, env.seq(alice));
    ...
    testExpiration()
    {
        ...
        env(fund(
            alice, chan, MUSO(1), NetClock::time_point{minExpiration + 100s}));
        BEAST_EXPECT(
            *channelExpiration(*env.current(), chan) ==
            counts(minExpiration) + 100);
        // decrease the expiration, but still above minExpiration
        env(fund(
            alice, chan, MUSO(1), NetClock::time_point{minExpiration + 50s}));
        BEAST_EXPECT(
            *channelExpiration(*env.current(), chan) ==
        ...
        env(fund(
            ...
            ter(temBAD_EXPIRATION));
        BEAST_EXPECT(!channelExpiration(*env.current(), chan));
        env(fund(alice, chan, MUSO(1), NetClock::time_point{minExpiration}));
        env.close(minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env(fund(
            alice, chan, MUSO(1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT(!channelExists(*env.current(), chan));
    }
    ...
    testDisallowMUSO()
    {
        ...
        auto const bob = Account("bob");
        {
            // Create a channel where dst disallows MUSO
            Env env(*this, supported_amendments() - featureDepositAuth);
            env.fund(MUSO(10000), alice, bob);
            env(fset(bob, asfDisallowMUSO));
            auto const chan = channel(alice, bob, env.seq(alice));
            env(create(alice, bob, MUSO(1000), 3600s, alice.pk()),
                ter(tecNO_TARGET));
            BEAST_EXPECT(!channelExists(*env.current(), chan));
        }
        {
            // Create a channel where dst disallows MUSO.  Ignore that flag,
        ...
        {
            // Claim to a channel where dst disallows MUSO
            // (channel is created before disallow MUSO is set)
            Env env(*this, supported_amendments() - featureDepositAuth);
            env.fund(MUSO(10000), alice, bob);
            auto const chan = channel(alice, bob, env.seq(alice));
            env(create(alice, bob, MUSO(1000), 3600s, alice.pk()));
            ...
            env(fset(bob, asfDisallowMUSO));
            auto const reqBal = MUSO(500).value();
            env(claim(alice, chan, reqBal, reqBal), ter(tecNO_TARGET));
        }
        {
            // Claim to a channel where dst disallows MUSO (channel is
            // created before disallow MUSO is set).  Ignore that flag
    ...
    testDepositAuth()
    {
        ...
        auto USDA = alice["USD"];
        {
            ...
            env(claim(alice, chan, MUSO(500).value(), MUSO(500).value()),
            ...
            auto const preBob = env.balance(bob);
            {
                auto const delta = MUSO(500).value();
            ...
            }
            {
                ...
                auto const delta = MUSO(600).value();
                auto const sig = signClaimAuth(pk, alice.sk(), chan, delta);
            ...
            }
            {
                ...
                auto const delta = MUSO(800).value();
                ...
                env.close();
                BEAST_EXPECT(
                    env.balance(bob) == preBob + MUSO(800) - (5 * baseFee));
            }
        }
    }
    ...
    testAccountChannelsRPC()
    {
        ...
        env.fund(MUSO(10000), alice, bob, charlie);
        auto const pk = alice.pk();
        auto const settleDelay = 3600s;
        auto const channelFunds = MUSO(1000);
    ...
    testAccountChannelsRPCMarkers()
    {
...

        {
            ...
            auto const settleDelay = 3600s;
            auto const channelFunds = MUSO(1);
    ...
    testMalformedPK()
    {
        ...
        auto const chan = channel(alice, bob, env.seq(alice));
        auto jv = create(alice, bob, MUSO(1000), settleDelay, pk);
    ...
    testMetaAndOwnership()
    {
        ...
        auto inOwnerDir = [](ReadView const& view,
            ...
            MUSO::Dir const ownerDir(view, keylet::ownerDir(acc.id()));
    ...
    testAccountDelete()
    {
        ...
        for (bool const withOwnerDirFix : {false, true})
        {
            ...
            auto chanAmt = channelAmount(*env.current(), chan);
            BEAST_EXPECT(chanBal == MUSO(0));
            BEAST_EXPECT(chanAmt == MUSO(1000));

            auto preBob = env.balance(bob);
            auto const delta = MUSO(50);
            auto reqBal = chanBal + delta;
            auto authAmt = reqBal + MUSO(100);
            assert(reqBal <= chanAmt);
            ...
            if (withOwnerDirFix)
            {
                ...
                env.close();
                BEAST_EXPECT(
                    env.balance(alice) == preAlice - MUSO(1000) - feeDrops);
                BEAST_EXPECT(
                    channelAmount(*env.current(), chan) == chanAmt + MUSO(1000));
                chanAmt = chanAmt + MUSO(1000);
            }
            else
            {
                auto const preAlice = env.balance(alice);
                env(fund(alice, chan, MUSO(1000)), ter(tecNO_DST));
                env.close();
                BEAST_EXPECT(env.balance(alice) == preAlice - feeDrops);
                BEAST_EXPECT(channelAmount(*env.current(), chan) == chanAmt);
            }
    ...
    testUsingTickets()
    {
        ...
        {
            auto const preAlice = env.balance(alice);
            env(fund(alice, chan, MUSO(1000)), ticket::use(aliceTicketSeq++));
...
BEAST_DEFINE_TESTSUITE(PayChan, app, MUSO);
}  // namespace test
}  // namespace MUSO
