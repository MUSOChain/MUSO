...
#include <MUSO/app/tx/applySteps.h>
...
struct Escrow_test : public beast::unit_test::suite
{
    ...
    testEnablement()
    {
        ...
        Env env(*this);
        env.fund(MUSO(5000), "alice", "bob");
        env(escrow("alice", "bob", MUSO(1000)), finish_time(env.now() + 1s));
        env.close();

        auto const seq1 = env.seq("alice");

        env(escrow("alice", "bob", MUSO(1000)),
            condition(cb1),
    ...
    testTiming()
    {
...

        {
            ...
            env(escrow("alice", "bob", MUSO(1000)), finish_time(ts));
    ...
    testDisallowMUSO()
    {
        ...
        {
            // Respect the "asfDisallowMUSO" account flag:
            Env env(*this, supported_amendments() - featureDepositAuth);

            env.fund(MUSO(5000), "bob", "george");
            env(fset("george", asfDisallowMUSO));
            env(escrow("bob", "george", MUSO(10)),
                finish_time(env.now() + 1s),
                ter(tecNO_TARGET));
        }
        {
            // Ignore the "asfDisallowMUSO" account flag, which we should
            // have been doing before.
            Env env(*this);

            env.fund(MUSO(5000), "bob", "george");
            env(fset("george", asfDisallowMUSO));
            env(escrow("bob", "george", MUSO(10)), finish_time(env.now() + 1s));
        }
    }
    ...
    test1571()
    {
...

        {
            ...
            env(finish("carol", "alice", seq2),
                ...
                fulfillment(fb1),
                fee(1500));
            BEAST_EXPECT(env.balance("bob") == MUSO(5200));
        }
    ...
    testFails()
    {
        ...
        env(escrow("alice", "carol", MUSO(1000)),
            finish_time(env.now() + 1s),
            ter(tecNO_DST));

        env.fund(MUSO(5000), "carol");
        ...
        // Sending zero or no MUSO:
        env(escrow("alice", "carol", MUSO(0)),
            finish_time(env.now() + 1s),
            ter(temBAD_AMOUNT));
        env(escrow("alice", "carol", MUSO(-1000)),
            finish_time(env.now() + 1s),
        ...
        env(escrow("alice", "carol", MUSO(1)), ter(temBAD_EXPIRATION));

        // Fail if neither a FinishTime nor a condition are attached:
        env(escrow("alice", "carol", MUSO(1)),
            cancel_time(env.now() + 1s),
            ter(temMALFORMED));
        ...
        {  // Fail if the sender wants to send more than he has:
            ...
            env.fund(accountReserve + accountIncrement + MUSO(50), "daniel");
            env(escrow("daniel", "bob", MUSO(51)),
                finish_time(env.now() + 1s),
                ter(tecUNFUNDED));

            env.fund(accountReserve + accountIncrement + MUSO(50), "evan");
            env(escrow("evan", "bob", MUSO(50)),
                finish_time(env.now() + 1s),
                ter(tecUNFUNDED));

            env.fund(accountReserve, "frank");
            env(escrow("frank", "bob", MUSO(1)),
                finish_time(env.now() + 1s),
                ter(tecINSUFFICIENT_RESERVE));
        }

        {  // Specify incorrect sequence number
            env.fund(MUSO(5000), "hannah");
            auto const seq = env.seq("hannah");
            env(escrow("hannah", "hannah", MUSO(10)),
                finish_time(env.now() + 1s),
                fee(1500));
            env.close();
        ...
        {  // Try to specify a condition for a non-conditional payment
            env.fund(MUSO(5000), "ivan");
            auto const seq = env.seq("ivan");

            env(escrow("ivan", "ivan", MUSO(10)), finish_time(env.now() + 1s));
            env.close();
    ...
    testLockup()
    {
...

        {
            ...
            env.fund(MUSO(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(escrow("alice", "alice", MUSO(1000)),
                finish_time(env.now() + 5s));
            env.require(balance("alice", MUSO(4000) - drops(10)));
            ...
            env(finish("bob", "alice", seq));
            env.require(balance("alice", MUSO(5000) - drops(10)));
        }
        {
            ...
            Env env(*this);
            env.fund(MUSO(5000), "alice", "bob", "zelda");
            auto const seq = env.seq("alice");
            env(escrow("alice", "bob", MUSO(1000)), finish_time(env.now() + 5s));
            env.require(balance("alice", MUSO(4000) - drops(10)));
            ...
            env.require(balance("alice", MUSO(4000) - drops(10)));
            env.require(balance("bob", MUSO(6000)));
            env.require(balance("zelda", MUSO(5000) - drops(40)));
        }
        {
            ...
            env.require(balance("alice", MUSO(4000) - (baseFee * 5)));
            env.require(balance("bob", MUSO(6000) - (baseFee * 5)));
            env.require(balance("zelda", MUSO(5000) - (baseFee * 4)));
        }
        {
            ...
            auto const baseFee = env.current()->fees().base;
            env.require(balance("alice", MUSO(4000) - (baseFee * 2)));
            env.require(balance("bob", MUSO(6000) - (baseFee * 2)));
            env.require(balance("zelda", MUSO(5000) - (baseFee * 1)));
        }
        {
            // Conditional
            Env env(*this);
    ...
    testEscrowConditions()
    {
        ...
        {  // Test cryptoconditions
            ...
            env.require(balance("alice", MUSO(4000) - drops(10)));
            env.require(balance("carol", MUSO(5000)));
            env(cancel("bob", "alice", seq), ter(tecNO_PERMISSION));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            ...
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 0);
            env.require(balance("carol", MUSO(6000)));
        ...
        {  // Test long & short conditions during creation
            ...
            env(finish("bob", "alice", seq),
                ...
                fulfillment(fb1),
                fee(1500));
            env.require(balance("alice", MUSO(4000) - drops(100)));
            env.require(balance("bob", MUSO(5000) - drops(1500)));
            env.require(balance("carol", MUSO(6000)));
        }
    ...
    testMetaAndOwnership()
    {
...

        {
            ...
            Env env(*this);
            env.fund(MUSO(5000), alice, bruce, carol);
            ...
            {
                MUSO::Dir bod(*env.current(), keylet::ownerDir(bruce.id()));
        ...
        }
        {
            ...
            env(escrow(alice, bruce, MUSO(1000)), finish_time(env.now() + 1s));
            BEAST_EXPECT(
                (*env.meta())[sfTransactionResult] ==
                static_cast<std::uint8_t>(tesSUCCESS));
            env.close(5s);
            env(escrow(bruce, carol, MUSO(1000)),
                finish_time(env.now() + 1s),
                cancel_time(env.now() + 2s));
            BEAST_EXPECT(
                (*env.meta())[sfTransactionResult] ==
...

            {
                ...
                MUSO::Dir cod(*env.current(), keylet::ownerDir(carol.id()));
    ...
    testConsequences()
    {
...

        {
            ...
            BEAST_EXPECT(pf.consequences.potentialSpend() == MUSO(1000));
        }
...
BEAST_DEFINE_TESTSUITE(Escrow, app, MUSO);
