    ...
    run() override
    {
        ...
        LoadFeeTrack l;
        {
            Fees const fees = [&]() {
                ...
                f.reserve = 200 * DROPS_PER_MUSO;
                f.increment = 50 * DROPS_PER_MUSO;
                return f;
            }();

            BEAST_EXPECT(
                scaleFeeLoad(FeeUnit64{0}, l, fees, false) == MUSOAmount{0});
            BEAST_EXPECT(
                scaleFeeLoad(FeeUnit64{10000}, l, fees, false) ==
                MUSOAmount{10000});
            BEAST_EXPECT(
                scaleFeeLoad(FeeUnit64{1}, l, fees, false) == MUSOAmount{1});
        }
        {
            Fees const fees = [&]() {
                Fees f;
            ...
            BEAST_EXPECT(
                ...
                MUSOAmount{100000});
            BEAST_EXPECT(
                scaleFeeLoad(FeeUnit64{1}, l, fees, false) == MUSOAmount{10});
        }
        {
            Fees const fees = [&]() {
                Fees f;
            ...
            BEAST_EXPECT(
                ...
                MUSOAmount{1000});
            BEAST_EXPECT(
                scaleFeeLoad(FeeUnit64{1}, l, fees, false) == MUSOAmount{0});
        }
    }
};

BEAST_DEFINE_TESTSUITE(LoadFeeTrack, MUSO_core, MUSO);
