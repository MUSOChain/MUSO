...
public:
    ...
    testStepLimit(FeatureBitset features)
    {
        ...
        env.fund(MUSO(100000000), gw, "alice", "bob", "carol", "dan");
        env.trust(USD(1), "bob");
        env(pay(gw, "bob", USD(1)));
        env.trust(USD(1), "dan");
        env(pay(gw, "dan", USD(1)));
        n_offers(env, 2000, "bob", MUSO(1), USD(1));
        n_offers(env, 1, "dan", MUSO(1), USD(1));

        // Alice offers to buy 1000 MUSO for 1000 USD. She takes Bob's first
        ...
        // Carol offers to buy 1000 MUSO for 1000 USD. She removes Bob's next
        // 1000 offers as unfunded and hits the step limit.
        env(offer("carol", USD(1000), MUSO(1000)));
    ...
    testStepAndCrossingLimit(FeatureBitset features)
    {
        ...
        env.fund(MUSO(100000000), gw, "alice", "bob", "carol", "dan", "evita");
        ...
        n_offers(env, 400, "alice", MUSO(1), USD(1));
        n_offers(env, carolsOfferCount, "carol", MUSO(1), USD(1));
        n_offers(env, evitasOfferCount, "evita", MUSO(1), USD(1));

        // Bob offers to buy 1000 MUSO for 1000 USD. He takes all 400 USD from
        // Alice's offers, 1 USD from Carol's and then removes 599 of Carol's
        // offers as unfunded, before hitting the step limit.
        env(offer("bob", USD(1000), MUSO(1000)));
        ...
        // Dan offers to buy maxConsumed + 50 MUSO USD. He removes all of
        // Carol's remaining offers as unfunded, then takes
        // (maxConsumed - 100) USD from Evita's, hitting the crossing limit.
        env(offer("dan", USD(maxConsumed + 50), MUSO(maxConsumed + 50)));
    ...
    testAutoBridgedLimitsTaker(FeatureBitset features)
    {
        ...
        n_offers(env, 302, "alice", EUR(2), MUSO(1));
        n_offers(env, 300, "alice", MUSO(1), USD(4));
        n_offers(env, 497, "carol", MUSO(1), USD(3));
        ...
        //  2. He spends another 2 EUR taking one of Alice's EUR->MUSO and
        //     one of Carol's MUSO-USD offers.  He gets 3 USD for that.
    ...
    testAutoBridgedLimitsFlowCross(FeatureBitset features)
    {
        ...
        // payment stops.
        {
            ...
            // best quality
            n_offers(env, 2000, alice, EUR(2), MUSO(1));
            n_offers(env, 100, alice, MUSO(1), USD(4));
            n_offers(
                env, 801, carol, MUSO(1), USD(3));  // only one offer is funded
            n_offers(env, 1000, alice, MUSO(1), USD(3));
            ...
            //     100 EUR->MUSO offers consumed.
            //     100 MUSO->USD offers consumed.
            ...
            //     c. 199 of alice's MUSO(1) to USD(3) offers are consumed.
            ...
            //     MUSO(1) to USD(3) offers available.
            //        Bob has spent 400 EUR and received 600 USD in this step.
            //        200 EUR->MUSO offers consumed
            //        800 unfunded MUSO->USD offers consumed
            //        200 funded MUSO->USD offers consumed (1 carol, 199 alice)
        ...
        }
        {
            ...
            //        at a given quality, and that limit is now reached.
            //     d. Now the strand is dry, even though there are still funded
            //     MUSO(1) to USD(3) offers available. Bob has spent 400 EUR and
    ...
    testOfferOverflow(FeatureBitset features)
    {
        ...
        env.fund(MUSO(100000000), gw, alice, bob);
        ...
        n_offers(env, 998, alice, MUSO(1.00), USD(1));
        n_offers(env, 998, alice, MUSO(0.99), USD(1));
        n_offers(env, 998, alice, MUSO(0.98), USD(1));
        n_offers(env, 998, alice, MUSO(0.97), USD(1));
        n_offers(env, 998, alice, MUSO(0.96), USD(1));
        n_offers(env, 998, alice, MUSO(0.95), USD(1));
        ...
        env(offer(bob, USD(8000), MUSO(8000)), ter(expectedTER));
        env.close();
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(CrossingLimits, tx, MUSO, 10);
