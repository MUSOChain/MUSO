...
class Discrepancy_test : public beast::unit_test::suite
{
    ...
    void
    testMUSODiscrepancy(FeatureBitset features)
    {
        testcase("Discrepancy test : MUSO Discrepancy");
        ...
        env.fund(MUSO(2000), A1);
        env.fund(MUSO(1000), A2, A6, A7);
        env.fund(MUSO(5000), A3);
        env.fund(MUSO(1000000), A4);
        env.fund(MUSO(600000), A5);
        env.close();
        ...
        env(offer(A4, MUSO(49147), A2["JPY"](34501)));
        env(offer(A5, A3["CNY"](3150), MUSO(80086)));
        env(offer(A7, MUSO(1233), A6["CNY"](25)));
        env.close();

        test::PathSet payPaths{
            test::Path{A2["JPY"], A2},
            test::Path{MUSO, A2["JPY"], A2},
            test::Path{A6, MUSO, A2["JPY"], A2}};
...
public:
    ...
    run() override
    {
        ...
        testMUSODiscrepancy(sa - featureFlowCross);
        testMUSODiscrepancy(sa);
    }
};

BEAST_DEFINE_TESTSUITE(Discrepancy, app, MUSO);
