...
class TrustAndBalance_test : public beast::unit_test::suite
{
    ...
    testPayNonexistent(FeatureBitset features)
    {
        ...
        env(pay(env.master, "alice", MUSO(1)), ter(tecNO_DST_INSUF_MUSO));
        env.close();
    }
    ...
    void
    testDirectMUSO(FeatureBitset features)
    {
        testcase("Direct Payment, MUSO");
        using namespace test::jtx;
    ...
    testIndirectMultiPath(bool with_rate, FeatureBitset features)
    {
        ...
        env.fund(MUSO(10000), gw, amazon, alice, bob, carol);
        env.close();
    ...
    testInvoiceID(FeatureBitset features)
    {
        ...
        Json::Value jv;
        auto tx = env.jt(
            pay(env.master, alice, MUSO(10000)),
...
BEAST_DEFINE_TESTSUITE_PRIO(TrustAndBalance, app, MUSO, 1);
