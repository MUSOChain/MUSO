...
class Ticket_test : public beast::unit_test::suite
{
    ...
    testTicketCreatePreflightFail()
    {
        ...
        env(ticket::create(master, 1), fee(MUSO(10)));
        checkTicketCreateMeta(env);
        env.close();
        env.require(owners(master, 1), tickets(master, 1));

        env(ticket::create(master, 1), fee(MUSO(-1)), ter(temBAD_FEE));
    ...
    testUsingTickets()
    {
        ...
        env(pay(alice, env.master, MUSO(20)), ticket::use(ticketSeq_DE + 1));
        checkTicketConsumeMeta(env);
        env.close();
        env.require(owners(alice, 1), tickets(alice, 1));
        BEAST_EXPECT(ticketSeq_DE + 2 == env.seq(alice));
    ...
    testTransactionDatabaseWithTickets()
    {
        ...
        env(pay(alice, env.master, MUSO(200)), ticket::use(--ticketSeq));
        ...
        env(pay(alice, env.master, MUSO(300)), ticket::use(--ticketSeq));
        uint256 const txHash_5{getTxID()};

        env(pay(alice, env.master, MUSO(400)), ticket::use(--ticketSeq));
...
BEAST_DEFINE_TESTSUITE(Ticket, tx, MUSO);
