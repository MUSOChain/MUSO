...
struct Regression_test : public beast::unit_test::suite
{
    ...
    testOffer1()
    {
        ...
        env(offer("alice", USD(10), MUSO(10)), require(owners("alice", 1)));
        env(offer("alice", USD(20), MUSO(10)),
            json(R"raw(
                { "OfferSequence" : 4 }
            )raw"),
    ...
    {
        testcase("Account balance < fee destroys correct amount of MUSO");
        using namespace jtx;
        Env env(*this);
        env.memoize("alice");
        ...
        auto expectedDrops = INITIAL_MUSO;
        BEAST_EXPECT(closed->info().drops == expectedDrops);

        auto const aliceMUSO = 400;
        auto const aliceAmount = MUSO(aliceMUSO);
            ...
            *closed, env.app().timeKeeper().closeTime());
        {
            ...
            auto const result =
                MUSO::apply(env.app(), accum, *jt.stx, tapNONE, env.journal);
        ...
        }
        {
            ...
            BEAST_EXPECT(balance == MUSO(0));
        }
        expectedDrops -= aliceMUSO * dropsPerMUSO;
        BEAST_EXPECT(next->info().drops == expectedDrops);
    }
...
BEAST_DEFINE_TESTSUITE(Regression, app, MUSO);
