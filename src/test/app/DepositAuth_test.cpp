...
struct DepositAuth_test : public beast::unit_test::suite
{
    ...
    testPayIOU()
    {
        // Exercise IOU payments and non-direct MUSO payments to an account
        ...
        env(offer(carol, USD(100), MUSO(100)));
        env.close();
        ...
        auto failedIouPayments = [this, &env, &alice, &bob, &USD]() {
            ...
            PrettyAmount const bobMUSOBalance{env.balance(bob, MUSO)};
            PrettyAmount const bobUsdBalance{env.balance(bob, USD)};

            env(pay(alice, bob, USD(50)), ter(tecNO_PERMISSION));
            env.close();

            // Note that even though alice is paying bob in MUSO, the payment
            ...
            BEAST_EXPECT(bobMUSOBalance == env.balance(bob, MUSO));
            BEAST_EXPECT(bobUsdBalance == env.balance(bob, USD));
        };

        //  Test when bob has an MUSO balance > base reserve.
        failedIouPayments();

        // Set bob's MUSO balance == base reserve.  Also demonstrate that
        // bob can make payments while his lsfDepositAuth flag is set.
        env(pay(bob, alice, USD(25)));
        env.close();

        {
            STAmount const bobPaysMUSO{env.balance(bob, MUSO) - reserve(env, 1)};
            MUSOAmount const bobPaysFee{reserve(env, 1) - reserve(env, 0)};
            env(pay(bob, alice, bobPaysMUSO), fee(bobPaysFee));
            env.close();
        }

        // Test when bob's MUSO balance == base reserve.
        BEAST_EXPECT(env.balance(bob, MUSO) == reserve(env, 0));
        BEAST_EXPECT(env.balance(bob, USD) == USD(25));
        failedIouPayments();

        // Test when bob has an MUSO balance == 0.
        env(noop(bob), fee(reserve(env, 0)));
        env.close();

        BEAST_EXPECT(env.balance(bob, MUSO) == MUSO(0));
        failedIouPayments();

        // Give bob enough MUSO for the fee to clear the lsfDepositAuth flag.
    ...
    testPayMUSO()
    {
        // Exercise direct MUSO payments to an account that has the
        ...
        env.close();
        BEAST_EXPECT(env.balance(bob, MUSO) == MUSO(10000) - drops(10));

        // bob has more MUSO than the base reserve.  Any MUSO payment should fail.
        env(pay(alice, bob, drops(1)), ter(tecNO_PERMISSION));
        env.close();
        BEAST_EXPECT(env.balance(bob, MUSO) == MUSO(10000) - drops(10));

        // Change bob's MUSO balance to exactly the base reserve.
        {
            STAmount const bobPaysMUSO{env.balance(bob, MUSO) - reserve(env, 1)};
            MUSOAmount const bobPaysFee{reserve(env, 1) - reserve(env, 0)};
            env(pay(bob, alice, bobPaysMUSO), fee(bobPaysFee));
        ...
        // bob has exactly the base reserve.  A small enough direct MUSO
        ...
        BEAST_EXPECT(env.balance(bob, MUSO) == reserve(env, 0) + drops(1));
        env(pay(alice, bob, drops(1)), ter(tecNO_PERMISSION));
        env.close();

        // Take bob down to a balance of 0 MUSO.
        env(noop(bob), fee(reserve(env, 0) + drops(1)));
        env.close();
        BEAST_EXPECT(env.balance(bob, MUSO) == drops(0));
        ...
        env.close();
        BEAST_EXPECT(
            env.balance(bob, MUSO) == (reserve(env, 0) + reserve(env, 0)));
        ...
        // Take bob back down to a zero MUSO balance.
        env(noop(bob), fee(env.balance(bob, MUSO)));
        env.close();
        BEAST_EXPECT(env.balance(bob, MUSO) == drops(0));
        ...
        env(pay(alice, bob, drops(1)));
        env.close();
        BEAST_EXPECT(env.balance(bob, MUSO) == drops(1));
    ...
    testNoMUSO()
    {
        ...
        // any behaviors regarding rippling and the NoMUSO flag.
        // Demonstrate that.
        testcase("No MUSO");
        ...
        auto testIssuer = [&](FeatureBitset const& features,
            ...
            env.fund(MUSO(10000), gw1, alice, bob);
            env(trust(gw1, alice["USD"](10), noMUSOPrev ? tfSetNoMUSO : 0));
            env(trust(gw1, bob["USD"](10), noMUSONext ? tfSetNoMUSO : 0));
            ...
            TER const result = (noMUSONext && noMUSOPrev) ? TER{tecPATH_DRY}
                                                              : TER{tesSUCCESS};
        ...
        auto testNonIssuer = [&](FeatureBitset const& features,
            ...
            env.fund(MUSO(10000), gw1, gw2, alice);
            env(trust(alice, USD1(10), noMUSOPrev ? tfSetNoMUSO : 0));
            env(trust(alice, USD2(10), noMUSONext ? tfSetNoMUSO : 0));
        ...
        // Test every combo of noMUSOPrev, noMUSONext, and withDepositAuth
        for (int i = 0; i < 8; ++i)
        {
            auto const noMUSOPrev = i & 0x1;
            auto const noMUSONext = i & 0x2;
...
struct DepositPreauth_test : public beast::unit_test::suite
{
    ...
    testEnable()
    {
        ...
        Account const becky{"becky"};
        {
            ...
            env.fund(MUSO(10000), alice, becky);
            env.close();
    ...
    testInvalid()
    {
        ...
        }
        {
            ...
            Json::Value tx{deposit::auth(alice, becky)};
            tx[sfAuthorize.jsonName] = to_string(MUSOAccount());
        ...
        // carol gets enough MUSO to (barely) meet the reserve.
    ...
    testPayment(FeatureBitset features)
    {
...

        {
            ...
            Env env(*this, features);
            env.fund(MUSO(5000), alice, becky, gw);
            env.close();
            ...
            env(offer(alice, MUSO(100), USD(100), tfPassive),
            ...
            env(pay(becky, becky, USD(10)), path(~USD), sendmax(MUSO(10)));
            env.close();
        ...
        if (supportsPreauth)
        {
            ...
            Env env(*this, features);
            env.fund(MUSO(5000), alice, becky, carol, gw);
            env.close();
            ...
            // Make MUSO and IOU payments from alice to becky.  Should be fine.
            env(pay(alice, becky, MUSO(100)));
            ...
            env(pay(alice, becky, MUSO(100)), ter(tecNO_PERMISSION));
            ...
            env(pay(becky, alice, MUSO(100)), ter(tecNO_PERMISSION));
...
BEAST_DEFINE_TESTSUITE(DepositAuth, app, MUSO);
BEAST_DEFINE_TESTSUITE(DepositPreauth, app, MUSO);
