...
class Check_test : public beast::unit_test::suite
{
    ...
    testEnabled()
    {
        ...
        Account const alice{"alice"};
        {
            ...
            env.fund(MUSO(1000), alice);

            uint256 const checkId{
                getCheckIndex(env.master, env.seq(env.master))};
            env(check::create(env.master, alice, MUSO(100)), ter(temDISABLED));
            env.close();

            env(check::cash(alice, checkId, MUSO(100)), ter(temDISABLED));
            env.close();
        ...
        }
        {
            ...
            uint256 const checkId1{
                getCheckIndex(env.master, env.seq(env.master))};
            env(check::create(env.master, alice, MUSO(100)));
            env.close();

            env(check::cash(alice, checkId1, MUSO(100)));
            env.close();
    ...
    testCreateValid()
    {
        ...
        STAmount const startBalance{MUSO(1000).value()};
        ...
        auto writeTwoChecks = [&env, &USD, this](
            ...
            env(check::create(from, to, MUSO(2000)));
            env.close();
        ...
        MUSOAmount const baseFeeDrops{env.current()->fees().base};
        env(check::create(alice, bob, USD(50)),
            msig(bogie, demon),
            fee(3 * baseFeeDrops));
        env.close();
    ...
    testCreateInvalid()
    {
        ...
        // Check to self.
        env(check::create(alice, alice, MUSO(10)), ter(temREDUNDANT));
        env.close();
    ...
    testCashMUSO()
    {
        // Explore many of the valid ways to cash a check for MUSO.
        testcase("Cash MUSO");
        ...
        env.fund(startBalance, alice, bob);
        {
            ...
            env(check::cash(bob, chkId, MUSO(10)));
            env.close();
            env.require(
                balance(alice, startBalance - MUSO(10) - drops(baseFeeDrops)));
            env.require(
                balance(bob, startBalance + MUSO(10) - drops(baseFeeDrops)));
            ...
            env(pay(env.master, alice, MUSO(10) + drops(baseFeeDrops)));
            env(pay(bob, env.master, MUSO(10) - drops(baseFeeDrops * 2)));
            env.close();
    ...
    testCashIOU()
    {
        ...
        IOU const USD{gw["USD"]};
        {
            ...
            env.fund(MUSO(1000), gw, alice, bob);
    ...
    testCashInvalid()
    {
        ...
        env.fund(MUSO(1000), gw, alice, bob, zoe);
        ...
        env(check::create(alice, bob, MUSO(10)), expiration(env.now() + 1s));
        env.close();
        ...
        // Same set of failing cases for both IOU and MUSO check cashing.
        ...
        failingCases(chkIdX, MUSO(10));
        failingCases(chkIdU, USD(20));

        // Verify that those two checks really were cashable.
        env(check::cash(bob, chkIdU, USD(20)));
        env.close();
        env(check::cash(bob, chkIdX, check::DeliverMin(MUSO(10))));
        verifyDeliveredAmount(env, MUSO(10));

        // Try to cash an expired check.
        env(check::cash(bob, chkIdExp, MUSO(10)), ter(tecEXPIRED));
        env.close();
    ...
    testFix1623Enable()
    {
        ...
        auto testEnable = [this](
            ...
            uint256 const chkId{getCheckIndex(alice, env.seq(alice))};
            env(check::create(alice, bob, MUSO(200)));
            env.close();

            env(check::cash(bob, chkId, check::DeliverMin(MUSO(100))));
    ...
    testWithTickets()
    {
        ...
        // alice creates four checks; two MUSO, two IOU.  Bob will cash
        // one of each and cancel one of each.
        uint256 const chkIdMUSO1{getCheckIndex(alice, aliceTicketSeq)};
        env(check::create(alice, bob, MUSO(200)), ticket::use(aliceTicketSeq++));

        uint256 const chkIdMUSO2{getCheckIndex(alice, aliceTicketSeq)};
        env(check::create(alice, bob, MUSO(300)), ticket::use(aliceTicketSeq++));
        ...
        env(check::cancel(bob, chkIdMUSO1), ticket::use(bobTicketSeq++));
        ...
        env(check::cash(bob, chkIdMUSO2, MUSO(300)), ticket::use(bobTicketSeq++));
...
BEAST_DEFINE_TESTSUITE(Check, tx, MUSO);
