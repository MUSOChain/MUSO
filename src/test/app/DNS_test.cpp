...
#include <MUSO/app/misc/detail/WorkSSL.h>
...
class DNS_test : public beast::unit_test::suite
{
    ...
    std::weak_ptr<MUSO::detail::Work> work_;
...
public:
    ...
    makeRequest(endpoint_type const& lastEndpoint, bool lastStatus)
    {
        auto onFetch = [&](error_code const& errorCode,
                           ...
                           MUSO::detail::response_type&& resp) {
        ...
        auto sp = std::make_shared<MUSO::detail::WorkSSL>(
            pUrl_.domain,
            pUrl_.path,
            port_,
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(DNS, MUSO_data, MUSO, 20);
