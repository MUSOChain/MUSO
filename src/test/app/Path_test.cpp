...
public:
        ...
        boost::optional<Currency> const& saSrcCurrency = boost::none)
    {
        ...
        params[jss::command] = "MUSO_path_find";
    ...
    {
        using namespace jtx;
        testcase("MUSO to MUSO");
        Env env(*this);
        env.fund(MUSO(10000), "alice", "bob");

        auto const result = find_paths(env, "alice", "bob", MUSO(5));
        BEAST_EXPECT(std::get<0>(result).empty());
    }
    ...
    path_find_consume_all()
    {
        ...
        {
            Env env(*this);
            env.fund(MUSO(10000), "alice", "bob", "carol", "dan", "edward");
...

        {
            ...
            env(offer("carol", MUSO(100), USD(100)));
            ...
            std::tie(st, sa, da) = find_paths(
                ...
                "bob",
                Account("bob")["AUD"](-1),
                boost::optional<STAmount>(MUSO(100000000)));
            BEAST_EXPECT(st.empty());
            std::tie(st, sa, da) = find_paths(
                env,
                "alice",
    ...
    alternative_path_consume_both()
    {
        ...
        env.fund(MUSO(10000), "alice", "bob", gw, gw2);
        env.trust(USD(600), "alice");
    ...
    alternative_paths_limit_returned_paths_to_best_quality()
    {
        ...
        auto const gw2 = Account("gateway2");
        auto const gw2_USD = gw2["USD"];
        env.fund(MUSO(10000), "alice", "bob", "carol", "dan", gw, gw2);
        env(rate("carol", 1.1));
    ...
    void
    issues_path_negative_MUSO_client_issue_23_smaller()
    {
        testcase("path negative: MUSO-client issue #23: smaller");
        using namespace jtx;
        Env env(*this);
        env.fund(MUSO(10000), "alice", "bob", "carol", "dan");
    ...
    void
    issues_path_negative_MUSO_client_issue_23_larger()
    {
        testcase("path negative: MUSO-client issue #23: larger");
        using namespace jtx;
        Env env(*this);
        env.fund(MUSO(10000), "alice", "bob", "carol", "dan", "edward");
    ...
    // carol holds gateway AUD, sells gateway AUD for MUSO
    // bob will hold gateway AUD
    // alice pays bob gateway AUD using MUSO
    void
    via_offers_via_gateway()
    {
        ...
        env(offer("carol", MUSO(50), AUD(50)));
        env(pay("alice", "bob", AUD(10)), sendmax(MUSO(100)), paths(MUSO));
    ...
    path_find_01()
    {
        testcase("Path Find: MUSO -> MUSO and MUSO -> IOU");
        using namespace jtx;
        Env env(*this);
        Account A1{"A1"};
        Account A2{"A2"};
        ...
        env.fund(MUSO(100000), A1);
        env.fund(MUSO(10000), A2);
        env.fund(MUSO(1000), A3, G1, G2, G3, M1);
        env.close();
        ...
        env(offer(M1, MUSO(10000), G3["ABC"](1000)));

        STPathSet st;
        STAmount sa, da;

        {
            auto const& send_amt = MUSO(10);
            std::tie(st, sa, da) =
                find_paths(env, A1, A2, send_amt, boost::none, MUSOCurrency());
...

        {
            ...
            // does not exist.
            auto const& send_amt = MUSO(200);
            std::tie(st, sa, da) = find_paths(
                env, A1, Account{"A0"}, send_amt, boost::none, MUSOCurrency());
            BEAST_EXPECT(equal(da, send_amt));
            BEAST_EXPECT(st.empty());
        }

        {
            ...
            std::tie(st, sa, da) =
                find_paths(env, A2, G3, send_amt, boost::none, MUSOCurrency());
            BEAST_EXPECT(equal(da, send_amt));
            BEAST_EXPECT(equal(sa, MUSO(100)));
...

        {
            ...
            std::tie(st, sa, da) =
                find_paths(env, A1, A3, send_amt, boost::none, MUSOCurrency());
            BEAST_EXPECT(equal(da, send_amt));
            BEAST_EXPECT(equal(sa, MUSO(10)));
    ...
    path_find_02()
    {
        testcase("Path Find: non-MUSO -> MUSO");
        using namespace jtx;
        Env env(*this);
        Account A1{"A1"};
        Account A2{"A2"};
        ...
        env.fund(MUSO(1000), A1, A2, G3);
        env.fund(MUSO(11000), M1);
        env.close();
        ...
        env(offer(M1, G3["ABC"](1000), MUSO(10000)));
        ...
        BEAST_EXPECT(same(st, stpath(G3, IPE(MUSOIssue()))));
    }
    ...
    path_find_04()
    {
        ...
        env.fund(MUSO(1000), G1BS, G2SW, A1, A2);
        env.fund(MUSO(11000), M1);
        env.close();
        ...
        // can charge a fee when transactions MUSO across. Liquidity
    ...
    path_find_05()
    {
        testcase("Path Find: non-MUSO -> non-MUSO, same currency");
        using namespace jtx;
        Env env(*this);
        Account A1{"A1"};
        Account A2{"A2"};
        ...
        env.fund(MUSO(1000), A1, A2, A3, G1, G2, G3, G4);
        env.fund(MUSO(10000), A4);
        env.fund(MUSO(11000), M1, M2);
        env.close();
        ...
        env(offer(M2, MUSO(10000), G2["HKD"](1000)));
        env(offer(M2, G1["HKD"](1000), MUSO(10000)));
...

        {
            ...
            BEAST_EXPECT(same(
                ...
                stpath(M1),
                stpath(M2),
                stpath(IPE(MUSOIssue()), IPE(G2["HKD"]))));
        }

        {
            ...
            BEAST_EXPECT(same(
                ...
                stpath(G1, IPE(MUSOIssue()), IPE(G2["HKD"]))));
        }

        {
            // I4) MUSO bridge" --
            //  Source -> AC -> OB to MUSO -> OB from MUSO -> AC -> Destination
            auto const& send_amt = A2["HKD"](10);
            std::tie(st, sa, da) = find_paths(
            ...
            BEAST_EXPECT(same(
                ...
                stpath(G1, IPE(MUSOIssue()), IPE(G2["HKD"]), G2)));
        }
    }
    ...
    path_find_06()
    {
        ...
        env.fund(MUSO(11000), M1);
        env.fund(MUSO(1000), A1, A2, A3, G1, G2);
        env.close();
    ...
    receive_max()
    {
        ...
        auto const USD = gw["USD"];
        {
            // MUSO -> IOU receive max
            Env env(*this);
            env.fund(MUSO(10000), alice, bob, charlie, gw);
            env.close();
            env.trust(USD(100), alice, bob, charlie);
            env.close();
            env(pay(gw, charlie, USD(10)));
            env.close();
            env(offer(charlie, MUSO(10), USD(10)));
            env.close();
            auto [st, sa, da] =
                find_paths(env, alice, bob, USD(-1), MUSO(100).value());
            BEAST_EXPECT(sa == MUSO(10));
        ...
        }
        {
            ...
            env.close();
            env(offer(charlie, USD(10), MUSO(10)));
            env.close();
            auto [st, sa, da] =
                find_paths(env, alice, bob, drops(-1), USD(100).value());
            BEAST_EXPECT(sa == USD(10));
            BEAST_EXPECT(equal(da, MUSO(10)));
            if (BEAST_EXPECT(st.size() == 1 && st[0].size() == 1))
            {
                auto const& pathElem = st[0][0];
                BEAST_EXPECT(
                    pathElem.isOffer() &&
                    pathElem.getIssuerID() == MUSOAccount() &&
                    pathElem.getCurrency() == MUSOCurrency());
            }
        }
    }
...
BEAST_DEFINE_TESTSUITE(Path, app, MUSO);
