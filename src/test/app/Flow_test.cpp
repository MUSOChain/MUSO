...
#include <MUSO/ledger/ApplyViewImpl.h>
#include <MUSO/ledger/PaymentSandbox.h>
#include <MUSO/ledger/Sandbox.h>
    ...
    Currency const& cur)
{
    ...
    {
        auto const flag =
            (src.id() > dst.id()) ? lsfHighNoMUSO : lsfLowNoMUSO;
...
struct Flow_test : public beast::unit_test::suite
{
    ...
    testDirectStep(FeatureBitset features)
    {
        ...
        auto const USD = gw["USD"];
        {
            ...
            env.fund(MUSO(10000), alice, bob, gw);
        ...
        }
        {
            ...
            env.fund(MUSO(10000), alice, bob);
            env(pay(alice, bob, MUSO(100)));
            env.require(balance(bob, MUSO(10000 + 100)));
            env.require(balance(alice, MUSOMinusFee(env, 10000 - 100)));
        }
        {
            // Partial payments
            Env env(*this, features);
        ...
        }
        {
            ...
            env.fund(MUSO(10000), alice, bob, carol, dan);
        ...
        }
        {
            ...
            env.fund(MUSO(10000), alice, bob, carol, dan, erin);
    ...
    testBookStep(FeatureBitset features)
    {
        ...
        {
            // simple IOU/MUSO MUSO/IOU offer
            Env env(*this, features);
            ...
            env(offer(bob, BTC(50), MUSO(50)));
            env(offer(bob, MUSO(50), USD(50)));

            env(pay(alice, carol, USD(50)), path(~MUSO, ~USD), sendmax(BTC(50)));

            env.require(balance(alice, BTC(0)));
            env.require(balance(bob, BTC(50)));
            env.require(balance(bob, USD(0)));
            env.require(balance(carol, USD(50)));
            BEAST_EXPECT(!isOffer(env, bob, MUSO(50), USD(50)));
            BEAST_EXPECT(!isOffer(env, bob, BTC(50), MUSO(50)));
        }
        {
            // simple MUSO -> USD through offer and sendmax
            Env env(*this, features);
            ...
            env(pay(alice, carol, USD(50)), path(~USD), sendmax(MUSO(50)));

            env.require(balance(alice, MUSOMinusFee(env, 10000 - 50)));
            env.require(balance(bob, MUSOMinusFee(env, 10000 + 50)));
            env.require(balance(bob, USD(0)));
            env.require(balance(carol, USD(50)));
            BEAST_EXPECT(!isOffer(env, bob, MUSO(50), USD(50)));
        }
        {
            // simple USD -> MUSO through offer and sendmax
            Env env(*this, features);
            ...
            env(pay(alice, carol, MUSO(50)), path(~MUSO), sendmax(USD(50)));

            env.require(balance(alice, USD(0)));
            env.require(balance(bob, MUSOMinusFee(env, 10000 - 50)));
            env.require(balance(bob, USD(50)));
            env.require(balance(carol, MUSO(10000 + 50)));
            BEAST_EXPECT(!isOffer(env, bob, USD(50), MUSO(50)));
        }
        {
    ...
    testTransferRate(FeatureBitset features)
    {
        ...
        }
        {
            ...
            env(pay(alice, carol, USD(50)), path(~USD), sendmax(MUSO(50)));
            env.require(
                balance(alice, MUSOMinusFee(env, 10000 - 50)),
...

        {
            ...
            env(pay(alice, carol, EUR(40)), path(~USD, ~EUR), sendmax(MUSO(40)));
            env.require(
                balance(alice, MUSOMinusFee(env, 10000 - 40)),
                balance(bob, USD(40)),
                balance(bob, EUR(0)),
        ...
        }
        {
            ...
            env.fund(MUSO(10000), alice, bob, carol, dan, gw);
            env(rate(gw, 1.25));
...

        {
            ...
            env(rate(gw, 1.25));
            env.trust(USD(1000), alice, bob);
            env(offer(gw, MUSO(100), USD(100)));
            env(pay(alice, bob, USD(100)), sendmax(MUSO(100)));
            env.require(
                balance(alice, MUSOMinusFee(env, 10000 - 100)),
                balance(bob, USD(100)));
        }
        if (!features[featureOwnerPaysFee])
        {
            ...
            env(rate(gw, 1.25));
            env.trust(USD(1000), alice, bob);
            env(offer(gw, MUSO(125), USD(125)));
            env(pay(alice, bob, USD(100)), sendmax(MUSO(200)));
            env.require(
                balance(alice, MUSOMinusFee(env, 10000 - 125)),
                balance(bob, USD(100)));
        }
    }
    ...
    testFalseDry(FeatureBitset features)
    {
        ...
        env.fund(MUSO(10000), alice, carol, gw);
    ...
    testLimitQuality()
    {
...

        {
            ...
            env(offer(bob, MUSO(50), USD(50)));
            env(offer(bob, MUSO(100), USD(50)));

            env(pay(alice, carol, USD(100)),
                path(~USD),
                sendmax(MUSO(100)),
                txflags(tfNoMUSOirect | tfPartialPayment | tfLimitQuality));
    ...
    void
    testSelfFundedMUSOEndpoint(bool consumeOffer, FeatureBitset features)
    {
        // Test that the deferred credit table is not bypassed for
        // MUSOEndpointSteps. If the account in the first step is sending MUSO and
        // that account also owns an offer that receives MUSO, it should not be
        // possible for that step to use the MUSO received in the offer as part
        // of the payment.
        testcase("Self funded MUSOEndpoint");
        ...
        env(offer(alice, MUSO(50000), USD(10)));
        ...
        env(pay(alice, alice, toSend),
            path(~USD),
            sendmax(MUSO(20000)),
            txflags(tfPartialPayment | tfNoMUSOirect));
    }
    ...
    testUnfundedOffer(FeatureBitset features)
    {
        ...
        using namespace jtx;
        {
            ...
            env.fund(MUSO(100000), alice, bob, gw);
            ...
            BEAST_EXPECT(!isOffer(env, gw, MUSO(0), USD(0)));
        }
        {
            // Test forward
            Env env(*this, features);
            ...
            BEAST_EXPECT(!isOffer(env, gw, USD(0), MUSO(0)));
        }
    }
    ...
    testReexecuteDirectStep(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(!getNoMUSOFlag(env, gw, alice, usdC));
    ...
    testRIPD1443()
    {
        ...
        env.fund(MUSO(100000000), alice, noMUSO(bob), carol, gw);
        env.trust(gw["USD"](10000), alice, carol);
        env(trust(bob, gw["USD"](10000), tfSetNoMUSO));
        env.trust(gw["USD"](10000), bob);
        env.close();

        // set no MUSO between bob and the gateway

        env(pay(gw, alice, gw["USD"](1000)));
        env.close();

        env(offer(alice, bob["USD"](1000), MUSO(1)));
        env.close();

        env(pay(alice, alice, MUSO(1)),
            path(gw, bob, ~MUSO),
            sendmax(gw["USD"](1000)),
            txflags(tfNoMUSOirect),
            ter(tecPATH_DRY));
        ...
        env(offer(alice, MUSO(1000), bob["USD"](1000)));
        env.close();
    ...
    testRIPD1449()
    {
        ...
        // pay alice -> MUSO -> USD/bob -> bob -> gw -> alice
        // set no MUSO on bob's side of the bob/gw trust line
        ...
        env(trust(bob, USD(10000), tfSetNoMUSO));
        ...
        env(offer(carol, MUSO(1), bob["USD"](1000)));
        env.close();
    ...
    testSelfPayLowQualityOffer(FeatureBitset features)
    {
        ...
        // MUSO than the offering account held.  This unit test reproduces
        // that failing case.
    ...
    testMUSOPathLoop()
    {
        ...
        for (auto const withFix : {true, false})
        {
            ...
            {
                // Payment path starting with MUSO
                Env env(*this, feats);
                env.fund(MUSO(10000), alice, bob, gw);
                env.trust(USD(1000), alice, bob);
                ...
                env(offer(alice, MUSO(100), USD(100)), txflags(tfPassive));
                env(offer(alice, USD(100), MUSO(100)), txflags(tfPassive));
                env(offer(alice, MUSO(100), EUR(100)), txflags(tfPassive));
                env.close();
        ...
        }
        {
            ...
            env(offer(alice, MUSO(100), USD(100)), txflags(tfPassive));
            env(offer(alice, EUR(100), MUSO(100)), txflags(tfPassive));
            env.close();
            // EUR -> //MUSO -> //USD ->MUSO
            env(pay(alice, bob, MUSO(1)),
                path(~MUSO, ~USD, ~MUSO),
                sendmax(EUR(1)),
                txflags(tfNoMUSOirect),
        ...
        }
        {
            ...
            env(offer(alice, MUSO(100), EUR(100)), txflags(tfPassive));
            env(offer(alice, EUR(100), MUSO(100)), txflags(tfPassive));
            env(offer(alice, MUSO(100), JPY(100)), txflags(tfPassive));
            env.close();

            env(pay(alice, bob, JPY(1)),
                path(~MUSO, ~EUR, ~MUSO, ~JPY),
                sendmax(USD(1)),
                txflags(tfNoMUSOirect),
                ter(temBAD_PATH_LOOP));
        }
    ...
    testTicketPay(FeatureBitset features)
    {
        ...
        env(pay(alice, bob, MUSO(1000)), ticket::use(ticketSeq));
        env.close();
        env.require(balance(bob, MUSO(1000)));
        env.require(balance(alice, MUSO(9000) - drops(20)));
    }
    ...
    testWithFeats(FeatureBitset features)
    {
        ...
        testSelfFundedMUSOEndpoint(false, features);
        testSelfFundedMUSOEndpoint(true, features);
...
BEAST_DEFINE_TESTSUITE_PRIO(Flow, app, MUSO, 2);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(Flow_manual, app, MUSO, 4);
