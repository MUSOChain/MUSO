...
class MUSOAmount_test : public beast::unit_test::suite
{
public:
    void
    testSigNum()
    ...
    testAddSub()
    {
        ...
        for (auto i : {-1, 0, 1})
        {
            ...
            for (auto j : {-1, 0, 1})
            {
                ...
                BEAST_EXPECT(MUSOAmount(i + j) == (x + y));
                BEAST_EXPECT(MUSOAmount(i - j) == (x - y));
    ...
    {
        // Tautology
        BEAST_EXPECT(DROPS_PER_MUSO.decimalMUSO() == 1);

        MUSOAmount test{1};
        BEAST_EXPECT(test.decimalMUSO() == 0.000001);

        test = -test;
        BEAST_EXPECT(test.decimalMUSO() == -0.000001);

        test = 100'000'000;
        BEAST_EXPECT(test.decimalMUSO() == 100);
    ...
    testFunctions()
    {
        // Explicitly test every defined function for the MUSOAmount class
        // since some of them are templated, but not used anywhere else.
        auto make = [&](auto x) -> MUSOAmount { return MUSOAmount{x}; };

        MUSOAmount defaulted;
        (void)defaulted;
        MUSOAmount test{0};
        ...
        MUSOAmount const targetSame{200u};
        test = make(targetSame);
        BEAST_EXPECT(test.drops() == 200);
        BEAST_EXPECT(test == targetSame);
        BEAST_EXPECT(test < MUSOAmount{1000});
        BEAST_EXPECT(test > MUSOAmount{100});
    ...
    testMulRatio()
    {
        ...
        constexpr auto maxMUSO =
            std::numeric_limits<MUSOAmount::value_type>::max();
        constexpr auto minMUSO =
            std::numeric_limits<MUSOAmount::value_type>::min();

        {
            // multiply by a number that would overflow then divide by the same
            // number, and check we didn't lose any value
            MUSOAmount big(maxMUSO);
...
{
    // underflow
    MUSOAmount bigNegative(minMUSO + 10);
    BEAST_EXPECT(mulRatio(bigNegative, 2, 1, true) == minMUSO);
}
}  // namespace MUSO
...
BEAST_DEFINE_TESTSUITE(MUSOAmount, protocol, MUSO);
