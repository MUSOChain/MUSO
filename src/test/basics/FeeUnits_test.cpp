...
private:
    ...
    {
        {
            ...
            BEAST_EXPECT((std::is_same_v<decltype(drops), MUSOAmount>));
        }
        {
            MUSOAmount x{100};
    ...
    testJson()
    {
        ...
        {
            MUSOAmount x{std::numeric_limits<std::int64_t>::max()};
            auto y = x.jsonClipped();
        ...
        {
            MUSOAmount x{std::numeric_limits<std::int64_t>::min()};
            auto y = x.jsonClipped();
            BEAST_EXPECT(y.type() == Json::intValue);
            BEAST_EXPECT(
...
public:
    ...
    run() override
    {
        BEAST_EXPECT(INITIAL_MUSO.drops() == 100'000'000'000'000'000);
        BEAST_EXPECT(INITIAL_MUSO == MUSOAmount{100'000'000'000'000'000});
...
BEAST_DEFINE_TESTSUITE(feeunits, MUSO_basics, MUSO);
