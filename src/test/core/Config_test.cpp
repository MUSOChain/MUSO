...
{
    static boost::format configContentsTemplate(R"MUSOConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
# This is primary persistent datastore for MUSO.  This includes transaction
# metadata, account states, and ledger headers.  Helpful information can be
# found here: https://MUSO.com/wiki/NodeBackEnd
# delete old ledgers while maintaining at least 2000. Do not require an
# external administrative command to initiate deletion.
[node_db]
type=memory
path=/Users/dummy/MUSO/config/db/rocksdb
open_files=2000
filter_bits=12
cache_mb=256
file_size_mb=8
...
[debug_logfile]
/Users/dummy/MUSO/config/log/debug.log
...
# Where to find some other servers speaking the MUSO protocol.
#
[ips]
r.MUSO.com 51235
...
/**
   Write a MUSO config file and remove when done.
 */
class MUSOCfgGuard : public MUSO::test::detail::FileDirGuard
{
private:
    path dataDir_;
...
public:
    ...
    ~MUSOCfgGuard()
    {
        ...
        catch (std::exception& e)
        {
            ...
            test_.log << "Error in ~MUSOCfgGuard: " << e.what() << std::endl;
        };
    }
};
...
valFileContents()
{
    std::string configContents(R"MUSOConfig(
[validators]
...
recommendedMUSOvalidators.com
moreMUSOvalidators.net
...
public:
    ...
    testLegacy()
    {
        ...
        std::string toLoad(R"MUSOConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
0
    ...
    testDbPath()
    {
        ...
        }
        {
            ...
            auto const cwd = current_path();
            MUSO::test::detail::DirGuard const g0(*this, "test_db");
            path const dataDirRel("test_data_dir");
            path const dataDirAbs(cwd / g0.subdir() / dataDirRel);
            detail::MUSOCfgGuard const g(
        ...
        }
        {
            ...
            detail::MUSOCfgGuard const g(*this, "test_db", dbPath, "");
            auto const& c(g.config());
        ...
        }
        {
            ...
            detail::MUSOCfgGuard const g(*this, "test_db", "", "");
            auto const& c(g.config());
    ...
    testValidatorKeys()
    {
        ...
        {
            Config c;
            static boost::format configTemplate(R"MUSOConfig(
[validation_seed]
%1%
...
021A99A537FDEBC34E4FCA03B39BEADD04299BB19E85097EC92B15A3518801E566
        ...
        }
        {
            ...
            // [validator_list_keys] are missing from MUSO cfg and
            // validators file
            Config c;
            boost::format cc("[validators_file]\n%1%\n");
            std::string error;
    ...
    {
        detail::MUSOCfgGuard const cfg(
            *this, "testSetup", explicitPath ? "test_db" : "", "");
        /* MUSOCfgGuard has a Config object that gets loaded on
    ...
    testPort()
    {
        detail::MUSOCfgGuard const cfg(*this, "testPort", "", "");
...
BEAST_DEFINE_TESTSUITE(Config, core, MUSO);
