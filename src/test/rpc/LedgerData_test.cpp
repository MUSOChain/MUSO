...
public:
    ...
    testLedgerType()
    {
        ...
        env(offer(Account{"bob0"}, USD(100), MUSO(100)));
...

        {
            ...
            jv[jss::Amount] = MUSO(50).value().getJson(JsonOptions::none);
...

        {
            ...
            jv[jss::Amount] = MUSO(100).value().getJson(JsonOptions::none);
        ...
        env(check::create("bob6", "bob7", MUSO(100)));
        ...
        {  // jvParams[jss::type] = "state";
            ...
            BEAST_EXPECT(checkArraySize(jrr[jss::state], 1));
            for (auto const& j : jrr[jss::state])
                BEAST_EXPECT(j["LedgerEntryType"] == jss::MUSOState);
        }
...
BEAST_DEFINE_TESTSUITE_PRIO(LedgerData, app, MUSO, 1);
