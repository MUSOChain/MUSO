...
class OwnerInfo_test : public beast::unit_test::suite
{
    ...
    testBasic()
    {
        ...
        env(offer(alice, USD(1), MUSO(1000)));
        env.close();

        env(pay(gw, alice, USD(50)));
        env(pay(gw, alice, CNY(50)));
        env(offer(alice, CNY(2), MUSO(1000)));
        ...
        if (!BEAST_EXPECT(result[jss::accepted].isMember(jss::MUSO_lines)))
            return;
        auto lines = result[jss::accepted][jss::MUSO_lines];
        ...
        BEAST_EXPECT(
            ...
            MUSO(1000).value().getJson(JsonOptions::none));
        BEAST_EXPECT(
            offers[0u][sfTakerPays.fieldName] ==
            USD(1).value().getJson(JsonOptions::none));

        // current ledger entry
        if (!BEAST_EXPECT(result[jss::current].isMember(jss::MUSO_lines)))
            return;
        lines = result[jss::current][jss::MUSO_lines];
        if (!BEAST_EXPECT(lines.isArray() && lines.size() == 2))
            return;
...
BEAST_DEFINE_TESTSUITE(OwnerInfo, app, MUSO);
