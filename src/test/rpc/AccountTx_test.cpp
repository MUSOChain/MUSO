...
class AccountTx_test : public beast::unit_test::suite
{
    ...
    testContents()
    {
        ...
        static const NodeSanity sanity[]{
            ...
            {16, jss::TrustSet,               {jss::MUSOState, jss::DirectoryNode, jss::DirectoryNode}, {},                               {jss::AccountRoot, jss::AccountRoot}},
...
/* "fund" creates becky's acct   */ { 5, jss::Payment,              {jss::AccountRoot}, {},                 {jss::AccountRoot}}
        ...
        env(pay(alice, becky, MUSO(45)));
        env.close();

        // becky's account root should be back.
        BEAST_EXPECT(env.closed()->exists(beckyAcctKey));
        BEAST_EXPECT(env.balance(becky) == MUSO(45));

        // becky pays alice.
        env(pay(becky, alice, MUSO(20)));
        env.close();
...
BEAST_DEFINE_TESTSUITE(AccountTx, app, MUSO);
