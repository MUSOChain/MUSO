...
#include <MUSO/beast/utility/temp_dir.h>
#include <MUSO/protocol/Feature.h>
#include <MUSO/protocol/jss.h>
#include <MUSO/resource/ResourceManager.h>
#include <MUSO/resource/impl/Entry.h>
#include <MUSO/resource/impl/Tuning.h>
...
class NoMUSOCheck_test : public beast::unit_test::suite
{
    void
    testBadInput()
    {
        testcase("Bad input to noMUSO_check");
        ...
        {  // missing account field
            auto const result =
                env.rpc("json", "noMUSO_check", "{}")[jss::result];
    ...
    {
        testcase << "Request noMUSO_check for " << (user ? "user" : "gateway")
        ...
        {
            env(fset(alice, asfDefaultMUSO));
            env(trust(alice, gw["USD"](100)));
        }
        else
        {
            env(fclear(alice, asfDefaultMUSO));
            env(trust(alice, gw["USD"](100), gw, tfSetNoMUSO));
        }
        env.close();
...
class NoMUSOCheckLimits_test : public beast::unit_test::suite
{
    void
    testLimits(bool admin)
    {
        ...
        for (auto i = 0; i < MUSO::RPC::Tuning::noMUSOCheck.rmax + 5; ++i)
        {
            if (!admin)
                checkBalance();
            ...
            env(pay(env.master, gw, MUSO(1000)),
                seq(autofill),
                fee(toDrops(
...
BEAST_DEFINE_TESTSUITE(NoMUSOCheck, app, MUSO);
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(NoMUSOCheckLimits, app, MUSO, 1);
