...
#include <MUSO/nodestore/impl/DecodedBlob.h>
#include <MUSO/nodestore/impl/Shard.h>
#include <chrono>
#include <numeric>
#include <test/jtx.h>
...
class DatabaseShard_test : public TestBase
{
    ...
    struct TestData
    {
        ...
        /* MUSOAmount_[i] is the amount for all payments on i-th ledger */
        std::vector<int> MUSOAmount_;
            ...
            : rng_(seedValue), nShards_(nShards)
        {
            ...
            for (std::uint32_t i = 0; i < nLedgers; ++i)
            {
                ...
                MUSOAmount_.push_back(rand_int(rng_, 90) + 10);
            }
        }
        ...
        makeLedgerData(test::jtx::Env& env_, std::uint32_t seq)
        {
            ...
            if (isNewAccounts(seq))
                env_.fund(MUSO(iniAmount), accounts_[nAccounts_[seq] - 1]);
        ...
        std::uint32_t seq)
    {
        ...
        for (auto const& tx : ledger->txs)
        {
            ...
            {
                std::int64_t MUSOAmount =
                    tx.first->getFieldAmount(sfAmount).MUSO().decimalMUSO();
                if (MUSOAmount == iniAmount)
                    ++iniCount;
                else
                {
                    ++payCount;
                    BEAST_EXPECT(MUSOAmount == data.MUSOAmount_[seq]);
                }
            }
...
BEAST_DEFINE_TESTSUITE_MANUAL(DatabaseShard, NodeStore, MUSO);
