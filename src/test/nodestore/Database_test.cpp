        ...
        int numObjsToTest = 2000)
    {
        ...
        if (type == "memory")
        {
            // Earliest ledger sequence tests
            {
                ...
                std::unique_ptr<Database> db =
                    Manager::instance().make_Database(
                        ...
                        journal_);
                BEAST_EXPECT(
                    db->earliestLedgerSeq() == MUSO_LEDGER_EARLIEST_SEQ);
            }
            ...
            try
            {
                ...
                nodeParams.set(
                    "earliest_seq", std::to_string(MUSO_LEDGER_EARLIEST_SEQ));
...
#if MUSO_ENABLE_SQLITE_BACKEND_TESTS
...
BEAST_DEFINE_TESTSUITE(Database, NodeStore, MUSO);
