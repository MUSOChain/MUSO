...
#include <MUSO/beast/hash/uhash.h>
...
public:
    ...
    testAmount()
    {
        ...
        BEAST_EXPECT(to_string(MUSO(5)) == "5 MUSO");
        BEAST_EXPECT(to_string(MUSO(.80)) == "0.8 MUSO");
        BEAST_EXPECT(to_string(MUSO(.005)) == "5000 drops");
        BEAST_EXPECT(to_string(MUSO(0.1)) == "0.1 MUSO");
        BEAST_EXPECT(to_string(MUSO(10000)) == "10000 MUSO");
        BEAST_EXPECT(to_string(drops(10)) == "10 drops");
        BEAST_EXPECT(to_string(drops(123400000)) == "123.4 MUSO");
        BEAST_EXPECT(to_string(MUSO(-5)) == "-5 MUSO");
        BEAST_EXPECT(to_string(MUSO(-.99)) == "-0.99 MUSO");
        BEAST_EXPECT(to_string(MUSO(-.005)) == "-5000 drops");
        BEAST_EXPECT(to_string(MUSO(-0.1)) == "-0.1 MUSO");
        BEAST_EXPECT(to_string(drops(-10)) == "-10 drops");
        BEAST_EXPECT(to_string(drops(-123400000)) == "-123.4 MUSO");

        BEAST_EXPECT(MUSO(1) == drops(1000000));
        BEAST_EXPECT(MUSO(1) == STAmount(1000000));
        BEAST_EXPECT(STAmount(1000000) == MUSO(1));
    ...
    testEnv()
    {
        ...
        // fund
        {
            ...
            env.fund(n, "dave", noMUSO("eric"));
            env.fund(n, "fred", noMUSO("gary", "hank"));
            env.fund(n, noMUSO("irene"));
            env.fund(n, noMUSO("jim"), "karen");
            env.fund(n, noMUSO("lisa", "mary"));

            // flags
            env.fund(n, noMUSO("xavier"));
            env.require(nflags("xavier", asfDefaultMUSO));
            env.fund(n, "yana");
            env.require(flags("yana", asfDefaultMUSO));
        }
    ...
    testRequire()
    {
        ...
        env.require(balance("alice", MUSO(none)));
        env.fund(MUSO(10000), "alice", gw);
        env.require(balance("alice", USD(none)));
        env.trust(USD(100), "alice");
        env.require(balance("alice", MUSO(10000)));  // fee refunded
    ...
    testPayments()
    {
        ...
        env.require(balance("alice", MUSO(10000)));
        env.require(balance("bob", MUSO(10000)));
        env.require(balance("carol", MUSO(10000)));
        env.require(balance(gw, MUSO(10000)));

        env(pay(env.master, "alice", MUSO(1000)), fee(none), ter(temMALFORMED));
        env(pay(env.master, "alice", MUSO(1000)), fee(1), ter(telINSUF_FEE_P));
        env(pay(env.master, "alice", MUSO(1000)), seq(none), ter(temMALFORMED));
        env(pay(env.master, "alice", MUSO(1000)), seq(20), ter(terPRE_SEQ));
        env(pay(env.master, "alice", MUSO(1000)), sig(none), ter(temMALFORMED));
        env(pay(env.master, "alice", MUSO(1000)), sig("bob"), ter(tefBAD_AUTH));

        env(pay(env.master, "dilbert", MUSO(1000)), sig(env.master));
        ...
        env(offer("carol", MUSO(50), USD(50)), require(owners("carol", 2)));
        env(pay("alice", "bob", any(USD(10))), ter(tecPATH_DRY));
        env(pay("alice", "bob", any(USD(10))),
            paths(MUSO),
            sendmax(MUSO(10)),
            ter(tecPATH_PARTIAL));
        env(pay("alice", "bob", any(USD(10))), paths(MUSO), sendmax(MUSO(20)));
    ...
    testFailHard()
    {
        ...
        jr = applyTxn(offer(alice, MUSO(1000), USD(1000)));
...
BEAST_DEFINE_TESTSUITE(Env, app, MUSO);
