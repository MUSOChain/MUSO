...
void
Env::fund(bool setDefaultMUSO, STAmount const& amount, Account const& account)
{
    memoize(account);
    if (setDefaultMUSO)
    {
        ...
        apply(
            ...
            sig(jtx::autofill));
        apply(
            fset(account, asfDefaultMUSO),
            jtx::seq(jtx::autofill),
            fee(jtx::autofill),
            sig(jtx::autofill));
        require(flags(account, asfDefaultMUSO));
    }
    else
    {
        apply(
            ...
            fee(jtx::autofill),
            sig(jtx::autofill));
        require(nflags(account, asfDefaultMUSO));
    }
