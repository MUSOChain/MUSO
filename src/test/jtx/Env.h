...
#ifndef MUSO_TEST_JTX_ENV_H_INCLUDED
#define MUSO_TEST_JTX_ENV_H_INCLUDED
...
/** Designate accounts as no-MUSO in Env::fund */
template <class... Args>
std::array<Account, 1 + sizeof...(Args)>
noMUSO(Account const& account, Args const&... args)
{
    return {{account, args...}};
}
...
supported_amendments()
{
    static const FeatureBitset ids = [] {
        auto const& sa = MUSO::detail::supportedAmendments();
...
public:
    ...
    /** Returns the current MUSO Network Time
    ...
    */
    ...
    /** Returns the MUSO balance on an account.
...
private:
    void
    fund(bool setDefaultMUSO, STAmount const& amount, Account const& account);
...
public:
    /** Create a new account with some MUSO.
        ...
        Effects:
            The asfDefaultMUSO on the account is set,
            and the sequence number is incremented, unless
            the account is wrapped with a call to noMUSO.

            The account's MUSO balance is set to amount.

            Generates a test that the balance is set.

        @param amount The amount of MUSO to transfer to
                      each account.

        @param args A heterogeneous list of accounts to fund
                    or calls to noMUSO with lists of accounts
                    to fund.
    */
