...
public:
    ...
    {
        {
            ...
            Issue const MUSO(MUSOIssue());

            // fractional MUSO (i.e. drops)
            testSetValue("1", MUSO);
            testSetValue("22", MUSO);
            testSetValue("333", MUSO);
            testSetValue("4444", MUSO);
            testSetValue("55555", MUSO);
            testSetValue("666666", MUSO);

            // 1 MUSO up to 100 billion, in powers of 10 (in drops)
            testSetValue("1000000", MUSO);
            testSetValue("10000000", MUSO);
            testSetValue("100000000", MUSO);
            testSetValue("1000000000", MUSO);
            testSetValue("10000000000", MUSO);
            testSetValue("100000000000", MUSO);
            testSetValue("1000000000000", MUSO);
            testSetValue("10000000000000", MUSO);
            testSetValue("100000000000000", MUSO);
            testSetValue("1000000000000000", MUSO);
            testSetValue("10000000000000000", MUSO);
            testSetValue("100000000000000000", MUSO);

            // Invalid native values:
            testSetValue("1.1", MUSO, false);
            testSetValue("100000000000000001", MUSO, false);
            testSetValue("1000000000000000000", MUSO, false);
        }
    ...
    testNativeCurrency()
    {
        ...
        unexpected(to_string(Currency()) != "MUSO", "cHC(MUSO)");
        Currency c;
    ...
    testCustomCurrency()
    {
        ...
        unexpected(
            multiply(STAmount(noIssue(), 20), STAmount(3), MUSOIssue())
                    .getText() != "60",
        ...
        unexpected(
            multiply(STAmount(20), STAmount(3), MUSOIssue()).getText() != "60",
        ...
        unexpected(
            divide(STAmount(noIssue(), 60), STAmount(3), MUSOIssue())
                    .getText() != "20",
        ...
        unexpected(
            divide(STAmount(noIssue(), 60), STAmount(noIssue(), 3), MUSOIssue())
                    .getText() != "20",
            "STAmount divide fail");
    ...
    testUnderflow()
    {
        ...
        bigDsmall = divide(smallValue, bigValue, MUSOIssue());

        BEAST_EXPECT(bigDsmall == beast::zero);

        bigDsmall = divide(smallValue, bigNative, MUSOIssue());
    ...
    testRounding()
    {
        ...
        STAmount dripTest1 = mulRound (twoThird2, two, MUSOIssue (), false);
        STAmount dripTest2 = multiply (twoThird2, two, MUSOIssue ());
        STAmount dripTest3 = mulRound (twoThird2, two, MUSOIssue (), true);
        log << dripTest1;
        log << dripTest2;
        log << dripTest3;
#endif
    ...
    testConvertMUSO()
    {
        testcase("STAmount to MUSOAmount conversions");
        ...
        for (std::uint64_t drops = 100000000000000000; drops != 1;
             drops = drops / 10)
        {
            auto const t = amountFromString(MUSO, std::to_string(drops));
            auto const s = t.MUSO();
            BEAST_EXPECT(s.drops() == drops);
            BEAST_EXPECT(t == STAmount(MUSOAmount(drops)));
            BEAST_EXPECT(s == MUSOAmount(drops));
        }

        try
        {
            auto const t = amountFromString(usd, "136500");
            fail(to_string(t.MUSO()));
        }
    ...
    testConvertIOU()
    {
        ...
        try
        {
            auto const t = amountFromString(MUSO, "136500");
...
BEAST_DEFINE_TESTSUITE(STAmount, MUSO_data, MUSO);
