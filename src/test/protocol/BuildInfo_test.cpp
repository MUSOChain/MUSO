...
public:
    ...
    testIsMUSOVersion()
    {
        testcase("IsMUSOVersion");
        auto vFF = 0xFFFF'FFFF'FFFF'FFFFLLU;
        BEAST_EXPECT(!BuildInfo::isMUSOVersion(vFF));
        auto vMUSO = 0x183B'0000'0000'0000LLU;
        BEAST_EXPECT(BuildInfo::isMUSOVersion(vMUSO));
    }
...
BEAST_DEFINE_TESTSUITE(BuildInfo, protocol, MUSO);
}  // namespace MUSO
