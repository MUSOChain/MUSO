...
class View_test : public beast::unit_test::suite
{
    ...
    testFlags()
    {
        ...
        env.trust(USD(100), alice, bob, carol);
        {
            ...
            env(offer(alice, MUSO(5), USD(5)));

            // Now freeze gw.
            env(fset(gw, asfGlobalFreeze));
            env.close();
            env(offer(alice, MUSO(4), USD(5)), ter(tecFROZEN));
            env.close();
        ...
        }
        {
            ...
            // carol's MUSO balance should be her holdings minus her reserve.
            auto const carolsMUSO = accountHolds(
                *env.closed(),
                carol,
                MUSOCurrency(),
                MUSOAccount(),
                fhZERO_IF_FROZEN,
                env.journal);
            // carol's MUSO balance:              10000
            // base reserve:                      -200
            // 1 trust line times its reserve: 1 * -50
            //                                 -------
            // carol's available balance:         9750
            BEAST_EXPECT(carolsMUSO == MUSO(9750));

            // carol should be able to spend *more* than her MUSO balance on
            // a fee by eating into her reserve.
            env(noop(carol), fee(carolsMUSO + MUSO(10)));
            env.close();

            // carol's MUSO balance should now show as zero.
            BEAST_EXPECT(
                MUSO(0) ==
                accountHolds(
                    *env.closed(),
    ...
    testAreCompatible()
    {
        ...
        eA.fund(MUSO(10000), alice);
        eA.close();
        auto const rdViewA3 = eA.closed();

        eA.fund(MUSO(10000), bob);
        eA.close();
        ...
        eB.fund(MUSO(10000), bob);
        eB.close();
        auto const rdViewB3 = eB.closed();

        eB.fund(MUSO(10000), alice);
        eB.close();
    ...
    testRegressions()
    {
        ...
        // Make sure OpenLedger::empty works
        {
            ...
            env.fund(MUSO(10000), Account("test"));
            BEAST_EXPECT(!env.app().openLedger().empty());
        }
    }
...
BEAST_DEFINE_TESTSUITE(View, ledger, MUSO);
BEAST_DEFINE_TESTSUITE(GetAmendments, ledger, MUSO);
