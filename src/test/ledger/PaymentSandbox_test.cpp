...
#include <MUSO/protocol/AmountConversions.h>
...
class PaymentSandbox_test : public beast::unit_test::suite
{
    ...
    */
    ...
    testSelfFunding(FeatureBitset features)
    {
        ...
        env.fund(MUSO(10000), snd, rcv, gw1, gw2);
    ...
    testSubtractCredits(FeatureBitset features)
    {
        ...
        env.fund(MUSO(10000), alice, gw1, gw2);
...

        {
            ...
            MUSOCredit(av, gw1, alice, toCredit, true, j);
            BEAST_EXPECT(
                accountHolds(
                    av, alice, iss.currency, iss.account, fhIGNORE_FREEZE, j) ==
                startingAmount + toCredit);

            MUSOCredit(av, alice, gw1, toDebit, true, j);
            BEAST_EXPECT(
                accountHolds(
                    av, alice, iss.currency, iss.account, fhIGNORE_FREEZE, j) ==
                startingAmount + toCredit - toDebit);
...

        {
            ...
            MUSOCredit(pv, gw1, alice, toCredit, true, j);
            BEAST_EXPECT(
                accountHolds(
                    pv, alice, iss.currency, iss.account, fhIGNORE_FREEZE, j) ==
                startingAmount);
    ...
    testReserve(FeatureBitset features)
    {
        ...
        auto accountFundsMUSO = [](ReadView const& view,
                                  AccountID const& id,
                                  beast::Journal j) -> MUSOAmount {
            return toAmount<MUSOAmount>(accountHolds(
                view, id, MUSOCurrency(), MUSOAccount(), fhZERO_IF_FROZEN, j));
        };

        auto reserve = [](jtx::Env& env, std::uint32_t count) -> MUSOAmount {
        ...
        PaymentSandbox sb(&av);
        {
            ...
            {
                auto r =
                    accountSend(sb, MUSOAccount(), alice, MUSO(100), env.journal);
                BEAST_EXPECT(r == tesSUCCESS);
            }
            {
                auto r =
                    accountSend(sb, alice, MUSOAccount(), MUSO(100), env.journal);
                BEAST_EXPECT(r == tesSUCCESS);
            }
            BEAST_EXPECT(
                accountFundsMUSO(sb, alice, env.journal) == beast::zero);
        }
    }
...
BEAST_DEFINE_TESTSUITE(PaymentSandbox, ledger, MUSO);
