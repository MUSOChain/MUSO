...
struct Directory_test : public beast::unit_test::suite
{
    ...
    testDirectoryOrdering()
    {
        ...
        // the offers.
        auto book = BookDirs(*env.current(), Book({MUSOIssue(), USD.issue()}));
        int count = 1;
        ...
        for (auto const& offer : book)
        {
            ...
            BEAST_EXPECT(offer->getFieldAmount(sfTakerGets) == MUSO(count));
        }
    }
    ...
    testDirIsEmpty()
    {
        ...
        env.fund(MUSO(1000000), alice, charlie, gw);
        env.close();
        ...
        // implicit trust lines.
        {
            ...
            for (auto const& c : cl)
            {
                env(offer(charlie, MUSO(50), c(50)));
                env.close();
            }
            BEAST_EXPECT(!dirIsEmpty(*env.closed(), keylet::ownerDir(alice)));
    ...
    testRipd1353()
    {
        ...
        // should have no entries and be empty:
        {
            ...
            uint256 const bookBase = getBookBase({MUSOIssue(), USD.issue()});
...
BEAST_DEFINE_TESTSUITE_PRIO(Directory, ledger, MUSO, 1);
