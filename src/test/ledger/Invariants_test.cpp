...
#include <MUSO/app/tx/impl/ApplyContext.h>
...
class Invariants_test : public beast::unit_test::suite
{
    ...
    {
        using namespace test::jtx;
        testcase << "MUSO created";
        doInvariantCheck(
            {{"MUSO net change was positive: 500"}},
            [](Account const& A1, Account const&, ApplyContext& ac) {
                // put a single account in the view and "manufacture" some MUSO
    ...
    testTypesMatch()
    {
        ...
        doInvariantCheck(
            {{"ledger entry type mismatch"},
             {"MUSO net change of -1000000000 doesn't match fee 0"}},
            [](Account const& A1, Account const&, ApplyContext& ac) {
    ...
    {
        using namespace test::jtx;
        testcase << "trust lines with MUSO not allowed";
        doInvariantCheck(
            {{"an MUSO trust line was created"}},
            [](Account const& A1, Account const& A2, ApplyContext& ac) {
                // create simple trust SLE with MUSO currency
                auto const sleNew = std::make_shared<SLE>(
                    keylet::line(A1, A2, MUSOIssue().currency));
                ac.view().insert(sleNew);
                return true;
            });
    }
    ...
    void
    testMUSOBalanceCheck()
    {
        using namespace test::jtx;
        testcase << "MUSO balance checks";

        doInvariantCheck(
            {{"Cannot return non-native STAmount as MUSOAmount"}},
            [](Account const& A1, Account const& A2, ApplyContext& ac) {
                // non-native balance
                auto const sle = ac.view().peek(keylet::account(A1.id()));
                if (!sle)
        ...
        doInvariantCheck(
            {{"incorrect account MUSO balance"},
             {"MUSO net change was positive: 99999999000000001"}},
            [this](Account const& A1, Account const&, ApplyContext& ac) {
                ...
                sle->setFieldAmount(sfBalance, INITIAL_MUSO + drops(1));
        ...
        doInvariantCheck(
            {{"incorrect account MUSO balance"},
             {"MUSO net change of -1000000001 doesn't match fee 0"}},
            [this](Account const& A1, Account const&, ApplyContext& ac) {
                // balance is negative
                auto const sle = ac.view().peek(keylet::account(A1.id()));
                if (!sle)
    ...
    testTransactionFeeCheck()
    {
        ...
        doInvariantCheck(
            {{"fee paid was negative: -1"},
             {"MUSO net change of 0 doesn't match fee -1"}},
            [](Account const&, Account const&, ApplyContext&) { return true; },
            MUSOAmount{-1});

        doInvariantCheck(
            {{"fee paid exceeds system limit: "s + to_string(INITIAL_MUSO)},
             {"MUSO net change of 0 doesn't match fee "s +
              to_string(INITIAL_MUSO)}},
            [](Account const&, Account const&, ApplyContext&) { return true; },
            MUSOAmount{INITIAL_MUSO});

        doInvariantCheck(
            {{"fee paid is 20 exceeds fee specified in transaction."},
             {"MUSO net change of 0 doesn't match fee 20"}},
            [](Account const&, Account const&, ApplyContext&) { return true; },
            MUSOAmount{20},
            STTx{ttACCOUNT_SET, [](STObject& tx) {
                     tx.setFieldAmount(sfFee, MUSOAmount{10});
                 }});
    }
    ...
    testNoBadOffers()
    {
        ...
        doInvariantCheck(
            ...
            [](Account const& A1, Account const&, ApplyContext& ac) {
                ...
                sleNew->setFieldAmount(sfTakerPays, MUSO(-1));
                ac.view().insert(sleNew);
                return true;
            });
        ...
        doInvariantCheck(
            ...
            [](Account const& A1, Account const&, ApplyContext& ac) {
                ...
                sleNew->setFieldAmount(sfTakerGets, MUSO(-1));
                ac.view().insert(sleNew);
                return true;
            });
        ...
        doInvariantCheck(
            ...
            [](Account const& A1, Account const&, ApplyContext& ac) {
                ...
                sleNew->setAccountID(sfAccount, A1.id());
                sleNew->setFieldU32(sfSequence, (*sle)[sfSequence]);
                sleNew->setFieldAmount(sfTakerPays, MUSO(10));
                sleNew->setFieldAmount(sfTakerGets, MUSO(11));
                ac.view().insert(sleNew);
                return true;
            });
    }
    ...
    testNoZeroEscrow()
    {
        ...
        doInvariantCheck(
            {{"MUSO net change of -1000000 doesn't match fee 0"},
            ...
            [](Account const& A1, Account const&, ApplyContext& ac) {
                ...
                auto sleNew = std::make_shared<SLE>(
                    keylet::escrow(A1, (*sle)[sfSequence] + 2));
                sleNew->setFieldAmount(sfAmount, MUSO(-1));
                ac.view().insert(sleNew);
                return true;
            });

        doInvariantCheck(
            {{"MUSO net change was positive: 100000000000000001"},
             {"escrow specifies invalid amount"}},
            [](Account const& A1, Account const&, ApplyContext& ac) {
                ...
                // Use `drops(1)` to bypass a call to STAmount::canonicalize
                // with an invalid value
                sleNew->setFieldAmount(sfAmount, INITIAL_MUSO + drops(1));
                ac.view().insert(sleNew);
                return true;
            });
    }
...
BEAST_DEFINE_TESTSUITE(Invariants, ledger, MUSO);
