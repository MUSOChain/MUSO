...
struct BookDirs_test : public beast::unit_test::suite
{
    ...
    test_bookdir(FeatureBitset features)
    {
        ...
        auto gw = Account("gw");
        auto USD = gw["USD"];
        env.fund(MUSO(1000000), "alice", "bob", "gw");

        {
            Book book(MUSOIssue(), USD.issue());
            {
        ...
        {
            env(offer("alice", Account("alice")["USD"](50), MUSO(10)));
            auto d = BookDirs(
                *env.current(),
                Book(Account("alice")["USD"].issue(), MUSOIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 1);
        }

        {
            env(offer("alice", gw["CNY"](50), MUSO(10)));
            auto d =
                BookDirs(*env.current(), Book(gw["CNY"].issue(), MUSOIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 1);
        }
...

        {
            ...
            for (auto i = 1, j = 3; i <= 3; ++i, --j)
                for (auto k = 0; k < 80; ++k)
                    env(offer("alice", AUD(i), MUSO(j)));

            auto d = BookDirs(*env.current(), Book(AUD.issue(), MUSOIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 240);
            auto i = 1, j = 3, k = 0;
            for (auto const& e : d)
            {
                BEAST_EXPECT(e->getFieldAmount(sfTakerPays) == AUD(i));
                BEAST_EXPECT(e->getFieldAmount(sfTakerGets) == MUSO(j));
                if (++k % 80 == 0)
                {
                    ++i;
                    --j;
...
BEAST_DEFINE_TESTSUITE(BookDirs, ledger, MUSO);
